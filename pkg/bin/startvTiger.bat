...
:initiate
rem if true means , ncrm crm mysql is being used
...
:StartApacheService
echo ""
echo "making an attempt to kill any existing ncrmcrm service"
echo ""
bin\apache -k stop -n ncrmcrm504
...
echo ""
echo ""
echo "Installing  ncrmcrm504 apache service after sleeping for 10 seconds"
echo ""
%SLEEP_STR% -n 10 127.0.0.1>nul
bin\apache -k install -n ncrmcrm504 -f conf\httpd.conf
echo ""
echo "Starting  ncrmcrm504 apache service"
echo ""
bin\apache -n ncrmcrm504 -k start
IF ERRORLEVEL 1 goto stopservice
goto checkmysql
...
echo ""
echo ""
echo "Apache in the location %apache_dir% is not running. Start Apache and then start ncrm crm"
echo ""
echo ""
...
:checkdatabase
echo ""
echo "check to see if ncrmcrm504 database already exists"
echo ""
mysql --port=%mysql_port% --user=%mysql_username% --password=%mysql_password% -e "show databases like 'ncrmcrm504'" | "%WINDIR%\system32\find.exe" "ncrmcrm504" > NUL
IF ERRORLEVEL 1 goto dbnotexists
echo ""
ECHO  "ncrmcrm504 database exists"
echo ""
goto end
...
:dbnotexists
echo ""
ECHO "ncrmcrm504 database does not exist"
echo ""
echo %cd%
echo ""
echo "Proceeding to create database ncrmcrm504 and populate the same"
echo ""
mysql --user=%mysql_username% --password=%mysql_password% --port=%mysql_port% -e "create database if not exists ncrmcrm504"
echo ""
echo "ncrmcrm504 database created"
echo ""
goto end
...
echo ""
echo ""
echo "********* Change the apache port in startnCrm.bat and stopnCrm.bat too and then access the product from the browser in the following manner http://localhost:apacheport******* "
echo ""
echo ""
echo "Thank You"
echo ""
