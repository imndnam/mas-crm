...
class Mobile_ICAL {
	...
	function generate() {
		...
		$properties = array();
		$properties['prodid'] = '-//ncrm/Mobile/NONSGML 1.0//EN';
		...
		$query = "SELECT " . implode(',', $fieldnames) . " FROM ncrm_activity
			INNER JOIN ncrm_crmentity ON
			(ncrm_activity.activityid=ncrm_crmentity.crmid 	AND ncrm_crmentity.deleted = 0 AND ncrm_crmentity.smownerid = ?)
			LEFT JOIN ncrm_activity_reminder ON ncrm_activity_reminder.activity_id=ncrm_activity.activityid
