...
class Mobile_WS_RelatedRecordsWithGrouping extends Mobile_WS_QueryWithGrouping {
	...
	function process(Mobile_API_Request $request) {
		...
		if ($functionHandler) {
			...
			$querySEtype = "ncrm_crmentity.setype as setype";
			if ($relatedmodule == 'Calendar') {
				$querySEtype = "ncrm_activity.activitytype as setype";
			}
			
			$query = sprintf("SELECT ncrm_crmentity.crmid, $querySEtype %s", substr($query, stripos($query, 'FROM')));
