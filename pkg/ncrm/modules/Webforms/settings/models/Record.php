...
class Settings_Webforms_Record_Model extends Settings_Ncrm_Record_Model {
	 ...
	 * @return <Array> list of link models <Ncrm_Link_Model>
	 */
	public function getRecordLinks() {
		...
		$recordLinks = array(
				...
				array(
						...
						'linkurl' => "javascript:Settings_Ncrm_List_Js.triggerDelete(event,'".$this->getDeleteUrl()."');",
						'linkicon' => 'icon-trash'
				)
		);
	...
	public function getDetailViewLinks() {
		...
		foreach ($detailViewLinks as $detailViewLink) {
			$linkModelList['DETAILVIEWBASIC'][] = Ncrm_Link_Model::getInstanceFromValues($detailViewLink);
		}
	...
	public function getSelectedFieldsList($mode='') {
		if (!$this->selectedFields) {
			...
			if ($targetModule) {
				...
				$targetModuleModel = Ncrm_Module_Model::getInstance($targetModule);
				$allFields = $targetModuleModel->getFields();

				$result = $db->pquery("SELECT * FROM ncrm_webforms_field WHERE webformid = ? ORDER BY sequence", array($this->getId()));
	...
	public function save() {
		...
		if ($mode) {
			$updateQuery = "UPDATE ncrm_webforms SET description = ?, returnurl = ?, ownerid = ?, enabled = ?, captcha = ? , roundrobin = ?, roundrobin_userid = ?, roundrobin_logic = ? ,targetmodule = ? WHERE id = ?";
			...
			$insertQuery = "INSERT INTO ncrm_webforms(name, targetmodule, publicid, enabled, description, ownerid, returnurl, captcha, roundrobin, roundrobin_userid, roundrobin_logic) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		...
		$db->pquery("DELETE FROM ncrm_webforms_field WHERE webformid = ?", array($this->getId()));

		//Saving data of source module fields info for this webform
		$selectedFieldsData = $this->get('selectedFieldsData');
		$sourceModuleModel = Ncrm_Module_Model::getInstance($this->get('targetmodule'));

		$fieldInsertQuery = "INSERT INTO ncrm_webforms_field(webformid, fieldname, neutralizedfield, defaultvalue, required, sequence, hidden) VALUES(?, ?, ?, ?, ?, ?, ?)";
		foreach ($selectedFieldsData as $fieldName => $fieldDetails) {
			...
			$fieldModel = Ncrm_Field_Model::getInstance($fieldName, $sourceModuleModel);
			...
			if ($dataType === 'date') {
				$fieldDefaultValue = Ncrm_Date_UIType::getDBInsertedValue($fieldDefaultValue);
			}
	
			if ($dataType === 'time') {
				$fieldDefaultValue = Ncrm_Time_UIType::getTimeValueWithSeconds($fieldDefaultValue);
			}
			...
			if ($dataType === 'reference') {
				...
				$referenceObject = NcrmWebserviceObject::fromName($db,$referenceModule);
	...
	public function checkDuplicate() {
		...
		$query = "SELECT 1 FROM ncrm_webforms WHERE name = ?";
	...
	static public function getInstanceById($recordId, $qualifiedModuleName) {
		...
		$result = $db->pquery("SELECT * FROM ncrm_webforms WHERE id = ?", array($recordId));
		if ($db->num_rows($result)) {
	 ...
	 * @return <Settings_Ncrm_Module_Model>
	 */
	static public function getCleanInstance($moduleName) {
		$recordModel = new self();
		$moduleModel = Settings_Ncrm_Module_Model::getInstance($moduleName);
