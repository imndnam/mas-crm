					...
					<div class="widget_header row-fluid">
						<span class="span9">
							<div class="row-fluid">
								...
								<span class="span7">
									{assign var=RELATED_MODULE_MODEL value=Ncrm_Module_Model::getInstance('HelpDesk')}
									{assign var=FIELD_MODEL value=$RELATED_MODULE_MODEL->getField('ticketstatus')}
									{assign var="FIELD_INFO" value=Zend_Json::encode($FIELD_MODEL->getFieldInfo())}
									{assign var=PICKLIST_VALUES value=$FIELD_MODEL->getPicklistValues()}
									{assign var="SPECIAL_VALIDATOR" value=$FIELD_MODEL->getValidator()}
									<select class="chzn-select" name="{$FIELD_MODEL->get('name')}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Ncrm_Base_Validator_Js.invokeValidation]]" data-fieldinfo='{$FIELD_INFO|escape}' {if !empty($SPECIAL_VALIDATOR)}data-validator='{Zend_Json::encode($SPECIAL_VALIDATOR)}'{/if} >
					...
					<div class="widget_header row-fluid">
						<span class="span9">
							<div class="row-fluid">
								...
								<span class="span7">
									{assign var=RELATED_MODULE_MODEL value=Ncrm_Module_Model::getInstance('ProjectTask')}
