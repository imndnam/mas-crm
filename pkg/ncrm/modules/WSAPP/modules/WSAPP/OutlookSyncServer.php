...
class OutlookSyncServer extends SyncServer{
    ...
    function getDestinationHandleDetails(){
        return array('handlerclass' => 'OutlookNcrmCRMHandler',
            'handlerpath' => 'modules/WSAPP/Handlers/OutlookNcrmCRMHandler.php');
	}
     ...
     function put($key, $element, $user) {
        ...
        foreach ($records as $record) {
            ...
            if (!empty($record['crmid'])) {
                ...
                $lookupResult = $db->pquery("SELECT crmid,modifiedtime FROM ncrm_crmentity WHERE crmid=?", array($crmid[1]));
