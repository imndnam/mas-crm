...
class RecycleBin_Module_Model extends Ncrm_Module_Model {
	...
	public function getListViewMassActions() {
		...
		foreach($massActionLinks as $massActionLink) {
			$links[] = Ncrm_Link_Model::getInstanceFromValues($massActionLink);
		}
	...
	public function emptyRecycleBin(){
		...
		$getIdsQuery='SELECT crmid from ncrm_crmentity WHERE deleted=?';
		...
		$db->query('DELETE FROM ncrm_crmentity WHERE deleted = 1');
		$db->query('DELETE FROM ncrm_relatedlists_rb');
	...
	public function deleteRecords($recordIds){
	        $db = PearDatabase::getInstance(); 
		//Delete the records in ncrm crmentity and relatedlists.
		$query = 'DELETE FROM ncrm_crmentity WHERE deleted = ? and crmid in('.generateQuestionMarks($recordIds).')';
		$db->pquery($query, array(1, $recordIds));
		
		$query = 'DELETE FROM ncrm_relatedlists_rb WHERE entityid in('.generateQuestionMarks($recordIds).')';
		$db->pquery($query, array($recordIds));

		// Delete entries of attachments from ncrm_attachments and ncrm_seattachmentsrel
		$this->deleteFiles($recordIds);
	...
	public function deleteFiles($recordIds){
		$db = PearDatabase::getInstance(); 
		$getAttachmentsIdQuery='SELECT * FROM ncrm_seattachmentsrel WHERE crmid in('.generateQuestionMarks($recordIds).')';
		...
		if(!empty($attachmentsIds)){
                        $deleteRelQuery='DELETE FROM ncrm_seattachmentsrel WHERE crmid in('.generateQuestionMarks($recordIds).')';
                        $db->pquery($deleteRelQuery,array($recordIds));
                        $attachmentsLocation=array();
                        $getPathQuery='SELECT * FROM ncrm_attachments WHERE attachmentsid in ('.generateQuestionMarks($attachmentsIds).')';
                        ...
                        $deleteAttachmentQuery='DELETE FROM ncrm_attachments WHERE attachmentsid in ('.generateQuestionMarks($attachmentsIds).')';
	...
	public function restore($sourceModule, $recordIds){
		...
		for($i=0;$i<count($recordIds);$i++) {
          ...
          public function getDeletedRecordsTotalCount() {  
                $db = PearDatabase::getInstance();  
                $totalCount = $db->pquery('select count(*) as count from ncrm_crmentity where deleted=1',array());  
