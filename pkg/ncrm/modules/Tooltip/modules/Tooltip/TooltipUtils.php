...
function getFieldList($module_name, $field_name = "") {
	...
	$query = "select * from ncrm_field where tabid = ?";
	$params = array($tabid);
	if (!empty ($field_name)) {
		$query .= " and fieldname not like ?";
		$params = array($tabid,$field_name);
	}
	$query.= " and columnname not like 'imagename' and uitype not in (61, 122) and ncrm_field.presence in (0,2)";
...
function moduleList() {
	global $adb;
	$sql = "select distinct ncrm_field.tabid,name from ncrm_field inner join ncrm_tab on ncrm_field.tabid=ncrm_tab.tabid where name not in ('Calendar','Events','Users')";
...
function tooltip_exists($fieldid, $related_fieldid) {
	global $adb;
	$query = "select * from ncrm_quickview where fieldid=? and related_fieldid=?";
...
function getToolTipText($view,$fieldname,$module,$value){
	...
	$quickview = 'select fieldname,fieldlabel from ncrm_quickview inner join ncrm_field on ncrm_quickview.related_fieldid=ncrm_field.fieldid where ncrm_quickview.fieldid = ? and currentview= ? and ncrm_field.presence in (0,2) order by ncrm_quickview.sequence';
...
function ToolTipExists($fieldname,$tabid){
	if(empty($fieldname) || empty($tabid)){
		...
		global $adb;
		$sql = "select fieldid from ncrm_field where tabid = ? and fieldname = ? and ncrm_field.presence in (0,2)";
		$result = $adb->pquery($sql,array($tabid,$fieldname));
		$count = $adb->num_rows($result);
		if($count > 0){
			$fieldid = $adb->query_result($result,0,'fieldid');
		
			$sql = "select * from ncrm_quickview where fieldid = ?";
...
function QuickViewFieldList($module){
	...
	$query = "select * from ncrm_field where tabid = ? and columnname not like 'imagename' and uitype not in (61, 122) and ncrm_field.presence in (0,2)";
