...
class MailManager_Relation_View extends MailManager_Abstract_View {
      ...
      if ($folder=="Sent"){
        $contacts=$request->get('_msendto');
			...
			if (!class_exists($quickCreateviewClassName)) {
				$quickCreateviewClassName = 'Ncrm_QuickCreateAjax_View';
			}
			...
			$recordModel = Ncrm_Record_Model::getCleanInstance($linkModule);
			...
			switch ($linkModule) {
				case 'Calendar' :   $activityType = $recordModel->get('activitytype');
									...
									if ($activityType != 'Task') {
										$endTime = Ncrm_Time_UIType::getTimeValueWithSeconds($request->get('time_end'));
	...
	public function setParentForHelpDesk($parent, $from) {
		...
		if(empty($parent)) {
			if(!empty($from)) {
				$parentInfo = MailManager::lookupMailInNcrm($from[0], $currentUserModel);
