...
class PBXManager_Record_Model extends Ncrm_Record_Model{
    
    const moduletableName = 'ncrm_pbxmanager';
    const lookuptableName = 'ncrm_pbxmanager_phonelookup';
    const entitytableName = 'ncrm_crmentity';
    ...
    public function saveRecordWithArrray($params){
        $moduleModel = Ncrm_Module_Model::getInstance('PBXManager');
        $recordModel = Ncrm_Record_Model::getCleanInstance('PBXManager');
    ...
    public static function getUserNumbers(){
        $numbers = null;
        $db = PearDatabase::getInstance();
        $query = 'SELECT id, phone_crm_extension FROM ncrm_users';
        $result = $db->pquery($query, array());
