...
function LogintoNcrmCRM($user_name,$password,$version)
{
	global $log,$adb;
	require_once('modules/Users/Users.php');
	include('ncrmversion.php');
	if($version != $ncrm_current_version)
	{
...
function create_account($username,$sessionid,$accountname,$email,$phone,$primary_address_street,$primary_address_city,$primary_address_state,$primary_address_postalcode,$primary_address_country)
{
	...
	if(isPermitted("Accounts","EditView") == "yes")
	{
		$query = "SELECT accountname FROM ncrm_account,ncrm_crmentity WHERE accountname =? and ncrm_account.accountid = ncrm_crmentity.crmid and ncrm_crmentity.deleted != 1";
...
function GetPicklistValues($username,$sessionid,$tablename)
{
	...
	{
		$query = "select " . $adb->sql_escape_string($tablename) . " from ncrm_". $adb->sql_escape_string($tablename);
	...
	{
		$query = "select " .$adb->sql_escape_string($tablename) . " from ncrm_". $adb->sql_escape_string($tablename) ." inner join ncrm_role2picklist on ncrm_role2picklist.picklistvalueid = ncrm_". $adb->sql_escape_string($tablename) .".picklist_valueid where roleid=? and picklistid in (select picklistid from ncrm_". $adb->sql_escape_string($tablename)." ) order by sortid";
...
function unsetServerSessionId($id)
{
	...
	$adb->query("delete from ncrm_soapservice where type='FireFox' and id=$id");
...
function getServerSessionId($id)
{
	...
	$query = "select * from ncrm_soapservice where type='FireFox' and id={$id}";
