...
include_once 'vtlib/Ncrm/Functions.php';
...
$NAMESPACE = 'http://www.ncrm.com/products/crm';
 ...
 */
class Ncrm_Soap_CustomerPortal {
...
function _getTicketModComments($ticketId) {
	global $adb;
	$sql = "SELECT * FROM ncrm_modcomments
			INNER JOIN ncrm_crmentity ON ncrm_modcomments.modcommentsid = ncrm_crmentity.crmid AND deleted = 0
	...
	for($i=0; $i<$rows; $i++) {
		...
		if(!empty($customer)) {
			$emailResult = $adb->pquery('SELECT * FROM ncrm_portalinfo WHERE id = ?', array($customer));
...
function get_combo_values($input_array)
{
	...
	$output = Array();
	$sql = "select  productid, productname from ncrm_products inner join ncrm_crmentity on ncrm_crmentity.crmid=ncrm_products.productid where ncrm_crmentity.deleted=0";
	...
	$roleres = $adb->pquery("SELECT roleid from ncrm_user2role where userid = ?",array($userid));
	$RowCount = $adb->num_rows($roleres);
	if($RowCount > 0){
		$admin_role = $adb->query_result($roleres,0,'roleid');
	}
	$result1 = $adb->pquery("select ncrm_ticketpriorities.ticketpriorities from ncrm_ticketpriorities inner join ncrm_role2picklist on ncrm_role2picklist.picklistvalueid = ncrm_ticketpriorities.picklist_valueid and ncrm_role2picklist.roleid='$admin_role' order by sortorderid", array());
	for($i=0;$i<$adb->num_rows($result1);$i++)
	{
		$output['ticketpriorities']['ticketpriorities'][$i] = $adb->query_result($result1,$i,"ticketpriorities");
	}

	$result2 = $adb->pquery("select ncrm_ticketseverities.ticketseverities from ncrm_ticketseverities inner join ncrm_role2picklist on ncrm_role2picklist.picklistvalueid = ncrm_ticketseverities.picklist_valueid and ncrm_role2picklist.roleid='$admin_role' order by sortorderid", array());
	for($i=0;$i<$adb->num_rows($result2);$i++)
	{
		$output['ticketseverities']['ticketseverities'][$i] = $adb->query_result($result2,$i,"ticketseverities");
	}

	$result3 = $adb->pquery("select ncrm_ticketcategories.ticketcategories from ncrm_ticketcategories inner join ncrm_role2picklist on ncrm_role2picklist.picklistvalueid = ncrm_ticketcategories.picklist_valueid and ncrm_role2picklist.roleid='$admin_role' order by sortorderid", array());
	...
	if(!vtlib_isModuleActive('ServiceContracts')) {
		...
		$servicequery = "SELECT ncrm_servicecontracts.servicecontractsid,ncrm_servicecontracts.subject
							FROM ncrm_servicecontracts
							INNER JOIN ncrm_crmentity on ncrm_crmentity.crmid=ncrm_servicecontracts.servicecontractsid
									AND ncrm_crmentity.deleted = 0
							WHERE ncrm_servicecontracts.sc_related_to = ?";
		$params = array($id);
		$showAll = show_all('HelpDesk');
		if($showAll == 'true') {
			$servicequery .= ' OR ncrm_servicecontracts.sc_related_to = (SELECT accountid FROM ncrm_contactdetails WHERE contactid=? AND accountid <> 0)
								OR ncrm_servicecontracts.sc_related_to IN
											(SELECT contactid FROM ncrm_contactdetails WHERE accountid =
													(SELECT accountid FROM ncrm_contactdetails WHERE contactid=? AND accountid <> 0))
							';
...
function get_KBase_details($input_array)
{
	...
	$category_query = "select ncrm_faqcategories.faqcategories from ncrm_faqcategories inner join ncrm_role2picklist on ncrm_role2picklist.picklistvalueid = ncrm_faqcategories.picklist_valueid and ncrm_role2picklist.roleid='$admin_role'";
	...
	if($check == true) {
		$product_query = "select productid, productname from ncrm_products inner join ncrm_crmentity on ncrm_crmentity.crmid=ncrm_products.productid where ncrm_crmentity.deleted=0";
	...
	$faq_query = "select ncrm_faq.*, ncrm_crmentity.createdtime, ncrm_crmentity.modifiedtime from ncrm_faq " .
		"inner join ncrm_crmentity on ncrm_crmentity.crmid=ncrm_faq.id " .
		"where ncrm_crmentity.deleted=0 and ncrm_faq.status='Published' order by ncrm_crmentity.modifiedtime DESC";
	...
	for($k=0;$k<$faq_noofrows;$k++)
	{
		...
		$faq_comment_query = "select * from ncrm_faqcomments where faqid=? order by createdtime DESC";
...
function save_faq_comment($input_array)
{
	...
	if(trim($comment) != '')
	{
		$faq_query = "insert into ncrm_faqcomments values(?,?,?,?)";
...
function get_tickets_list($input_array) {
	...
	{
		$contactquery = "SELECT contactid, accountid FROM ncrm_contactdetails " .
			" INNER JOIN ncrm_crmentity ON ncrm_crmentity.crmid = ncrm_contactdetails.contactid" .
			" AND ncrm_crmentity.deleted = 0 " .
			" WHERE (accountid = (SELECT accountid FROM ncrm_contactdetails WHERE contactid = ?)  AND accountid != 0) OR contactid = ?";
	...
	$query = "SELECT ncrm_troubletickets.*, ncrm_crmentity.smownerid,ncrm_crmentity.createdtime, ncrm_crmentity.modifiedtime, '' AS setype
		FROM ncrm_troubletickets
		INNER JOIN ncrm_crmentity ON ncrm_crmentity.crmid = ncrm_troubletickets.ticketid AND ncrm_crmentity.deleted = 0
		WHERE (ncrm_troubletickets.contact_id IN (". generateQuestionMarks($entity_ids_list) .")";
	if($acc_id) {
		$query .= " OR ncrm_troubletickets.parent_id = $acc_id) ";
	} else {
		$query .= ')';
	}
...
function create_ticket($input_array)
{
	...
	$accountResult = $adb->pquery('SELECT accountid FROM ncrm_contactdetails WHERE contactid = ?', array($parent_id));
	$accountId = $adb->query_result($accountResult, 0, 'accountid');
	if(!empty($accountId)) $ticket->column_fields['parent_id'] = $accountId;

	$ticket->save("HelpDesk");

	$ticketresult = $adb->pquery("select ncrm_troubletickets.ticketid from ncrm_troubletickets
		inner join ncrm_crmentity on ncrm_crmentity.crmid = ncrm_troubletickets.ticketid inner join ncrm_ticketcf on ncrm_ticketcf.ticketid = ncrm_troubletickets.ticketid
		where ncrm_crmentity.deleted=0 and ncrm_troubletickets.ticketid = ?", array($ticket->id));
	...
	if($servicecontractid != ''){
		$res = $adb->pquery("insert into ncrm_crmentityrel values(?,?,?,?)",
...
function authenticate_user($username,$password,$version,$login = 'true')
{
	...
	$sql = "select id, user_name, user_password,last_login_time, support_start_date, support_end_date, cryptmode
				from ncrm_portalinfo
					inner join ncrm_customerdetails on ncrm_portalinfo.id=ncrm_customerdetails.customerid
					inner join ncrm_crmentity on ncrm_crmentity.crmid=ncrm_portalinfo.id
				where ncrm_crmentity.deleted=0 and user_name=?
					and isactive=1 and ncrm_customerdetails.portal=1
					and ncrm_customerdetails.support_start_date <= ? and ncrm_customerdetails.support_end_date >= ?";
	...
	for ($i = 0; $i < $num_rows; ++$i) {
		...
		if (Ncrm_Functions::compareEncryptedPassword($password, $adb->query_result($result, $i, 'user_password'), $adb->query_result($result, $i, 'cryptmode'))) {
			break;
		} else {
			$customerid = null;
		}
	...
	if($login != 'false')
	{
		...
		$sql="insert into ncrm_soapservice values(?,?,?)";
...
function change_password($input_array)
{
	...
	$sql = "update ncrm_portalinfo set user_password=?, cryptmode=? where id=? and user_name=?";
	$result = $adb->pquery($sql, array(Ncrm_Functions::generateEncryptedPassword($password), 'CRYPT', $id, $username));
...
function update_login_details($input_array)
{
	...
	if($flag == 'login')
	{
		$sql = "update ncrm_portalinfo set login_time=? where id=?";
		$result = $adb->pquery($sql, array($current_time, $id));
	}
	elseif($flag == 'logout')
	{
		$sql = "update ncrm_portalinfo set logout_time=?, last_login_time=login_time where id=?";
		$result = $adb->pquery($sql, array($current_time, $id));
	}
...
function send_mail_for_password($mailid)
{
	...
	$sql = "select * from ncrm_portalinfo  where user_name = ? ";
	...
	$fromquery = "select ncrm_users.user_name, ncrm_users.email1 from ncrm_users inner join ncrm_crmentity on ncrm_users.id = ncrm_crmentity.smownerid inner join ncrm_contactdetails on ncrm_contactdetails.contactid=ncrm_crmentity.crmid where ncrm_contactdetails.email =?";
	...
	$mailserverresult = $adb->pquery("select * from ncrm_systems where server_type=?", array('email'));
...
function get_ticket_creator($input_array)
{
	...
	$res = $adb->pquery("select smcreatorid from ncrm_crmentity where crmid=?", array($ticketid));
...
function get_picklists($input_array)
{
	...
	$res = $adb->pquery("select ncrm_". $picklist_name.".* from ncrm_". $picklist_name." inner join ncrm_role2picklist on ncrm_role2picklist.picklistvalueid = ncrm_". $picklist_name.".picklist_valueid and ncrm_role2picklist.roleid='$admin_role'", array());
...
function get_ticket_attachments($input_array)
{
	...
	$query = "select ncrm_troubletickets.ticketid, ncrm_attachments.*,ncrm_notes.filename,ncrm_notes.filelocationtype from ncrm_troubletickets " .
		"left join ncrm_senotesrel on ncrm_senotesrel.crmid=ncrm_troubletickets.ticketid " .
		"left join ncrm_notes on ncrm_notes.notesid=ncrm_senotesrel.notesid " .
		...
		"left join ncrm_attachments on ncrm_attachments.attachmentsid = ncrm_seattachmentsrel.attachmentsid " .
		"and ncrm_crmentity.deleted = 0 where ncrm_troubletickets.ticketid =?";
...
function get_filecontent($input_array)
{
	...
	$query = 'SELECT ncrm_attachments.path FROM ncrm_attachments
	INNER JOIN ncrm_seattachmentsrel ON ncrm_seattachmentsrel.attachmentsid = ncrm_attachments.attachmentsid
	INNER JOIN ncrm_notes ON ncrm_notes.notesid = ncrm_seattachmentsrel.crmid
	INNER JOIN ncrm_senotesrel ON ncrm_senotesrel.notesid = ncrm_notes.notesid
	INNER JOIN ncrm_troubletickets ON ncrm_troubletickets.ticketid = ncrm_senotesrel.crmid
	WHERE ncrm_troubletickets.ticketid = ? AND ncrm_attachments.name = ? AND ncrm_attachments.attachmentsid = ?';
...
function add_ticket_attachment($input_array)
{
	...
	$attachmentid = $adb->getUniqueID("ncrm_crmentity");
	...
	$crmquery = "insert into ncrm_crmentity (crmid,setype,description,createdtime) values(?,?,?,?)";
	$crmresult = $adb->pquery($crmquery, array($attachmentid, 'HelpDesk Attachment', $description, $date_var));

	$attachmentquery = "insert into ncrm_attachments(attachmentsid,name,description,type,path) values(?,?,?,?,?)";
	$attachmentreulst = $adb->pquery($attachmentquery, array($attachmentid, $filename, $description, $filetype, $upload_filepath));

	$relatedquery = "insert into ncrm_seattachmentsrel values(?,?)";
	...
	$related_doc = 'insert into ncrm_seattachmentsrel values (?,?)';
	$res = $adb->pquery($related_doc,array($focus->id,$attachmentid));

	$tic_doc = 'insert into ncrm_senotesrel values(?,?)';
...
function getServerSessionId($id)
{
	...
	$sessionid = Ncrm_Soap_CustomerPortal::lookupSessionId($id);
	if($sessionid === false) {
		$query = "select * from ncrm_soapservice where type='customer' and id=?";
		$result = $adb->pquery($query, array($id));
		if($adb->num_rows($result) > 0) {
			$sessionid = $adb->query_result($result,0,'sessionid');
			Ncrm_Soap_CustomerPortal::updateSessionId($id, $sessionid);
		}
	}
	return $sessionid;
}
...
function unsetServerSessionId($id)
{
	...
	$id = (int) $id;
	Ncrm_Soap_CustomerPortal::updateSessionId($id, false);

	$adb->pquery("delete from ncrm_soapservice where type='customer' and id=?", array($id));
...
function get_account_name($accountid)
{
	...
	$res = $adb->pquery("select accountname from ncrm_account where accountid=?", array($accountid));
...
function get_contact_name($contactid)
{
	...
	if($contactid != '')
	{
		$sql = "select firstname,lastname from ncrm_contactdetails where contactid=?";
...
function get_check_account_id($id)
{
	...
	$res = $adb->pquery("select accountid from ncrm_contactdetails where contactid=?", array($id));
...
function get_vendor_name($vendorid)
{
	...
	$res = $adb->pquery("select vendorname from ncrm_vendor where vendorid=?", array($vendorid));
...
function get_list_values($id,$module,$sessionid,$only_mine='true')
{
	...
	if($module == 'Quotes')
	{
		$query = "select distinct ncrm_quotes.*,ncrm_crmentity.smownerid,
		case when ncrm_quotes.contactid is not null then ncrm_quotes.contactid else ncrm_quotes.accountid end as entityid,
		case when ncrm_quotes.contactid is not null then 'Contacts' else 'Accounts' end as setype,
		ncrm_potential.potentialname,ncrm_account.accountid
		from ncrm_quotes left join ncrm_crmentity on ncrm_crmentity.crmid=ncrm_quotes.quoteid
		...
		where ncrm_crmentity.deleted=0 and (ncrm_quotes.accountid in  (". generateQuestionMarks($entity_ids_list) .") or contactid in (". generateQuestionMarks($entity_ids_list) ."))";
	...
	{
		$query ="select distinct ncrm_invoice.*,ncrm_crmentity.smownerid,
		case when ncrm_invoice.contactid !=0 then ncrm_invoice.contactid else ncrm_invoice.accountid end as entityid,
		case when ncrm_invoice.contactid !=0 then 'Contacts' else 'Accounts' end as setype
		from ncrm_invoice
		left join ncrm_crmentity on ncrm_crmentity.crmid=ncrm_invoice.invoiceid
		where ncrm_crmentity.deleted=0 and (accountid in (". generateQuestionMarks($entity_ids_list) .") or contactid in  (". generateQuestionMarks($entity_ids_list) ."))";
		$params = array($entity_ids_list,$entity_ids_list);
		$fields_list['Related To'] = 'entityid';
	}
	else if ($module == 'Documents')
	{
		$query ="select ncrm_notes.*, ncrm_crmentity.*, ncrm_senotesrel.crmid as entityid, '' as setype,ncrm_attachmentsfolder.foldername from ncrm_notes " .
		"inner join ncrm_crmentity on ncrm_crmentity.crmid = ncrm_notes.notesid " .
		"left join ncrm_senotesrel on ncrm_senotesrel.notesid=ncrm_notes.notesid " .
		"LEFT JOIN ncrm_attachmentsfolder ON ncrm_attachmentsfolder.folderid = ncrm_notes.folderid " .
		"where ncrm_crmentity.deleted = 0 and  ncrm_senotesrel.crmid in (".generateQuestionMarks($entity_ids_list).")";
		$params = array($entity_ids_list);
		$fields_list['Related To'] = 'entityid';
	}else if ($module == 'Contacts'){
		$query = "select ncrm_contactdetails.*,ncrm_crmentity.smownerid from ncrm_contactdetails
		 inner join ncrm_crmentity on ncrm_crmentity.crmid=ncrm_contactdetails.contactid
		 where ncrm_crmentity.deleted = 0 and contactid IN (".generateQuestionMarks($entity_ids_list).")";
		$params = array($entity_ids_list);
	}else if ($module == 'Assets') {
		$accountRes = $adb->pquery("SELECT accountid FROM ncrm_contactdetails
						INNER JOIN ncrm_crmentity ON ncrm_contactdetails.contactid = ncrm_crmentity.crmid
		...
		$query = "select ncrm_assets.*, ncrm_assets.account as entityid , ncrm_crmentity.smownerid from ncrm_assets
						inner join ncrm_crmentity on ncrm_assets.assetsid = ncrm_crmentity.crmid
						left join ncrm_account on ncrm_account.accountid = ncrm_assets.account
						left join ncrm_products on ncrm_products.productid = ncrm_assets.product
						where ncrm_crmentity.deleted = 0 and account = ?";
		$params = array($accountid);
		$fields_list['Related To'] = 'entityid';
		}
	}else if ($module == 'Project') {
		$query = "SELECT ncrm_project.*, ncrm_crmentity.smownerid
					FROM ncrm_project
					INNER JOIN ncrm_crmentity ON ncrm_crmentity.crmid = ncrm_project.projectid
					WHERE ncrm_crmentity.deleted = 0 AND ncrm_project.linktoaccountscontacts IN (".generateQuestionMarks($entity_ids_list).")";
		$params = array($entity_ids_list);
	...
	for( $j= 0;$j < $noofdata; $j++)
	{
		...
		foreach($fields_list as $fieldlabel =>$fieldname ) {
			...
			if($fieldname == 'product' && $module == 'Assets'){
				...
				$fres = $adb->pquery('select ncrm_products.productname from ncrm_products where productid=?',array($crmid));
...
function get_filecontent_detail($id,$folderid,$module,$customerid,$sessionid)
{
	...
	if($module == 'Documents')
	{
		$query="SELECT filetype FROM ncrm_notes WHERE notesid =?";
		$res = $adb->pquery($query, array($id));
		$filetype = $adb->query_result($res, 0, "filetype");
		updateDownloadCount($id);

		$fileidQuery = 'select attachmentsid from ncrm_seattachmentsrel where crmid = ?';
		$fileres = $adb->pquery($fileidQuery,array($id));
		$fileid = $adb->query_result($fileres,0,'attachmentsid');

		$filepathQuery = 'select path,name from ncrm_attachments where attachmentsid = ?';
	...
	{
		$query ='select ncrm_attachments.*,ncrm_seattachmentsrel.* from ncrm_attachments inner join ncrm_seattachmentsrel on ncrm_seattachmentsrel.attachmentsid=ncrm_attachments.attachmentsid where ncrm_seattachmentsrel.crmid =?';
...
function updateDownloadCount($id){
	...
	$updateDownloadCount = "UPDATE ncrm_notes SET filedownloadcount = filedownloadcount+1 WHERE notesid = ?";
...
function get_salesorder_name($id)
{
	...
	$res = $adb->pquery(" select subject from ncrm_salesorder where salesorderid=?", array($id));
...
function get_invoice_detail($id,$module,$customerid,$sessionid)
{
	...
	$fieldquery = "SELECT fieldname, columnname, fieldlabel,block,uitype FROM ncrm_field WHERE tabid = ? AND displaytype in (1,2,4) ORDER BY block,sequence";
	$fieldres = $adb->pquery($fieldquery,array(getTabid($module)));
	$nooffields = $adb->num_rows($fieldres);
	$query = "select ncrm_invoice.*,ncrm_crmentity.* ,ncrm_invoicebillads.*,ncrm_invoiceshipads.*,
		ncrm_invoicecf.* from ncrm_invoice
		inner join ncrm_crmentity on ncrm_crmentity.crmid = ncrm_invoice.invoiceid
		LEFT JOIN ncrm_invoicebillads ON ncrm_invoice.invoiceid = ncrm_invoicebillads.invoicebilladdressid
		LEFT JOIN ncrm_invoiceshipads ON ncrm_invoice.invoiceid = ncrm_invoiceshipads.invoiceshipaddressid
		INNER JOIN ncrm_invoicecf ON ncrm_invoice.invoiceid = ncrm_invoicecf.invoiceid
		where ncrm_invoice.invoiceid=?";
	$res = $adb->pquery($query, array($id));
	...
	for($i=0;$i<$nooffields;$i++)
	{
		...
		$blocknameQuery = "select blocklabel from ncrm_blocks where blockid = ?";
...
function get_product_list_values($id,$modulename,$sessionid,$only_mine='true')
{
	...
	$query = array();
	$params = array();
	$query[] = "SELECT ncrm_products.*,ncrm_seproductsrel.crmid as entityid, ncrm_seproductsrel.setype FROM ncrm_products
		INNER JOIN ncrm_crmentity on ncrm_products.productid = ncrm_crmentity.crmid
		LEFT JOIN ncrm_seproductsrel on ncrm_seproductsrel.productid = ncrm_products.productid
		WHERE ncrm_seproductsrel.crmid in (". generateQuestionMarks($entity_ids_list).") and ncrm_crmentity.deleted = 0 ";
	$params[] = array($entity_ids_list);

	$checkQuotes = checkModuleActive('Quotes');
	if($checkQuotes == true){
		$query[] = "select distinct ncrm_products.*,
			case when ncrm_quotes.contactid is not null then ncrm_quotes.contactid else ncrm_quotes.accountid end as entityid,
			case when ncrm_quotes.contactid is not null then 'Contacts' else 'Accounts' end as setype
			from ncrm_quotes INNER join ncrm_crmentity on ncrm_crmentity.crmid=ncrm_quotes.quoteid
			...
			where ncrm_inventoryproductrel.productid = ncrm_products.productid AND ncrm_crmentity.deleted=0 and (accountid in  (". generateQuestionMarks($entity_ids_list) .") or contactid in (". generateQuestionMarks($entity_ids_list) ."))";
		$params[] = array($entity_ids_list,$entity_ids_list);
	}
...
function get_details($id,$module,$customerid,$sessionid)
{
	...
	if($module == 'Quotes'){
		$query =  "SELECT
			ncrm_quotes.*,ncrm_crmentity.*,ncrm_quotesbillads.*,ncrm_quotesshipads.*,
			ncrm_quotescf.* FROM ncrm_quotes
			...
			WHERE ncrm_quotes.quoteid=(". generateQuestionMarks($id) .") AND ncrm_crmentity.deleted = 0";

	}
	else if($module == 'Documents'){
		$query =  "SELECT
			ncrm_notes.*,ncrm_crmentity.*,ncrm_attachmentsfolder.foldername,ncrm_notescf.*
			FROM ncrm_notes
			INNER JOIN ncrm_crmentity ON ncrm_crmentity.crmid = ncrm_notes.notesid
			...
			WHERE ncrm_notes.notesid=(". generateQuestionMarks($id) .") AND ncrm_crmentity.deleted=0";
	}
	else if($module == 'HelpDesk'){
		$query ="SELECT
			ncrm_troubletickets.*,ncrm_crmentity.smownerid,ncrm_crmentity.createdtime,ncrm_crmentity.modifiedtime,
			ncrm_ticketcf.*,ncrm_crmentity.description  FROM ncrm_troubletickets
			...
			WHERE (ncrm_troubletickets.ticketid=(". generateQuestionMarks($id) .") AND ncrm_crmentity.deleted = 0)";
	}
	else if($module == 'Services'){
		$query ="SELECT ncrm_service.*,ncrm_crmentity.*,ncrm_servicecf.*  FROM ncrm_service
			INNER JOIN ncrm_crmentity
				ON ncrm_crmentity.crmid = ncrm_service.serviceid AND ncrm_crmentity.deleted = 0
			LEFT JOIN ncrm_servicecf
				ON ncrm_service.serviceid = ncrm_servicecf.serviceid
			WHERE ncrm_service.serviceid= (". generateQuestionMarks($id) .")";
	}
	else if($module == 'Contacts'){
		$query = "SELECT ncrm_contactdetails.*,ncrm_contactaddress.*,ncrm_contactsubdetails.*,ncrm_contactscf.*" .
			" ,ncrm_crmentity.*,ncrm_customerdetails.*
			...
			WHERE ncrm_contactdetails.contactid = (". generateQuestionMarks($id) .") AND ncrm_crmentity.deleted = 0";
	}
	else if($module == 'Accounts'){
		$query = "SELECT ncrm_account.*,ncrm_accountbillads.*,ncrm_accountshipads.*,ncrm_accountscf.*,
			ncrm_crmentity.* FROM ncrm_account
			INNER JOIN ncrm_crmentity
		...
		" WHERE ncrm_account.accountid = (". generateQuestionMarks($id) .") AND ncrm_crmentity.deleted = 0";
	}
	else if ($module == 'Products'){
		$query = "SELECT ncrm_products.*,ncrm_productcf.*,ncrm_crmentity.* " .
		"FROM ncrm_products " .
		"INNER JOIN ncrm_crmentity " .
			"ON ncrm_crmentity.crmid = ncrm_products.productid " .
		"LEFT JOIN ncrm_productcf " .
			"ON ncrm_productcf.productid = ncrm_products.productid " .
		"LEFT JOIN ncrm_vendor
			ON ncrm_vendor.vendorid = ncrm_products.vendor_id " .
		"WHERE ncrm_products.productid = (". generateQuestionMarks($id) .") AND ncrm_crmentity.deleted = 0";
	} else if($module == 'Assets') {
		$query = "SELECT ncrm_assets.*, ncrm_assetscf.*, ncrm_crmentity.*
		FROM ncrm_assets
		INNER JOIN ncrm_crmentity
		ON ncrm_assets.assetsid = ncrm_crmentity.crmid
		INNER JOIN ncrm_assetscf
		ON ncrm_assetscf.assetsid = ncrm_assets.assetsid
		WHERE ncrm_crmentity.deleted = 0 AND ncrm_assets.assetsid = (". generateQuestionMarks($id) .")";
	} else if ($module == 'Project') {
		$query = "SELECT ncrm_project.*, ncrm_projectcf.*, ncrm_crmentity.*
					FROM ncrm_project
					INNER JOIN ncrm_crmentity ON ncrm_crmentity.crmid = ncrm_project.projectid
					LEFT JOIN ncrm_projectcf ON ncrm_projectcf.projectid = ncrm_project.projectid
					WHERE ncrm_project.projectid = ? AND ncrm_crmentity.deleted = 0";
	}

	$params = array($id);
	$res = $adb->pquery($query,$params);

	$fieldquery = "SELECT fieldname,columnname,fieldlabel,blocklabel,uitype FROM ncrm_field
		INNER JOIN  ncrm_blocks on ncrm_blocks.blockid=ncrm_field.block WHERE ncrm_field.tabid = ? AND displaytype in (1,2,4)
		ORDER BY ncrm_field.block,ncrm_field.sequence";
	...
	for($i=0;$i<$nooffields;$i++)
	{
		...
		if($module == 'Assets' ){
			if($fieldname == 'account'){
				...
				$accountres = $adb->pquery("select ncrm_account.accountname from ncrm_account where accountid=?",array($accountid));
				$accountname = $adb->query_result($accountres,0,'accountname');
				$fieldvalue = $accountname;
			}
			if($fieldname == 'product'){
				$productid = $adb->query_result($res,0,'product');
				$productres = $adb->pquery("select ncrm_products.productname from ncrm_products where productid=?",array($productid));
			...
			if($fieldname == 'invoiceid'){
				...
				$invoiceres = $adb->pquery("select ncrm_invoice.subject from ncrm_invoice where invoiceid=?",array($invoiceid));
...
function check_permission($customerid, $module, $entityid) {
	...
	$faqquery = "select id from ncrm_faq";
	...
	switch($module) {
		case 'Products'	: 	$query = "SELECT ncrm_seproductsrel.productid FROM ncrm_seproductsrel
								INNER JOIN ncrm_crmentity
								ON ncrm_seproductsrel.productid=ncrm_crmentity.crmid
								WHERE ncrm_seproductsrel.crmid IN (". generateQuestionMarks($allowed_contacts_and_accounts).")
									AND ncrm_crmentity.deleted=0
									AND ncrm_seproductsrel.productid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
							$query = "SELECT ncrm_inventoryproductrel.productid, ncrm_inventoryproductrel.id
													FROM ncrm_inventoryproductrel
													INNER JOIN ncrm_crmentity
													ON ncrm_inventoryproductrel.productid=ncrm_crmentity.crmid
													LEFT JOIN ncrm_quotes
													ON ncrm_inventoryproductrel.id = ncrm_quotes.quoteid
													WHERE ncrm_crmentity.deleted=0
														AND (ncrm_quotes.contactid IN (". generateQuestionMarks($allowed_contacts_and_accounts).") or ncrm_quotes.accountid IN (".generateQuestionMarks($allowed_contacts_and_accounts)."))
														AND ncrm_inventoryproductrel.productid = ?";
							...
							$query = "SELECT ncrm_inventoryproductrel.productid, ncrm_inventoryproductrel.id
													...
													INNER JOIN ncrm_crmentity
													ON ncrm_inventoryproductrel.productid=ncrm_crmentity.crmid
													LEFT JOIN ncrm_invoice
													ON ncrm_inventoryproductrel.id = ncrm_invoice.invoiceid
													WHERE ncrm_crmentity.deleted=0
														AND (ncrm_invoice.contactid IN (". generateQuestionMarks($allowed_contacts_and_accounts).") or ncrm_invoice.accountid IN (".generateQuestionMarks($allowed_contacts_and_accounts)."))
														AND ncrm_inventoryproductrel.productid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
		...
		case 'Quotes'	:	$query = "SELECT ncrm_quotes.quoteid
								FROM ncrm_quotes
								INNER JOIN ncrm_crmentity
								ON ncrm_quotes.quoteid=ncrm_crmentity.crmid
								WHERE ncrm_crmentity.deleted=0
									AND (ncrm_quotes.contactid IN (". generateQuestionMarks($allowed_contacts_and_accounts).") or ncrm_quotes.accountid IN (".generateQuestionMarks($allowed_contacts_and_accounts)."))
									AND ncrm_quotes.quoteid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
		...
		case 'Invoice'	:	$query = "SELECT ncrm_invoice.invoiceid
								FROM ncrm_invoice
								INNER JOIN ncrm_crmentity
								ON ncrm_invoice.invoiceid=ncrm_crmentity.crmid
								WHERE ncrm_crmentity.deleted=0
									AND (ncrm_invoice.contactid IN (". generateQuestionMarks($allowed_contacts_and_accounts).") or ncrm_invoice.accountid IN (".generateQuestionMarks($allowed_contacts_and_accounts)."))
									AND ncrm_invoice.invoiceid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
		...
		case 'Documents'	: 	$query = "SELECT ncrm_senotesrel.notesid FROM ncrm_senotesrel
									INNER JOIN ncrm_crmentity ON ncrm_crmentity.crmid = ncrm_senotesrel.notesid AND ncrm_crmentity.deleted = 0
									WHERE ncrm_senotesrel.crmid IN (". generateQuestionMarks($allowed_contacts_and_accounts) .")
									AND ncrm_senotesrel.notesid = ?";
								$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
								if ($adb->num_rows($res) > 0) {
									return true;
								}
								if(checkModuleActive('Project')) {
									$query = "SELECT ncrm_senotesrel.notesid FROM ncrm_senotesrel
										INNER JOIN ncrm_project ON ncrm_project.projectid = ncrm_senotesrel.crmid
										WHERE ncrm_project.linktoaccountscontacts IN (". generateQuestionMarks($allowed_contacts_and_accounts) .")
										AND ncrm_senotesrel.notesid = ?";
									$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
									if ($adb->num_rows($res) > 0) {
										return true;
								...
								$query = "SELECT ncrm_senotesrel.notesid FROM ncrm_senotesrel
															INNER JOIN ncrm_crmentity ON ncrm_crmentity.crmid = ncrm_senotesrel.notesid AND ncrm_crmentity.deleted = 0
															WHERE ncrm_senotesrel.crmid IN (". generateQuestionMarks($faq_id) .")
															AND ncrm_senotesrel.notesid = ?";
		...
		case 'HelpDesk'	:	if($acc_id) $accCondition = "OR ncrm_troubletickets.parent_id = $acc_id";
							$query = "SELECT ncrm_troubletickets.ticketid FROM ncrm_troubletickets
									INNER JOIN ncrm_crmentity ON ncrm_crmentity.crmid = ncrm_troubletickets.ticketid AND ncrm_crmentity.deleted = 0
									WHERE (ncrm_troubletickets.contact_id IN (". generateQuestionMarks($allowed_contacts_and_accounts) .") $accCondition )
									AND ncrm_troubletickets.ticketid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
							...
							$query = "SELECT ncrm_troubletickets.ticketid FROM ncrm_troubletickets
									...
									WHERE ncrm_crmentity.deleted = 0 AND
											(ncrm_crmentityrel.crmid IN
												(SELECT projectid FROM ncrm_project WHERE linktoaccountscontacts
													IN (". generateQuestionMarks($allowed_contacts_and_accounts) .") AND ncrm_crmentityrel.relcrmid = $entityid)
											OR ncrm_crmentityrel.relcrmid IN
												(SELECT projectid FROM ncrm_project WHERE linktoaccountscontacts
													IN (". generateQuestionMarks($allowed_contacts_and_accounts) .") AND ncrm_crmentityrel.crmid = $entityid)
										AND ncrm_troubletickets.ticketid = ?)";
		...
		case 'Services'	:	$query = "SELECT ncrm_service.serviceid FROM ncrm_service
									INNER JOIN ncrm_crmentity ON ncrm_crmentity.crmid = ncrm_service.serviceid AND ncrm_crmentity.deleted = 0
									LEFT JOIN ncrm_crmentityrel ON (ncrm_crmentityrel.relcrmid=ncrm_service.serviceid OR ncrm_crmentityrel.crmid=ncrm_service.serviceid)
									WHERE (ncrm_crmentityrel.crmid IN (". generateQuestionMarks($allowed_contacts_and_accounts) .")  OR " .
		 							"(ncrm_crmentityrel.relcrmid IN (".generateQuestionMarks($allowed_contacts_and_accounts).") AND ncrm_crmentityrel.module = 'Services'))
									AND ncrm_service.serviceid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts,$allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
		...
		case 'Accounts' : 	$query = "SELECT ncrm_account.accountid FROM ncrm_account " .
									"INNER JOIN ncrm_crmentity ON ncrm_crmentity.crmid = ncrm_account.accountid " .
									"INNER JOIN ncrm_contactdetails ON ncrm_contactdetails.accountid = ncrm_account.accountid " .
									"WHERE ncrm_crmentity.deleted = 0 and ncrm_contactdetails.contactid = ? and ncrm_contactdetails.accountid = ?";
		...
		case 'Assets' : $query = "SELECT ncrm_assets.assetname FROM ncrm_assets
								INNER JOIN ncrm_crmentity ON  ncrm_assets.assetsid = ncrm_crmentity.crmid
								WHERE ncrm_crmentity.deleted = 0 and ncrm_assets.account = ? ";
						$accountid = '';
						$accountRes = $adb->pquery("SELECT accountid FROM ncrm_contactdetails
								INNER JOIN ncrm_crmentity ON ncrm_contactdetails.contactid = ncrm_crmentity.crmid
		...
		case 'Project'	:	$query = "SELECT ncrm_project.projectid FROM ncrm_project
									INNER JOIN ncrm_crmentity ON ncrm_crmentity.crmid = ncrm_project.projectid AND ncrm_crmentity.deleted = 0
									WHERE ncrm_project.linktoaccountscontacts IN (". generateQuestionMarks($allowed_contacts_and_accounts) .")
									AND ncrm_project.projectid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
							if ($adb->num_rows($res) > 0) {
								return true;
							}
...
function get_documents($id,$module,$customerid,$sessionid)
{
	...
	$query ="select ncrm_notes.title,'Documents' ActivityType, ncrm_notes.filename,
		crm2.createdtime,ncrm_notes.notesid,ncrm_notes.folderid,
		ncrm_notes.notecontent description, ncrm_users.user_name, ncrm_notes.filelocationtype
		from ncrm_notes
		LEFT join ncrm_senotesrel on ncrm_senotesrel.notesid= ncrm_notes.notesid
		INNER join ncrm_crmentity on ncrm_crmentity.crmid= ncrm_senotesrel.crmid
		LEFT join ncrm_crmentity crm2 on crm2.crmid=ncrm_notes.notesid and crm2.deleted=0
		LEFT JOIN ncrm_groups
		ON ncrm_groups.groupid = ncrm_crmentity.smownerid
		LEFT join ncrm_users on crm2.smownerid= ncrm_users.id
		where ncrm_crmentity.crmid=?";
	$res = $adb->pquery($query,array($id));
	$noofdata = $adb->num_rows($res);
	for( $j= 0;$j < $noofdata; $j++)
	{
...
function get_project_components($id,$module,$customerid,$sessionid) {
	...
	if ($module == 'ProjectTask') {
		$query ="SELECT ncrm_projecttask.*, ncrm_crmentity.smownerid
				FROM ncrm_projecttask
				INNER JOIN ncrm_project ON ncrm_project.projectid = ncrm_projecttask.projectid AND ncrm_project.projectid = ?
				INNER JOIN ncrm_crmentity ON ncrm_crmentity.crmid = ncrm_projecttask.projecttaskid AND ncrm_crmentity.deleted = 0";
	} elseif ($module == 'ProjectMilestone') {
		$query ="SELECT ncrm_projectmilestone.*, ncrm_crmentity.smownerid
				FROM ncrm_projectmilestone
				INNER JOIN ncrm_project ON ncrm_project.projectid = ncrm_projectmilestone.projectid AND ncrm_project.projectid = ?
				INNER JOIN ncrm_crmentity ON ncrm_crmentity.crmid = ncrm_projectmilestone.projectmilestoneid AND ncrm_crmentity.deleted = 0";
	}
...
function get_project_tickets($id,$module,$customerid,$sessionid) {
	...
	$query = "SELECT ncrm_troubletickets.*, ncrm_crmentity.smownerid FROM ncrm_troubletickets
		INNER JOIN ncrm_crmentity ON ncrm_crmentity.crmid = ncrm_troubletickets.ticketid
		INNER JOIN ncrm_crmentityrel ON (ncrm_crmentityrel.relcrmid = ncrm_crmentity.crmid OR ncrm_crmentityrel.crmid = ncrm_crmentity.crmid)
		WHERE ncrm_crmentity.deleted = 0 AND (ncrm_crmentityrel.crmid = ? OR ncrm_crmentityrel.relcrmid = ?)";
...
function get_service_list_values($id,$modulename,$sessionid,$only_mine='true')
{
	...
	$query[] = "select ncrm_service.*," .
		"case when ncrm_crmentityrel.crmid != ncrm_service.serviceid then ncrm_crmentityrel.crmid else ncrm_crmentityrel.relcrmid end as entityid, " .
		 "'' as setype from ncrm_service " .
		 "inner join ncrm_crmentity on ncrm_crmentity.crmid=ncrm_service.serviceid " .
		 "left join ncrm_crmentityrel on (ncrm_crmentityrel.relcrmid=ncrm_service.serviceid or ncrm_crmentityrel.crmid=ncrm_service.serviceid) " .
		 "where ncrm_crmentity.deleted = 0 and " .
		 "( ncrm_crmentityrel.crmid in (".generateQuestionMarks($entity_ids_list).") OR " .
		 "(ncrm_crmentityrel.relcrmid in (".generateQuestionMarks($entity_ids_list).") AND ncrm_crmentityrel.module = 'Services')" .
		 ")";
	...
	$checkQuotes = checkModuleActive('Quotes');
	if($checkQuotes == true){
		$query[] = "select distinct ncrm_service.*,
			case when ncrm_quotes.contactid is not null then ncrm_quotes.contactid else ncrm_quotes.accountid end as entityid,
			case when ncrm_quotes.contactid is not null then 'Contacts' else 'Accounts' end as setype
			from ncrm_quotes INNER join ncrm_crmentity on ncrm_crmentity.crmid=ncrm_quotes.quoteid
			left join ncrm_inventoryproductrel on ncrm_inventoryproductrel.id=ncrm_quotes.quoteid
			left join ncrm_service on ncrm_service.serviceid = ncrm_inventoryproductrel.productid
			where ncrm_inventoryproductrel.productid = ncrm_service.serviceid AND ncrm_crmentity.deleted=0 and (accountid in  (". generateQuestionMarks($entity_ids_list) .") or contactid in (". generateQuestionMarks($entity_ids_list) ."))";
		$params[] = array($entity_ids_list,$entity_ids_list);
	}
	$checkInvoices = checkModuleActive('Invoice');
	if($checkInvoices == true){
		...
		for( $j= 0;$j < $noofdata[$k]; $j++)
		{
			...
			foreach($fields_list as $fieldlabel=> $fieldname) {
				...
				if($fieldname == 'entityid') {
					...
					$module = $adb->query_result($res[$k],$j,'setype');
					if($module == ''){
						$module = $adb->query_result($adb->pquery("SELECT setype FROM ncrm_crmentity WHERE crmid = ?", array($crmid)),0,'setype');
					}
					if ($crmid != '' && $module != '') {
						$fieldvalues = getEntityName($module, array($crmid));
						if($module == 'Contacts')
...
function get_modules()
{
	...
	$modules = Ncrm_Soap_CustomerPortal::lookupAllowedModules();
	if($modules === false) {
		$modules = array();

		$query = $adb->pquery("SELECT ncrm_customerportal_tabs.* FROM ncrm_customerportal_tabs
			INNER JOIN ncrm_tab ON ncrm_tab.tabid = ncrm_customerportal_tabs.tabid
			WHERE ncrm_tab.presence = 0 AND ncrm_customerportal_tabs.visible = 1", array());
		...
		Ncrm_Soap_CustomerPortal::updateAllowedModules($modules);
	}
...
function show_all($module){
	...
	$query = $adb->pquery("SELECT prefvalue from ncrm_customerportal_prefs where tabid = ?", array($tabid));
	$norows = $adb->num_rows($query);
	if($norows > 0){
...
function getRelatedServiceContracts($crmid){
	...
	$query = "SELECT * FROM ncrm_servicecontracts " .
	"INNER JOIN ncrm_crmentity ON ncrm_crmentity.crmid = ncrm_servicecontracts.servicecontractsid AND ncrm_crmentity.deleted = 0 " .
	"LEFT JOIN ncrm_crmentityrel ON ncrm_crmentityrel.crmid = ncrm_servicecontracts.servicecontractsid " .
	"WHERE (ncrm_crmentityrel.relcrmid = ? and ncrm_crmentityrel.module= 'ServiceContracts')";
...
function getPortalUserid() {
	...
	$userid = Ncrm_Soap_CustomerPortal::lookupPrefValue('userid');
	if($userid === false) {
		$res = $adb->pquery("SELECT prefvalue FROM ncrm_customerportal_prefs WHERE prefkey = 'userid' AND tabid = 0", array());
		$norows = $adb->num_rows($res);
		if($norows > 0) {
			$userid = $adb->query_result($res,0,'prefvalue');
			// Update the cache information now.
			Ncrm_Soap_CustomerPortal::updatePrefValue('userid', $userid);
		}
	}
	return $userid;
...
function getDefaultAssigneeId() {
	...
	// Look the value from cache first
	$defaultassignee = Ncrm_Soap_CustomerPortal::lookupPrefValue('defaultassignee');
	if($defaultassignee === false) {
		$res = $adb->pquery("SELECT prefvalue FROM ncrm_customerportal_prefs WHERE prefkey = 'defaultassignee' AND tabid = 0", array());
		$norows = $adb->num_rows($res);
		if($norows > 0) {
			$defaultassignee = $adb->query_result($res,0,'prefvalue');
			// Update the cache information now.
			Ncrm_Soap_CustomerPortal::updatePrefValue('defaultassignee', $defaultassignee);
		}
	}
