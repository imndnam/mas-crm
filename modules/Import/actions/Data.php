...
class Import_Data_Action extends Ncrm_Action_Controller {
	...
	public function importData() {
		...
		$moduleModel = Ncrm_Module_Model::getInstance($this->module);
        // pre fetch the fields and premmisions of module
        Ncrm_Field_Model::getAllForModule($moduleModel);
        if($this->user->is_admin == 'off'){
            Ncrm_Field_Model::preFetchModuleFieldPermission($moduleModel->getId());
        }
	...
	public function createRecords() {
		...
		for ($i = 0; $i < $numberOfRecords; ++$i) {
            ...
            if($createRecord || $mergeType == Import_Utils_Helper::$AUTO_MERGE_MERGEFIELDS || $mergeType == Import_Utils_Helper::$AUTO_MERGE_OVERWRITE){
                ...
                $label = trim($label);
                $adb->pquery('UPDATE ncrm_crmentity SET label=? WHERE crmid=?', array($label, $recordId));
            }
        ...
        if($this->entityData) {
            ...
            $entity->triggerEvent('ncrm.batchevent.save',$this->entityData);
        }
	...
	public function transformForImport($fieldData, $moduleMeta, $fillDefault=true, $checkMandatoryFieldValues=true) {
		...
		foreach ($fieldData as $fieldName => $fieldValue) {
			...
			if ($fieldInstance->getFieldDataType() == 'owner') {
				...
				$olderCacheEnable = Ncrm_Cache::$cacheEnable;
				Ncrm_Cache::$cacheEnable = false;
				...
				if (!in_array($picklistValueInLowerCase, $allPicklistValuesInLowerCase)) {
					$moduleObject = Ncrm_Module::getInstance($moduleMeta->getEntityName());
					$fieldObject = Ncrm_Field::getInstance($fieldName, $moduleObject);
				...
				Ncrm_Cache::$cacheEnable = $olderCacheEnable;
	...
	public static function runScheduledImport() {
		...
		foreach ($scheduledImports as $scheduledId => $importDataController) {
			...
			$emailSubject = 'NCRM - Scheduled Import Report for '.$importDataController->module;
			...
			$emailData = 'NCRM has just completed your import process. <br/><br/>' .
							$importResult . '<br/><br/>'.
			...
			$ncrmMailer->to = array( array($userEmail, $userName));
			$ncrmMailer->Subject = $emailSubject;
			$ncrmMailer->Body    = $emailData;
			$ncrmMailer->Send();

			$importDataController->finishImport();
		}
		Ncrm_Mailer::dispatchQueue(null);
	}
