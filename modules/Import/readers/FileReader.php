...
class Import_FileReader_Reader {
	...
	public function  __construct($request, $user) {
		...
		$this->user = $user;
        $this->moduleModel = Ncrm_Module_Model::getInstance($this->request->get('module'));
	}
	 ...
	 * @param $fieldObject <Ncrm_Field_Model>
    ...
    public function getModuleFieldDBColumnType() {
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT tablename FROM ncrm_field WHERE tabid=? GROUP BY tablename', array($this->moduleModel->getId()));
        $tables = array();
