...
class Documents_Record_Model extends Ncrm_Record_Model {
	...
	function getFileDetails() {
		...
		$result = $db->pquery("SELECT * FROM ncrm_attachments
	...
	function updateFileStatus($status) {
		$db = PearDatabase::getInstance();
                ...
                $db->pquery("UPDATE ncrm_notes SET filestatus = ? WHERE notesid= ?", array($status,$this->get('id')));
	}
	...
	function updateDownloadCount() {
		...
		$result = $db->pquery("SELECT filedownloadcount FROM ncrm_notes WHERE notesid = ?", array($notesId));
		$downloadCount = $db->query_result($result, 0, 'filedownloadcount') + 1;

		$db->pquery("UPDATE ncrm_notes SET filedownloadcount = ? WHERE notesid = ?", array($downloadCount, $notesId));
	}
