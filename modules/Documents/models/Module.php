...
class Documents_Module_Model extends Ncrm_Module_Model {
	...
	public static function getAllFolders() {
		...
		$result = $db->pquery('SELECT * FROM ncrm_attachmentsfolder ORDER BY sequence', array());
	...
	public function getQueryByModuleField($sourceModule, $field, $record, $listQuery) {
		if($sourceModule === 'Emails' && $field === 'composeEmail') {
			$condition = ' (( ncrm_notes.filelocationtype LIKE "%I%")) AND ncrm_notes.filename != "" AND ncrm_notes.filestatus = 1';
		} else {
			$condition = " ncrm_notes.notesid NOT IN (SELECT notesid FROM ncrm_senotesrel WHERE crmid = '$record') AND ncrm_notes.filestatus = 1";
		}
