...
class Products_Relation_Model extends Ncrm_Relation_Model {
	...
	public function getQuery($recordModel, $actions=false){
		...
		if($functionName == 'get_product_pricebooks'){
			$selectColumnSql = $selectColumnSql.' ,ncrm_pricebookproductrel.listprice, ncrm_pricebook.currency_id, ncrm_products.unit_price';
		}
		if($functionName == 'get_service_pricebooks'){
			$selectColumnSql = $selectColumnSql.' ,ncrm_pricebookproductrel.listprice, ncrm_pricebook.currency_id, ncrm_service.unit_price';
		}
	...
	public function deleteRelation($sourceRecordId, $relatedRecordId) {
		...
		if(($sourceModuleName == 'Products' || $sourceModuleName == 'Services') && $relatedModuleName == 'PriceBooks') {
			...
			$priceBookModel = Ncrm_Record_Model::getInstanceById($relatedRecordId, $relatedModuleName);
	...
	public function deleteProductToProductRelation($sourceRecordId, $relatedRecordId) {
		...
		if(!empty($sourceRecordId) && !empty($relatedRecordId)){
			$db->pquery('DELETE FROM ncrm_seproductsrel WHERE crmid = ? AND productid = ?', array($relatedRecordId, $sourceRecordId));
			return true;
		}
	}
	...
	public function isSubProduct($subProductId){
		if(!empty($subProductId)){
			$db = PearDatabase::getInstance();
			$result = $db->pquery('SELECT crmid FROM ncrm_seproductsrel WHERE crmid = ?', array($subProductId));
	...
	public function addListPrice($sourceRecordId, $destinationRecordId, $listPrice) {
		$sourceModuleName = $this->getParentModuleModel()->get('name');
		$relatedModuleName = $this->getRelationModuleModel()->get('name');
		$relationModuleModel = Ncrm_Record_Model::getInstanceById($destinationRecordId, $relatedModuleName);
		
		$productModel = Ncrm_Record_Model::getInstanceById($sourceRecordId, $sourceModuleName);
