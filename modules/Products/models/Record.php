...
class Products_Record_Model extends Ncrm_Record_Model {
	...
	function getTaxes() {
		...
		$result = $db->pquery('SELECT * FROM ncrm_producttaxrel
					INNER JOIN ncrm_inventorytaxinfo ON ncrm_inventorytaxinfo.taxid = ncrm_producttaxrel.taxid
					INNER JOIN ncrm_crmentity ON ncrm_producttaxrel.productid = ncrm_crmentity.crmid AND ncrm_crmentity.deleted = 0
					WHERE ncrm_producttaxrel.productid = ? AND ncrm_inventorytaxinfo.deleted = 0', array($this->getId()));
		$taxes = array();
	...
	static function getListPriceValues($id) {
    	$db = PearDatabase::getInstance();            
        $listPrice = $db->pquery('SELECT * FROM ncrm_productcurrencyrel WHERE productid	= ?', array($id)); 
	...
	function getSubProducts() {
		...
		$result = $db->pquery("SELECT ncrm_products.productid FROM ncrm_products
			INNER JOIN ncrm_crmentity ON ncrm_crmentity.crmid = ncrm_products.productid
			LEFT JOIN ncrm_seproductsrel ON ncrm_seproductsrel.crmid = ncrm_products.productid AND ncrm_products.discontinued = 1 
                        AND ncrm_seproductsrel.setype='Products'
			...
			WHERE ncrm_crmentity.deleted = 0 AND ncrm_seproductsrel.productid = ? ", array($this->getId()));

		$subProductList = array();
		for($i=0; $i<$db->num_rows($result); $i++) {
			$subProductId = $db->query_result($result, $i, 'productid');
			$subProductList[] = Ncrm_Record_Model::getInstanceById($subProductId, 'Products');
		}
	...
	public function getImageDetails() {
		...
		if ($recordId) {
			$sql = "SELECT ncrm_attachments.*, ncrm_crmentity.setype FROM ncrm_attachments
						...
						WHERE ncrm_crmentity.setype = 'Products Image' AND ncrm_seattachmentsrel.crmid = ?";
	...
	public static function getSearchResult($searchKey, $module=false) {
		...
		$query = 'SELECT label, crmid, setype, createdtime FROM ncrm_crmentity WHERE label LIKE ? AND ncrm_crmentity.deleted = 0';
		$params = array("%$searchKey%");

		if($module !== false) {
			$query .= ' AND setype = ?';
			if($module == 'Products'){
				$query = 'SELECT label, crmid, setype, createdtime FROM ncrm_crmentity INNER JOIN ncrm_products ON 
							ncrm_products.productid = ncrm_crmentity.crmid WHERE label LIKE ? AND ncrm_crmentity.deleted = 0 
							AND ncrm_products.discontinued = 1 AND setype = ?';
			}else if($module == 'Services'){
				$query = 'SELECT label, crmid, setype, createdtime FROM ncrm_crmentity INNER JOIN ncrm_service ON 
							ncrm_service.serviceid = ncrm_crmentity.crmid WHERE label LIKE ? AND ncrm_crmentity.deleted = 0 
							AND ncrm_service.discontinued = 1 AND setype = ?';
			}
			$params[] = $module;
		}
	...
	public function getActiveStatusOfRecord(){
		...
		$recordId = $this->getId();
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT discontinued FROM ncrm_products WHERE productid = ?',array($recordId));
