...
<?php
	...
	function checkModuleWorkflow($modulename) {
		...
		$query = "SELECT name FROM ncrm_tab WHERE name not in (" . generateQuestionMarks($modules_not_supported) . ") AND isentitytype=1 AND presence = 0 AND tabid = ?";
	...
	function vtGetModules($adb) {
		...
		$sql = "select distinct ncrm_field.tabid, name
			...
			where ncrm_tab.name not in(" . generateQuestionMarks($modules_not_supported) . ") and ncrm_tab.isentitytype=1 and ncrm_tab.presence in (0,2) ";
