 ...
 */
class Portal_ListView_Model extends Ncrm_ListView_Model {
    
	public function getListViewEntries($pagingModel) {
        $db = PearDatabase::getInstance();
        $moduleModel = Ncrm_Module_Model::getInstance('Portal');
        ...
        for($i = 0; $i < $db->num_rows($listResult); $i++) {
            ...
            $listViewEntries[$row['portalid']]['createdtime'] = Ncrm_Date_UIType::getDisplayDateValue($row['createdtime']);
        }
        $index = 0;
    ...
    public function getQuery() {
        $query = 'SELECT portalid, portalname, portalurl, createdtime FROM ncrm_portal';
