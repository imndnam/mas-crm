...
class Portal_Module_Model extends Ncrm_Module_Model {
    ...
    public function getSideBarLinks($linkParams) {
        ...
        if(empty($recordId)) {
            $portalId = $db->getUniqueID('ncrm_portal');
            $query = "INSERT INTO ncrm_portal VALUES(?,?,?,?,?,?)";
            $params = array($portalId, $bookmarkName, $bookmarkUrl, 0, 0, date('Y-m-d H:i:s'));
        }
        else {
            $query = "UPDATE ncrm_portal SET portalname=?, portalurl=? WHERE portalid=?";
    ...
    public function getRecord($recordId) {
        ...
        $result = $db->pquery('SELECT portalname, portalurl FROM ncrm_portal WHERE portalid = ?', array($recordId));
    ...
    public function deleteRecord($recordId) {
        $db = PearDatabase::getInstance();
        $db->pquery('DELETE FROM ncrm_portal WHERE portalid = ?', array($recordId));
    }
    
    public function getWebsiteUrl($recordId) {
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT portalurl FROM ncrm_portal WHERE portalid=?', array($recordId));
    ...
    public function getAllRecords() {
        ...
        $result = $db->pquery('SELECT portalid, portalname FROM ncrm_portal', array());
