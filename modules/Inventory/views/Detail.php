...
class Inventory_Detail_View extends Ncrm_Detail_View {
	 ...
	 * @return type
	 */
	function showDetailViewByMode(Ncrm_Request $request) {
	 ...
	 * @param Ncrm_Request $request
	 */
	function showLineItemDetails(Ncrm_Request $request) {
		...
		if ($taxtype == 'group') {
			...
			for($i=0; $i<$taxCount; $i++) {
				$taxDetails[$i]['amount'] = Ncrm_Currency_UIType::transformDisplayValue($taxDetails[$i]['amount'], null, true);
			}
		...
		for($i=0; $i<$taxCount; $i++) {
			$shippingTaxDetails[$i]['amount'] = Ncrm_Currency_UIType::transformDisplayValue($shippingTaxDetails[$i]['amount'], null, true);
		}
		...
		foreach ($currencyFieldsList as $fieldName) {
			$finalDetails[$fieldName] = Ncrm_Currency_UIType::transformDisplayValue($finalDetails[$fieldName], null, true);
		}
		...
		for ($i=1; $i<=$productsCount; $i++) {
			...
			if ($taxtype == 'individual') {
				...
				for($j=0; $j<$taxCount; $j++) {
					$taxDetails[$j]['amount'] = Ncrm_Currency_UIType::transformDisplayValue($taxDetails[$j]['amount'], null, true);
				}
			...
			foreach ($currencyFieldsList as $fieldName) {
				$product[$fieldName.$i] = Ncrm_Currency_UIType::transformDisplayValue($product[$fieldName.$i], null, true);
			}
