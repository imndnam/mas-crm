...
class Inventory_SendEmail_View extends Ncrm_ComposeEmail_View {
     ...
     */
    public function composeMailData(Ncrm_Request $request) {
        parent::composeMailData($request);
        $viewer = $this->getViewer($request);
        $inventoryRecordId = $request->get('record');
        $recordModel = Ncrm_Record_Model::getInstanceById($inventoryRecordId, $request->getModule());
    ...
    public function populateTo($request){
        ...
        foreach($toEmailConsiderableFields as $fieldName){
            ...
            $fieldLabel = Ncrm_Util_Helper::getLabel($fieldValue);
            $referenceModuleModel = Ncrm_Module_Model::getInstance($referenceModule);
