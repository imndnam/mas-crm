		...
		if ('open' == $this->getOperationArg($request)) {
			...
			if (!empty($to_string)) {
				...
				foreach($toArray as $to) {
					$relatedtos = MailManager::lookupMailInNcrm($to, $currentUserModel);
					...
					if ($status === true) {
						...
						if ($realid[1] == -1) {
							$db->pquery('DELETE FROM ncrm_salesmanactivityrel WHERE smid=? AND activityid=?',$params);
							$db->pquery('INSERT INTO ncrm_salesmanactivityrel VALUES (?,?)', $params);
						} else {
							$db->pquery('DELETE FROM ncrm_seactivityrel WHERE crmid=? AND activityid=?', $params);
							$db->pquery('INSERT INTO ncrm_seactivityrel VALUES (?,?)', $params);
						}
					}
				}
			}
