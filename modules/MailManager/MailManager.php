...
class MailManager {
	...
	static function updateMailAssociation($mailuid, $emailid, $crmid) {
		global $adb;
		$adb->pquery("INSERT INTO ncrm_mailmanager_mailrel (mailuid, emailid, crmid) VALUES (?,?,?)", array($mailuid, $emailid, $crmid));
	}

	static function lookupMailInNcrm($searchTerm, $user) {
		...
		foreach($referenceModules as $referenceModule) {
			...
			if(!empty($searchFieldList) && !empty($referenceModuleEmailFields)) {
                ...
                if($referenceModule == 'Users' && !is_admin($user)){
                    ...
                    global $adb;
                    $where .= " AND ncrm_users.status='Active'";
                    $query = "select $searchFieldListString,id from ncrm_users $where";
	...
	static function lookupMailAssociation($mailuid) {
		...
		$result = $adb->pquery(
			"SELECT ncrm_mailmanager_mailrel.* FROM ncrm_mailmanager_mailrel INNER JOIN
			ncrm_crmentity ON ncrm_crmentity.crmid=ncrm_mailmanager_mailrel.crmid AND ncrm_crmentity.deleted=0
			AND ncrm_mailmanager_mailrel.mailuid=? LIMIT 1", array(decode_html($mailuid)));
