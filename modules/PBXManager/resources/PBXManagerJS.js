...
var Ncrm_PBXManager_Js = {
    /**
	 * Function registers PBX for popups
	 */
    registerPBXCall : function() {
        Ncrm_PBXManager_Js.requestPBXgetCalls();
    },
    ...
    registerPBXOutboundCall : function(number,record) {
        Ncrm_PBXManager_Js.makeOutboundCall(number,record);
    },
    /**
            ...
            if(data.success && data.result) {
                for(i=0; i< data.result.length; i++) {
                    ...
                    if(jQuery('#pbxcall_'+record.pbxmanagerid+'').size()== 0 )
                        Ncrm_PBXManager_Js.showPBXIncomingCallPopup(record);
                    else
                        Ncrm_PBXManager_Js.updatePBXIncomingCallPopup(record);
                    }
                }
        });
        Ncrm_PBXManager_Js.removeCompletedCallPopup();
    },
    ...
    showPBXIncomingCallPopup : function(record) {
            ...
            if(record.user) {
                if(record.user != record.current_user_id) {
                    Ncrm_PBXManager_Js.removeCallPopup(record.pbxmanagerid);
                }
            }
                
            // To check if it is new or existing contact
            Ncrm_PBXManager_Js.checkIfRelatedModuleRecordExist(record);
            ...
            jQuery('#pbxcontactsave_'+record.pbxmanagerid+'').bind('click', function(e) {
              ...
              Ncrm_PBXManager_Js.createRecord(e, record);
            ...
            AppConnector.request(url).then(function(data){
                if(data.result){
                    if(data.result!='in-progress' && data.result!='ringing'){
                        Ncrm_PBXManager_Js.removeCallPopup(callid);
                    }
                }
            });    
        }
    ...
    makeOutboundCall : function(number, record){
     ...
     registerEvents : function(){
        ...
        AppConnector.request(url).then(function(data){
            if(data.result) {
                Ncrm_PBXManager_Js.registerPBXCall();
                setInterval("Ncrm_PBXManager_Js.registerPBXCall()", 3000);
            }
        });
    }
...
jQuery(document).ready(function() {
    Ncrm_PBXManager_Js.registerEvents();
});
