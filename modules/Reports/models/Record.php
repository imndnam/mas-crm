...
class Reports_Record_Model extends Ncrm_Record_Model {
	/**
	...
	public static function getInstanceById($recordId) {
		...
		$self = new self();
		$reportResult = $db->pquery('SELECT * FROM ncrm_report WHERE reportid = ?', array($recordId));
	...
	function initialize() {
		$reportId = $this->getId();
		$this->report = Ncrm_Report_Model::getInstance($reportId);
	}
	...
	function getPrimaryModuleFields() {
		...
		//need to add this ncrm_crmentity:crmid:".$module."_ID:crmid:I
	...
	function getSelectedFields() {
		...
		$result = $db->pquery("SELECT ncrm_selectcolumn.columnname FROM ncrm_report
					INNER JOIN ncrm_selectquery ON ncrm_selectquery.queryid = ncrm_report.queryid
					INNER JOIN ncrm_selectcolumn ON ncrm_selectcolumn.queryid = ncrm_selectquery.queryid
					WHERE ncrm_report.reportid = ? ORDER BY ncrm_selectcolumn.columnindex", array($this->getId()));
	...
	function getSelectedCalculationFields() {
		...
		$result = $db->pquery('SELECT ncrm_reportsummary.columnname FROM ncrm_reportsummary
					INNER JOIN ncrm_report ON ncrm_report.reportid = ncrm_reportsummary.reportsummaryid
					WHERE ncrm_report.reportid=?', array($this->getId()));
	...
	function getSelectedSortFields() {
		...
		$result = $db->pquery('SELECT ncrm_reportsortcol.* FROM ncrm_report
					INNER JOIN ncrm_reportsortcol ON ncrm_report.reportid = ncrm_reportsortcol.reportid
					WHERE ncrm_report.reportid = ? ORDER BY ncrm_reportsortcol.sortcolid',array($this->getId()));
	...
	function getSelectedStandardFilter() {
		...
		$result = $db->pquery('SELECT * FROM ncrm_reportdatefilter WHERE datefilterid = ? AND startdate != ? AND enddate != ?',
	...
	function save() {
		...
		$reportId = $this->getId();
		if(empty($reportId)) {
			$reportId = $db->getUniqueID("ncrm_selectquery");
			$this->setId($reportId);

			$db->pquery('INSERT INTO ncrm_selectquery(queryid, startindex, numofobjects) VALUES(?,?,?)',
					array($reportId, 0, 0));
			...
			$db->pquery('INSERT INTO ncrm_report(reportid, folderid, reportname, description,
								reporttype, queryid, state, owner, sharingtype) VALUES(?,?,?,?,?,?,?,?,?)', $reportParams);


			$secondaryModule = $this->getSecondaryModules();
			$db->pquery('INSERT INTO ncrm_reportmodules(reportmodulesid, primarymodule, secondarymodules) VALUES(?,?,?)',
			...
			$reportId = $this->getId();
			$db->pquery('DELETE FROM ncrm_selectcolumn WHERE queryid = ?', array($reportId));
			$this->saveSelectedFields();

			$db->pquery("DELETE FROM ncrm_reportsharing WHERE reportid = ?", array($reportId));
			$this->saveSharingInformation();


			$db->pquery('UPDATE ncrm_reportmodules SET primarymodule = ?,secondarymodules = ? WHERE reportmodulesid = ?',
					array($this->getPrimaryModule(), $this->getSecondaryModules(), $reportId));

			$db->pquery('UPDATE ncrm_report SET reportname = ?, description = ?, reporttype = ?, folderid = ? WHERE
				reportid = ?', array($this->get('reportname'), $this->get('description'), $this->get('reporttype'), $this->get('folderid'), $reportId));


			$db->pquery('DELETE FROM ncrm_reportsortcol WHERE reportid = ?', array($reportId));
			$db->pquery('DELETE FROM ncrm_reportgroupbycolumn WHERE reportid = ?',array($reportId));
			$this->saveSortFields();

			$db->pquery('DELETE FROM ncrm_reportsummary WHERE reportsummaryid = ?', array($reportId));
			$this->saveCalculationFields();

			$db->pquery('DELETE FROM ncrm_reportdatefilter WHERE datefilterid = ?', array($reportId));
			$this->saveStandardFilter();
	...
	function saveSortFields() {
        ...
        if(!empty($sortFields)){
            ...
            foreach($sortFields as $fieldInfo) {
                $db->pquery('INSERT INTO ncrm_reportsortcol(sortcolid, reportid, columnname, sortorder) VALUES (?,?,?,?)',
                        array($i, $this->getId(), $fieldInfo[0], $fieldInfo[1]));
                if(IsDateField($fieldInfo[0])) {
                    if(empty($fieldInfo[2])){
                        $fieldInfo[2] = 'None';
                    }
                    $db->pquery("INSERT INTO ncrm_reportgroupbycolumn(reportid, sortid, sortcolname, dategroupbycriteria)
	...
	function saveCalculationFields() {
		...
		for ($i=0; $i<count($calculationFields); $i++) {
			$db->pquery('INSERT INTO ncrm_reportsummary (reportsummaryid, summarytype, columnname) VALUES (?,?,?)',
	...
	function saveStandardFilter() {
		...
		if(!empty($standardFilter)) {
			$db->pquery('INSERT INTO ncrm_reportdatefilter (datefilterid, datecolumnname, datefilter, startdate, enddate)
	...
	function saveSharingInformation() {
		...
		for($i=0; $i<count($sharingInfo); $i++) {
			$db->pquery('INSERT INTO ncrm_reportsharing(reportid, shareid, setype) VALUES (?,?,?)',
        ...
        if(!empty($selectedFields)){
           for($i=0 ;$i<count($selectedFields);$i++) {
                if(!empty($selectedFields[$i])) {
                    $db->pquery("INSERT INTO ncrm_selectcolumn(queryid, columnindex, columnname) VALUES (?,?,?)",
	...
	function saveAdvancedFilters() {
		...
		if(!empty($advancedFilter)) {
			...
			$db->pquery('DELETE FROM ncrm_relcriteria WHERE queryid = ?', array($reportId));
			$db->pquery('DELETE FROM ncrm_relcriteria_grouping WHERE queryid = ?', array($reportId));
			...
			foreach($advancedFilter as $groupIndex => $groupInfo) {
				...
				foreach($groupColumns as $columnIndex => $columnCondition) {
					...
					if($fieldType == 'currency') {
						if($field->getUIType() == '72') {
							...
							$advFilterValue = Ncrm_Currency_UIType::convertToDBFormat($advFilterValue, null, true);
						} else {
							$advFilterValue = Ncrm_Currency_UIType::convertToDBFormat($advFilterValue);
						}
					}
					...
					$db->pquery('INSERT INTO ncrm_relcriteria (queryid, columnindex, columnname, comparator, value,
				...
				$db->pquery("INSERT INTO ncrm_relcriteria_grouping(groupid, queryid, group_condition, condition_expression) VALUES (?,?,?,?)",
	...
	function saveScheduleInformation() {
		...
		$db->pquery('INSERT INTO ncrm_scheduled_reports(reportid, recipients, schedule, format, next_trigger_time) VALUES
	...
	function deleteScheduling() {
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM ncrm_scheduled_reports WHERE reportid = ?', array($this->getId()));
	}
	...
	function move($folderId) {
		...
		$db->pquery("UPDATE ncrm_report SET folderid = ? WHERE reportid = ?", array($folderId, $this->getId()));
	}
	...
	public function transformStandardFilter(){
		...
		if(!empty($standardFilter)){
			...
			if($fields[1] == 'createdtime' || $fields[1] == 'modifiedtime' ||($fields[0] == 'ncrm_activity' && $fields[1] == 'date_start')){
	...
	public function checkDuplicate() {
		...
		$query = "SELECT 1 FROM ncrm_report WHERE reportname = ?";
	...
	function showLineItemFieldsInFilter($calculationFields=false) {
		...
		if(!empty($calculationFields)) {
			foreach($calculationFields as $field) {
				if(stripos($field, 'cb:ncrm_inventoryproductrel') !== false) {
					return true;
				}
			}
			return false;
    ...
    public function getRecordsListFromRequest(Ncrm_Request $request) {
    ...
    function saveReportType(){
        ...
        if(!empty($data)){
            $db->pquery('DELETE FROM ncrm_reporttype WHERE reportid = ?', array($this->getId()));
            $db->pquery("INSERT INTO ncrm_reporttype(reportid, data) VALUES (?,?)",
    ...
    function getReportTypeInfo() {
		...
		$result = $db->pquery("SELECT data FROM ncrm_reporttype WHERE reportid = ?", array($this->getId()));
		...
		if(is_array($fields)) foreach($fields as $module => $blocks) {
			if(is_array($blocks)) foreach($blocks as $blockLabel => $blockFields) {
				if(is_array($blockFields)) foreach($blockFields as $reportFieldInfo => $fieldLabel) {
					...
					if(empty($fieldInstance) || $fieldInfo[0] == 'ncrm_inventoryproductrel' || $fieldInstance->getFieldDataType() == 'email'
		...
		if(is_array($secondaryModules)) {
			...
			foreach($secondaryModules as $secondaryModule) {
				if(!empty($secondaryModule)) {
					$secondaryModuleModel = Ncrm_Module_Model::getInstance($secondaryModule);
		...
		if(is_array($fields)) foreach($fields as $module => $blocks) {
			if(is_array($blocks)) foreach($blocks as $blockLabel => $blockFields) {
				if(is_array($blockFields)) foreach($blockFields as $reportFieldInfo => $fieldLabel) {
					...
					if(empty($fieldInstance) || $fieldInfo[0] == 'ncrm_inventoryproductrel'
