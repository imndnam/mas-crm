...
class Reports_Module_Model extends Ncrm_Module_Model {
	...
	function deleteRecord($reportModel) {
		...
		if($currentUser->isAdminUser() || in_array($owner, $subOrdinates) || $owner == $currentUser->getId()) {
			...
			$db->pquery('DELETE FROM ncrm_selectquery WHERE queryid = ?', array($reportId));

			$db->pquery('DELETE FROM ncrm_report WHERE reportid = ?', array($reportId));

			$db->pquery('DELETE FROM ncrm_schedulereports WHERE reportid = ?', array($reportId));

            $db->pquery('DELETE FROM ncrm_reporttype WHERE reportid = ?', array($reportId));

			$result = $db->pquery('SELECT * FROM ncrm_homereportchart WHERE reportid = ?',array($reportId));
			$numOfRows = $db->num_rows($result);
			for ($i = 0; $i < $numOfRows; $i++) {
				$homePageChartIdsList[] = $adb->query_result($result, $i, 'stuffid');
			}
			if ($homePageChartIdsList) {
				$deleteQuery = 'DELETE FROM ncrm_homestuff WHERE stuffid IN (' . implode(",", $homePageChartIdsList) . ')';
	 ...
	 * @return <Array of Ncrm_Link_Model>
	 */
	...
	function getRecentRecords($limit = 10) {
		...
		$result = $db->pquery('SELECT * FROM ncrm_report ORDER BY reportid DESC LIMIT ?', array($limit));
