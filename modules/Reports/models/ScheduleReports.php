...
class Reports_ScheduleReports_Model extends Ncrm_Base_Model {
	...
	public static function getInstanceById($recordId) {
		...
		if (!empty($recordId)) {
			$scheduledReportResult = $db->pquery('SELECT * FROM ncrm_schedulereports WHERE reportid = ?', array($recordId));
	...
	public function saveScheduleReport() {
		...
		if ($scheduleid == self::$SCHEDULED_ON_SPECIFIC_DATE) {
            ...
            $currentTime = Ncrm_Util_Helper::getActiveAdminCurrentDateTime();
		...
		if ($isReportScheduled == '0' || $isReportScheduled == '' || $isReportScheduled == false) {
			$deleteScheduledReportSql = "DELETE FROM ncrm_schedulereports WHERE reportid=?";
			$adb->pquery($deleteScheduledReportSql, array($reportid));
		} else {
			$checkScheduledResult = $adb->pquery('SELECT 1 FROM ncrm_schedulereports WHERE reportid=?', array($reportid));
			if ($adb->num_rows($checkScheduledResult) > 0) {
				$scheduledReportSql = 'UPDATE ncrm_schedulereports SET scheduleid=?, recipients=?, schdate=?, schtime=?, schdayoftheweek=?, schdayofthemonth=?, schannualdates=?, specificemails=?, next_trigger_time=? WHERE reportid=?';
				...
				$scheduleReportSql = 'INSERT INTO ncrm_schedulereports (reportid,scheduleid,recipients,schdate,schtime,schdayoftheweek,schdayofthemonth,schannualdates,next_trigger_time,specificemails) VALUES (?,?,?,?,?,?,?,?,?,?)';
	...
	public function getRecipientEmails() {
		...
		if (!empty($recipientsList) && count($recipientsList) > 0) {
			foreach ($recipientsList as $userId) {
                if(!Ncrm_Util_Helper::isUserDeleted($userId)) {
	...
	public function sendEmail() {
        ...
        Ncrm_Utils::ModuleLog('ScheduleReprots', $recipientEmails);
        ...
        Ncrm_Utils::ModuleLog('ScheduleReprots Send Mail Start ::', $currentTime);
		$reportname = decode_html($reportRecordModel->getName());
        $subject = $reportname;
        Ncrm_Utils::ModuleLog('ScheduleReprot Name ::', $reportname);
		$ncrmMailer->Subject = $subject;
		$ncrmMailer->Body = $this->getEmailContent($reportRecordModel);
		$ncrmMailer->IsHTML();
		...
		foreach ($attachments as $attachmentName => $path) {
			$ncrmMailer->AddAttachment($path, decode_html($attachmentName));
		}
		//Added cc to account owner
		$accountOwnerId = Users::getActiveAdminId();
		$ncrmMailer->AddCC(getUserEmail($accountOwnerId), getUserFullName($accountOwnerId));
		$status = $ncrmMailer->Send(true);
	...
	public function updateNextTriggerTime() {
		$adb = PearDatabase::getInstance();
		$nextTriggerTime = $this->getNextTriggerTime();
        Ncrm_Utils::ModuleLog('ScheduleReprot Next Trigger Time >> ', $nextTriggerTime);
		$adb->pquery('UPDATE ncrm_schedulereports SET next_trigger_time=? WHERE reportid=?', array($nextTriggerTime, $this->get('reportid')));
        Ncrm_Utils::ModuleLog('ScheduleReprot', 'Next Trigger Time updated');
	}
	...
	public static function getScheduledReports() {
		...
		@date_default_timezone_set($default_timezone);
		$result = $adb->pquery("SELECT reportid FROM ncrm_schedulereports WHERE next_trigger_time = '' || next_trigger_time <= ?", array($currentTimestamp));
	...
	public static function runScheduledReports() {
		...
		foreach ($scheduledReports as $scheduledReport) {
            ...
            Ncrm_Utils::ModuleLog('ScheduleReprot Send Mail Status ', $status);
			if($status)
