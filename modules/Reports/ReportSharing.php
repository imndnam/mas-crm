...
$report_std_filter = new ncrmCRM_Smarty; 
...
function getVisibleCriteria($recordid='')
{
	...
	if($recordid!='')
	{
		$result = $adb->pquery("select sharingtype from ncrm_report where reportid=?",array($recordid));
		$selcriteria=$adb->query_result($result,0,"sharingtype");
	}
	if($selcriteria == ""){
		$selcriteria = 'Public';
	}
	$filter_result = $adb->query("select * from ncrm_reportfilters");
...
function getShareInfo($recordid=''){
	global $adb;
	$member_query = $adb->pquery("SELECT ncrm_reportsharing.setype,ncrm_users.id,ncrm_users.user_name FROM ncrm_reportsharing INNER JOIN ncrm_users on ncrm_users.id = ncrm_reportsharing.shareid WHERE ncrm_reportsharing.setype='users' AND ncrm_reportsharing.reportid = ?",array($recordid));
	...
	$member_query = $adb->pquery("SELECT ncrm_reportsharing.setype,ncrm_groups.groupid,ncrm_groups.groupname FROM ncrm_reportsharing INNER JOIN ncrm_groups on ncrm_groups.groupid = ncrm_reportsharing.shareid WHERE ncrm_reportsharing.setype='groups' AND ncrm_reportsharing.reportid = ?",array($recordid));
	$noofrows = $adb->num_rows($member_query);
	if($noofrows > 0){
		for($i=0;$i<$noofrows;$i++){
