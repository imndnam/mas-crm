...
class Ncrm_Reference_UIType extends Ncrm_Base_UIType {
	...
	public function getReferenceModule($value) {
		...
		if(in_array($referenceEntityType, $referenceModuleList)) {
			return Ncrm_Module_Model::getInstance($referenceEntityType);
		} elseif (in_array('Users', $referenceModuleList)) {
			return Ncrm_Module_Model::getInstance('Users');
		}
		return null;
	}
	...
	public function getDisplayValue($value) {
		...
		if($referenceModule && !empty($value)) {
			...
			if($referenceModuleName == 'Users') {
				...
				$nameResult = $db->pquery('SELECT first_name, last_name FROM ncrm_users WHERE id = ?', array($value));
