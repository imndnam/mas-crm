...
class Ncrm_Email_UIType extends Ncrm_Base_UIType {
		...
		if($value){
			...
			if ($internalMailer == 1) {
                ...
                if($moduleName == "Users" && $fieldName == "user_name"){
                    $value = "<a class='cursorPointer' onclick=\"Ncrm_Helper_Js.getInternalMailer($recordId," .
                    		"'$fieldName','$moduleName');\">" . textlength_check($value) . "</a>";
                }else{
                	$value = "<a class='emailField cursorPointer' onclick=\"Ncrm_Helper_Js.getInternalMailer($recordId," .
                    		"'$fieldName','$moduleName');\">" . textlength_check($value) . "</a>";
                }
			} else {
                if($moduleName == "Users" && $fieldName == "user_name"){
