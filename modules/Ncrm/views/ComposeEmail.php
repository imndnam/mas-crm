...
class Ncrm_ComposeEmail_View extends Ncrm_Footer_View {
	...
	public function composeMailData($request){
		...
		if ($documentIds) {
			...
			foreach ($documentIds as $documentId) {
				$documentRecordModel = Ncrm_Record_Model::getInstanceById($documentId, $sourceModule);
		...
		if ($sourceRecordId) {
			$sourceRecordModel = Ncrm_Record_Model::getInstanceById($sourceRecordId);
		...
		foreach($selectIds as $id) {
			if ($id) {
				...
				if (count($parentIdComponents) > 1) {
					...
					$recordModel = Ncrm_Record_Model::getInstanceById($id, $fieldModule);
				} else {
					$recordModel = Ncrm_Record_Model::getInstanceById($id);
				}
				if($selectedFields){
		...
		$documentsModel = Ncrm_Module_Model::getInstance('Documents');
		$documentsURL = $documentsModel->getInternalDocumentsURL();

		$emailTemplateModuleModel = Settings_Ncrm_Module_Model::getInstance('Settings:EmailTemplates');
		...
		$viewer->assign('MODULE_IS_ACTIVE', $userPrevilegesModel->hasModulePermission(Ncrm_Module_Model::getInstance('EmailTemplates')->getId()));
		//
	...
	function emailEdit($request){
        ...
        foreach($parentIds as $index=>$parentFieldId) {
			...
			if ($parentIdComponents[1] === '-1') {
				...
				$parentRecordModel = Ncrm_Record_Model::getInstanceById($parentId);
			}
