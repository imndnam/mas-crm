...
class Ncrm_BasicAjax_View extends Ncrm_Basic_View {
	 ...
	 */
	function showAdvancedSearch(Ncrm_Request $request) {
		//Modules for which search is excluded
		$excludedModuleForSearch = array('Ncrm', 'Reports');
	 ...
	 * @param Ncrm_Request $request
	 */
	function showSearchResults(Ncrm_Request $request) {
		...
		if(is_array($advFilterList) && count($advFilterList) > 0) {
			...
			for($i=0; $i<$rows; ++$i) {
				...
				$recordInstance = Ncrm_Record_Model::getInstanceById($row[0]);
			...
			$matchingRecords =  Ncrm_Record_Model::getSearchResult($searchKey, $searchModule);
		}
