...
class Ncrm_MassActionAjax_View extends Ncrm_IndexAjax_View {
	 ...
	 */
	function showMassEditForm (Ncrm_Request $request){
		...
		$recordStructureInstance = Ncrm_RecordStructure_Model::getInstanceForModule($moduleModel, Ncrm_RecordStructure_Model::RECORD_STRUCTURE_MODE_MASSEDIT);
		$fieldInfo = array();
	 ...
	 * @param Ncrm_Request $request
	 */
	function showAddCommentForm(Ncrm_Request $request){
	 ...
	 * @param Ncrm_Request $request
	 */
	function showComposeEmailForm(Ncrm_Request $request) {
		$moduleName = 'Emails';
		$sourceModule = $request->getModule();
		$cvId = $request->get('viewname');
		$selectedIds = $request->get('selected_ids');
		...
		$moduleModel = Ncrm_Module_Model::getInstance($sourceModule);
        ...
        if($emailFieldCount > 1) {
            ...
            $queryWithFromClause = 'SELECT '. implode(',',$emailColumnNames). ' FROM ncrm_crmentity ';
            foreach($emailFields as $emailFieldModel) {
                ...
                $queryWithFromClause .= ' INNER JOIN '.$fieldTableName .
                            ' ON '.$fieldTableName.'.'.$tabNameIndexList[$fieldTableName].'= ncrm_crmentity.crmid';
            }
            $query =  $queryWithFromClause . ' WHERE ncrm_crmentity.deleted = 0 AND crmid IN ('.  generateQuestionMarks($recordIds).') AND (';
	 ...
	 * @param Ncrm_Request $request
	 */
	function showSendSMSForm(Ncrm_Request $request) {
		...
		if(count($selectedIds) == 1){
			...
			$selectedRecordModel = Ncrm_Record_Model::getInstanceById($recordId, $sourceModule);
	 ...
	 * @return integer
	 */
	function getRecordsListFromRequest(Ncrm_Request $request, $module = false) {
		$cvId = $request->get('viewname');
		$selectedIds = $request->get('selected_ids');
		$excludedIds = $request->get('excluded_ids');
        if(empty($module)) {
		...
		if ($sourceRecord && $sourceModule) {
			$sourceRecordModel = Ncrm_Record_Model::getInstanceById($sourceRecord, $sourceModule);
	 ...
	 * @param Ncrm_Request $request
	 */
	function showMailMergeTemplates(Ncrm_Request $request) {
		$selectedIds = $request->get('selected_ids');
		$excludedIds = $request->get('excluded_ids');
		$cvId = $request->get('viewname');
		$module = $request->getModule();
	 ...
	 * @param Ncrm_Request $request
	 */
	function showDuplicatesSearchForm(Ncrm_Request $request) {
		$module = $request->getModule();
	...
	function transferOwnership(Ncrm_Request $request){
		$module = $request->getModule();
		$moduleModel = Ncrm_Module_Model::getInstance($module);
