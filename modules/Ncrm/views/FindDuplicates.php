...
class Ncrm_FindDuplicates_View extends Ncrm_List_View {
	...
	function getHeaderScripts(Ncrm_Request $request) {
		...
		$jsFileNames = array(
			...
			'modules.Ncrm.resources.FindDuplicates',
		);
	...
	public function initializeListViewContents(Ncrm_Request $request, Ncrm_Viewer $viewer) {
		...
		$massActionLink = array(
			...
			'linkurl' => 'Javascript:Ncrm_FindDuplicates_Js.massDeleteRecords("index.php?module='.$module.'&action=MassDelete");',
			'linkicon' => ''
		);
		$massActionLinks[] = Ncrm_Link_Model::getInstanceFromValues($massActionLink);
		...
		$dataModelInstance = Ncrm_FindDuplicate_Model::getInstance($module);
	...
	function getRecordsCount(Ncrm_Request $request) {
		$moduleName = $request->getModule();
		$duplicateSearchFields = $request->get('fields');
		$dataModelInstance = Ncrm_FindDuplicate_Model::getInstance($moduleName);
