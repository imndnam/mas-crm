...
class Ncrm_Popup_View extends Ncrm_Footer_View {
	 ...
	 * @return <String>
	 */
	function getModule(Ncrm_request $request) {
		$moduleName = $request->getModule();
		return $moduleName;
	}
	...
	function getHeaderScripts(Ncrm_Request $request) {
		...
		$jsFileNames = array(
			...
			'modules.Ncrm.resources.BaseList',
	...
	public function initializeListViewContents(Ncrm_Request $request, Ncrm_Viewer $viewer) {
        ...
        $isRecordExists = Ncrm_Util_Helper::checkRecordExistance($relatedParentId);
		...
		if(!empty($relatedParentModule) && !empty($relatedParentId)) {
			$parentRecordModel = Ncrm_Record_Model::getInstanceById($relatedParentId, $relatedParentModule);
			$listViewModel = Ncrm_RelationListView_Model::getInstance($parentRecordModel, $moduleName, $label);
		}else{
			$listViewModel = Ncrm_ListView_Model::getInstanceForPopup($moduleName);
		}
