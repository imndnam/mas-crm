...
class Ncrm_Import_View extends Ncrm_Index_View {
	...
	function importBasicStep(Ncrm_Request $request) {
	...
	function uploadAndParse(Ncrm_Request $request) {
	...
	function import(Ncrm_Request $request) {
		$user = Users_Record_Model::getCurrentUserModel();
		Import_Main_View::import($request, $user);
	}

	function undoImport(Ncrm_Request $request) {
        ...
        $entity->triggerEvent('ncrm.batchevent.delete',$entityData);
        $NCRM_BULK_SAVE_MODE = $previousBulkSaveMode;
		$viewer->assign('FOR_MODULE', $moduleName);
		$viewer->assign('MODULE', 'Import');
	...
	function lastImportedRecords(Ncrm_Request $request) {
		$importList = new Import_List_View();
		$importList->process($request);
	}

	function deleteMap(Ncrm_Request $request) {
		Import_Main_View::deleteMap($request);
	}

	//TODO need to move it to an action
	function clearCorruptedData(Ncrm_Request $request) {
		$user = Users_Record_Model::getCurrentUserModel();
		Import_Utils_Helper::clearUserImportInfo($user);
		$this->importBasicStep($request);
	}

	function cancelImport(Ncrm_Request $request) {
	...
	function checkImportStatus(Ncrm_Request $request) {
		$moduleName = $request->getModule();
		$user = Users_Record_Model::getCurrentUserModel();
		$mode = $request->getMode();
