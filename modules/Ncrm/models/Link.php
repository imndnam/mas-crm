...
/**
 * Ncrm Link Model Class
 */
class Ncrm_Link_Model extends Ncrm_Link {
	 ...
	 * @return Ncrm_Link_Model instance
	 */
	...
	public function getIconPath() {
		...
		return Ncrm_Theme::getImagePath($this->getIcon());
	}
	 ...
	 * @param Ncrm_Link_Model $link - link model
	 * @result Ncrm_Link_Model - current Instance;
	 */
	public function addChildLink(Ncrm_Link_Model $link) {
	 ...
	 * @result <array> - list of Ncrm_Link_Model instances
	 */
	...
	public function convertToNativeLink() {
		...
		if(!$this->isPageLoadLink()){
           ...
           $url = Ncrm_Util_Helper::toSafeHTML($url);
           return $url;
		}
		...
		if(!empty($module) && !empty ($sourceModule) && !empty($sourceRecord)) {
			...
			$relatedModuleModel = Ncrm_Module_Model::getInstance($module);
	...
	/**
	 * Function to get the instance of Ncrm Link Model from the given array of key-value mapping
	...
	/**
	 * Function to get the instance of Ncrm Link Model from a given Ncrm_Link object
	...
	/**
	 * Function to get all the Ncrm Link Models for a module of the given list of link types
	 * @param <Number> $tabid
	 * @param <Array> $type
	...
	public static function getAllByType($tabid, $type = false, $parameters = false) {
		$links = Ncrm_Cache::get('links-'.$tabid, $type);
		if(!$links) {
			$links = parent::getAllByType($tabid, $type, $parameters);
			Ncrm_Cache::set('links-'.$tabid, $type, $links);
		}
