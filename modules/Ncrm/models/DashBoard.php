...
class Ncrm_DashBoard_Model extends Ncrm_Base_Model {
	 ...
	 * @return <Array of Ncrm_Widget_Model>
	 */
	public function getSelectableDashboard() {
		...
		$sql = 'SELECT * FROM ncrm_links WHERE linktype = ?
					AND tabid = ? AND linkid NOT IN (SELECT linkid FROM ncrm_module_dashboard_widgets
					WHERE userid = ?)';
		$params = array('DASHBOARDWIDGET', $moduleModel->getId(), $currentUser->getId());

		$sql .= ' UNION SELECT * FROM ncrm_links WHERE linklabel in (?,?)';
		$params[] = 'Mini List';
		$params[] = 'Notebook';
		...
		for($i=0; $i<$db->num_rows($result); $i++) {
			...
			if($row['linklabel'] == 'Tag Cloud') {
			 ...
			 if($this->checkModulePermission($row)) {
                $widgets[] = Ncrm_Widget_Model::getInstanceFromValues($row);
            }
		}
	...
	public function getDashboards() {
		...
		$sql = " SELECT ncrm_links.*, ncrm_module_dashboard_widgets.userid, ncrm_module_dashboard_widgets.filterid, ncrm_module_dashboard_widgets.data, ncrm_module_dashboard_widgets.id as widgetid, ncrm_module_dashboard_widgets.position as position, ncrm_links.linkid as id FROM ncrm_links ".
				" INNER JOIN ncrm_module_dashboard_widgets ON ncrm_links.linkid=ncrm_module_dashboard_widgets.linkid".
				" WHERE (ncrm_module_dashboard_widgets.userid = ? AND linktype = ? AND tabid = ?)";
	 ...
	 * @return <Ncrm_DashBoard_Model>
	 */
	public static function getInstance($moduleName) {
		$modelClassName = Ncrm_Loader::getComponentClassName('Model', 'DashBoard', $moduleName);
