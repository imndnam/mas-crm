...
/**
 * Ncrm MenuStructure Model
 */
class Ncrm_MenuStructure_Model extends Ncrm_Base_Model {
	 ...
	 * @return <array> - Associate array of Parent name mapped to Ncrm_Menu_Model instances
	 */
	public function getMore() {
		...
		# Merge analytics into tools
		$moreTabs['TOOLS'] = array_merge($moreTabs['ANALYTICS'], $moreTabs['TOOLS']);
		unset($moreTabs['ANALYTICS']);
		return $moreTabs;
	}
	...
	/**
	 * Function to get an instance of the Ncrm MenuStructure Model from list of menu models
	 * @param <array> $menuModelList - array of Ncrm_Menu_Model instances
	 * @return Ncrm_MenuStructure_Model instance
	 */
	public static function getInstanceFromMenuList($menuModelList, $selectedMenu='') {
		...
		foreach ($menuListArray[self::MORE_MENU_INDEX] as $parent => &$values) {
			uksort($values, array('Ncrm_MenuStructure_Model', 'sortMenuItemsByProcess'));
		}
		//uksort($menuListArray[self::TOP_MENU_INDEX], array('Ncrm_MenuStructure_Model', 'sortMenuItemsByProcess'));
