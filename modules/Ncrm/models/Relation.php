...
class Ncrm_Relation_Model extends Ncrm_Base_Model{
	...
	public function getParentModuleModel(){
		if(empty($this->parentModule)){
			$this->parentModule = Ncrm_Module_Model::getInstance($this->get('tabid'));
		}
	...
	public function getRelationModuleModel(){
		if(empty($this->relatedModule)){
			$this->relatedModule = Ncrm_Module_Model::getInstance($this->get('related_tabid'));
		}
	...
	public static function getInstance($parentModuleModel, $relatedModuleModel, $label=false) {
		...
		$query = 'SELECT ncrm_relatedlists.*,ncrm_tab.name as modulename FROM ncrm_relatedlists
					INNER JOIN ncrm_tab on ncrm_tab.tabid = ncrm_relatedlists.related_tabid AND ncrm_tab.presence != 1
					WHERE ncrm_relatedlists.tabid = ? AND related_tabid = ?';
		...
		if($db->num_rows($result)) {
			...
			$relationModelClassName = Ncrm_Loader::getComponentClassName('Model', 'Relation', $parentModuleModel->get('name'));
	...
	public static function getAllRelations($parentModuleModel, $selected = true, $onlyActive = true) {
        ...
        $query = 'SELECT ncrm_relatedlists.*,ncrm_tab.name as modulename FROM ncrm_relatedlists 
                    INNER JOIN ncrm_tab on ncrm_relatedlists.related_tabid = ncrm_tab.tabid
                    WHERE ncrm_relatedlists.tabid = ? AND related_tabid != 0';

		if ($selected) {
			$query .= ' AND ncrm_relatedlists.presence <> 1';
		}
        if($onlyActive){
            $query .= ' AND ncrm_tab.presence <> 1 ';
        }
        $query .= ' AND ncrm_relatedlists.name NOT IN ('.generateQuestionMarks($skipReltionsList).') ORDER BY sequence'; // TODO: Need to handle entries that has related_tabid 0
		...
		for($i=0; $i<$db->num_rows($result); $i++) {
			...
			//$relationModuleModel = Ncrm_Module_Model::getCleanInstance($moduleName);
	...
	public function getRelationField() {
		...
		if (!$relationField) {
			...
			foreach($relatedModelFields as $fieldName => $fieldModel) {
				if($fieldModel->getFieldDataType() == Ncrm_Field_Model::REFERENCE_TYPE) {
    ...
    public static  function updateRelationSequenceAndPresence($relatedInfoList, $sourceModuleTabId) {
        $db = PearDatabase::getInstance();
        $query = 'UPDATE ncrm_relatedlists SET sequence=CASE ';
        $relation_ids = array();
