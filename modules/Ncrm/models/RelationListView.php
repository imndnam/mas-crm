...
class Ncrm_RelationListView_Model extends Ncrm_Base_Model {
	...
	public function getSelectRelationLinks() {
		...
		foreach($selectLinkList as $selectLink) {
			$selectLinkModel[] = Ncrm_Link_Model::getInstanceFromValues($selectLink);
		}
		return $selectLinkModel;
	}
	...
	public function getAddRelationLinks() {
		...
		foreach($addLinkList as $addLink) {
			$addLinkModel[] = Ncrm_Link_Model::getInstanceFromValues($addLink);
		}
		return $addLinkModel;
	}
	...
	public function getEntries($pagingModel) {
		...
		if($orderBy) {
            ...
            if($orderByFieldModuleModel && $orderByFieldModuleModel->isReferenceField()) {
                ...
                $qualifiedOrderBy = 'ncrm_crmentity'.$orderByFieldModuleModel->get('column');
                $selectAndFromClause .= ' LEFT JOIN ncrm_crmentity AS '.$qualifiedOrderBy.' ON '.
                ...
                $query .= ' ORDER BY '.$qualifiedOrderBy.'.label '.$sortOrder;
				 ...
				 $query .= ' ORDER BY COALESCE(CONCAT(ncrm_users.first_name,ncrm_users.last_name),ncrm_groups.groupname) '.$sortOrder;
			} else{
	...
	public function getRelationQuery() {
		...
		$query = $queryComponents[0].' ,ncrm_crmentity.crmid FROM '.$queryComponents[1];
	...
	public static function getInstance($parentRecordModel, $relationModuleName, $label=false) {
		...
		$className = Ncrm_Loader::getComponentClassName('Model', 'RelationListView', $parentModuleName);
		$instance = new $className();

		$parentModuleModel = $parentRecordModel->getModule();
		$relatedModuleModel = Ncrm_Module_Model::getInstance($relationModuleName);
		$instance->setRelatedModuleModel($relatedModuleModel);
		
		$relationModel = Ncrm_Relation_Model::getInstance($parentModuleModel, $relatedModuleModel, $label);
		...
		if(!$relationModel){
			...
			foreach ($referenceFieldOfParentModule as $fieldName=>$fieldModel) {
				...
				if(in_array($relatedModuleName, $refredModulesOfReferenceField)){
					$relationModelClassName = Ncrm_Loader::getComponentClassName('Model', 'Relation', $parentModuleModel->getName());
	...
	public function getRelatedEntriesCount() {
		...
		if ($position) {
			...
			$relationQuery = 'SELECT COUNT(DISTINCT ncrm_crmentity.crmid) AS count'; 
    ...
    public function getCurrencySymbol($recordId, $fieldModel) {
        ...
        if(($fieldName == 'currency_id') && ($moduleName == 'Products' || $moduleName == 'Services')) {
            $query = "SELECT currency_symbol FROM ncrm_currency_info WHERE id = ("; 
            if($moduleName == 'Products') 
                $query .= "SELECT currency_id FROM ncrm_products WHERE productid = ?)"; 
            else if($moduleName == 'Services')
                $query .= "SELECT currency_id FROM ncrm_service WHERE serviceid = ?)"; 

            $result = $db->pquery($query, array($recordId)); 
            return $db->query_result($result, 0, 'currency_symbol');    
        } else if(($tableName == 'ncrm_invoice' || $tableName == 'ncrm_quotes' || $tableName == 'ncrm_purchaseorder' || $tableName == 'ncrm_salesorder') &&
            ...
            $query = "SELECT currency_symbol FROM ncrm_currency_info WHERE id = ( SELECT currency_id FROM ".$tableName." WHERE ".$focus->table_index." = ? )";
            $result = $db->pquery($query, array($recordId)); 
            return $db->query_result($result, 0, 'currency_symbol');
        } else {
