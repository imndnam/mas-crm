...
class Ncrm_MiniList_Model extends Ncrm_Widget_Model {
	...
	public function getTargetModuleModel() {
		if (!$this->targetModuleModel) {
			$this->targetModuleModel = Ncrm_Module_Model::getInstance($this->getTargetModule());
		}
	...
	public function getTitle($prefix='') {
		...
		$suffix = '';
		$customviewrs = $db->pquery('SELECT viewname FROM ncrm_customview WHERE cvid=?', array($this->widgetModel->get('filterid')));
	...
	public function getHeaders() {
		...
		if (!$this->listviewHeaders) {
			...
			foreach ($this->listviewController->getListViewHeaderFields() as $fieldName => $webserviceField) {
				$fieldObj = Ncrm_Field::getInstance($webserviceField->getFieldId());
				$headerFieldModels[$fieldName] = Ncrm_Field_Model::getInstanceFromFieldObject($fieldObj);
			}
	...
	public function getRecords() {
		...
		if (!$this->listviewRecords) {
			...
			$query .= ' ORDER BY ncrm_crmentity.modifiedtime DESC ';
			$query .= ' LIMIT 0,' . $this->getRecordLimit();
			$query = str_replace(" FROM ", ",ncrm_crmentity.crmid as id FROM ", $query);
