 ...
 * All Rights Reserved.
 *************************************************************************************/
require_once 'vtlib/Ncrm/Block.php';

class Ncrm_Block_Model extends Ncrm_Block {

	public $fields = false;

	public function getFields() {
		if(empty($this->fields)) {
			$moduleFields = Ncrm_Field_Model::getAllForModule($this->module);
            $this->fields = array();
    ...
    public function __update() {
        ...
        $query = 'UPDATE ncrm_blocks SET blocklabel=?,display_status=? WHERE blockid=?';
	 ...
	 * @return <array> - list of Ncrm_Block_Model
	 */
	public static function getAllForModule($moduleModel) {
		$blockObjects = Ncrm_Cache::get('ModuleBlock',$moduleModel->getName());
        
        if(!$blockObjects){
            $blockObjects = parent::getAllForModule($moduleModel);
            Ncrm_Cache::set('ModuleBlock',$moduleModel->getName(),$blockObjects);
        }
	...
	/**
	 * Function to retrieve block instance from Ncrm_Block object
	 * @param Ncrm_Block $blockObject - vtlib block object
	 * @return Ncrm_Block_Model
	 */
	public static function getInstanceFromBlockObject(Ncrm_Block $blockObject) {
		$objectProperties = get_object_vars($blockObject);
		$blockClassName = Ncrm_Loader::getComponentClassName('Model', 'Block', $blockObject->module->name);
    ...
    public static function updateSequenceNumber($sequenceList) {
        $db = PearDatabase::getInstance();
        $query = 'UPDATE ncrm_blocks SET sequence = CASE blockid ';
    ...
    public static function checkFieldsExists($blockId) {
        $db = PearDatabase::getInstance();
        $query = 'SELECT 1 FROM ncrm_field WHERE block=?';
	...
	public static function pushDown($fromSequence, $sourceModuleTabId) {
		$db = PearDatabase::getInstance();
		$query = 'UPDATE ncrm_blocks SET sequence=sequence+1 WHERE sequence > ? and tabid=?';
		$result = $db->pquery($query, array($fromSequence,$sourceModuleTabId));
	}
    
    public static function getAllBlockSequenceList($moduleTabId) {
        $db = PearDatabase::getInstance();
        $query = 'SELECT blockid,sequence FROM ncrm_blocks where tabid=?';
	...
	public static function checkDuplicate($blockLabel, $tabId) {
		...
		$result = $db->pquery('SELECT 1 FROM ncrm_blocks WHERE blocklabel = ? AND tabid = ?', array($blockLabel, $tabId));
