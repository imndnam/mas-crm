...
/**
 * Ncrm Entity Record Model Class
 */
class Ncrm_Record_Model extends Ncrm_Base_Model {
	...
	public function getName() {
		...
		return Ncrm_Util_Helper::toSafeHTML(decode_html($displayName));
	}
	 ...
	 * @return Ncrm_Record_Model instance
	 */
	...
	/**
	 * Function returns the Ncrm_Field_Model
	...
	/**
	 * Static Function to get the instance of a clean Ncrm Record Model for the given module name
	 * @param <String> $moduleName
	 * @return Ncrm_Record_Model or Module Specific Record Model instance
	 */
	...
	/**
	 * Static Function to get the instance of the Ncrm Record Model given the recordid and the module name
	...
	public static function getInstanceById($recordId, $module=null) {
		...
		if(is_object($module) && is_a($module, 'Ncrm_Module_Model')) {
			...
			$module = Ncrm_Module_Model::getInstance($moduleName);
		}
	...
	public function deleteImage($imageId) {
		...
		$checkResult = $db->pquery('SELECT crmid FROM ncrm_seattachmentsrel WHERE attachmentsid = ?', array($imageId));
		$crmId = $db->query_result($checkResult, 0, 'crmid');

		if ($this->getId() === $crmId) {
			$db->pquery('DELETE FROM ncrm_attachments WHERE attachmentsid = ?', array($imageId));
			$db->pquery('DELETE FROM ncrm_seattachmentsrel WHERE attachmentsid = ?', array($imageId));
			return true;
		}
		return false;
	}
	...
	public function getDescriptionValue() {
		...
		if(empty($description)) {
			$db = PearDatabase::getInstance();
			$result = $db->pquery("SELECT description FROM ncrm_crmentity WHERE crmid = ?", array($this->getId()));
