...
/**
 * Ncrm Module Model Class
 */
class Ncrm_Module_Model extends Ncrm_Module {
	...
	public function isCommentEnabled() {
		...
		$commentsModuleModel = Ncrm_Module_Model::getInstance('ModComments');
		if($commentsModuleModel && $commentsModuleModel->isActive()) {
			$relatedToFieldResult = $db->pquery('SELECT fieldid FROM ncrm_field WHERE fieldname = ? AND tabid = ?',
					array('related_to', $commentsModuleModel->getId()));
			$fieldId = $db->query_result($relatedToFieldResult, 0, 'fieldid');
			if(!empty($fieldId)) {
				$relatedModuleResult = $db->pquery('SELECT relmodule FROM ncrm_fieldmodulerel WHERE fieldid = ?', array($fieldId));
	...
	public function getModuleMeta($userModel = false) {
		if(empty($this->moduleMeta)){
			...
			$this->moduleMeta = Ncrm_ModuleMeta_Model::getInstance($this->get('name'), $userModel);
		}
    ...
    public function getAllFilterCvidForModule() {
		...
		$result = $db->pquery("SELECT cvid FROM ncrm_customview WHERE viewname = 'All' AND entitytype = ?",
					array($this->getName()));
	...
	/**
	 * Function to get a Ncrm Record Model instance from an array of key-value mapping
	...
	public function getRecordFromArray($valueArray, $rawData=false) {
		$modelClassName = Ncrm_Loader::getComponentClassName('Model', 'Record', $this->get('name'));
	 ...
	 * @return <Array of Ncrm_Block_Model> - list of block models
	 */
	public function getBlocks() {
		if(empty($this->blocks)) {
			$blocksList = array();
			$moduleBlocks = Ncrm_Block_Model::getAllForModule($this);
	...
	public function getFields($blockInstance=false) {
		if(empty($this->fields)){
			$moduleBlockFields = Ncrm_Field_Model::getAllForModule($this);
            $this->fields = array();
	...
	/**
	 * Function gives fields based on the type
	 * @return <Ncrm_Field_Model> with field label as key
	 */
	 ...
	 * @return <Ncrm_Field_Model> with field id as key
	 */
	 ...
	 * @return <Array of Ncrm_Relation_Model>
	 */
	public function getRelations() {
		if(empty($this->relations)) {
			return Ncrm_Relation_Model::getAllRelations($this);
		}
	...
	public function getField($fieldName){
		return Ncrm_Field_Model::getInstance($fieldName,$this);
	}
	...
	public function getRecentRecords($limit=10) {
		...
		$query = 'SELECT * FROM ncrm_crmentity '.$nonAdminQuery.' WHERE setype=? AND '.$deletedCondition.' AND modifiedby = ? ORDER BY modifiedtime DESC LIMIT ?';
    ...
    public function getDeletedRecordCondition() {
       return 'ncrm_crmentity.deleted = 0';
    }
	...
	public function hasSequenceNumberField() {
        if(!empty($this->fields)) {
            ...
            $db = PearDatabase::getInstance();
            $query = 'SELECT 1 FROM ncrm_field WHERE uitype=4 and tabid=?';
	...
	public static function getInstance($value) {
        $instance = Ncrm_Cache::get('module',$value);
        if(!$instance){
            $instance = false;
            $moduleObject = parent::getInstance($value);
            if($moduleObject) {
                $instance = self::getInstanceFromModuleObject($moduleObject);
                Ncrm_Cache::set('module',$value,$instance);
				if (is_string($value)) {
					Ncrm_Cache::set('module', $moduleObject->name, $instance);
				} else if (is_int($value)) {
					Ncrm_Cache::set('module', $moduleObject->id, $instance);
				}
            }
        }
		return $instance;
	...
	public static function getInstanceFromModuleObject(Ncrm_Module $moduleObj){
		...
		$modelClassName = Ncrm_Loader::getComponentClassName('Model', 'Module', $objectProperties['name']);
	...
	/**
	 * Function to get the instance of Ncrm Module Model from a given list of key-value mapping
	 * @param <Array> $valueArray
	 * @return Ncrm_Module_Model instance
	 */
	public static function getInstanceFromArray($valueArray) {
		$modelClassName = Ncrm_Loader::getComponentClassName('Model', 'Module', $valueArray['name']);
	 ...
	 * @return <array> List of module models <Ncrm_Module_Model>
	 */
	public static function getAll($presence = array(), $restrictedModulesList = array()) {
		$db = PearDatabase::getInstance();
		self::preModuleInitialize2();
        $moduleModels = Ncrm_Cache::get('ncrm', 'modules');


        if(!$moduleModels){
            $moduleModels = array();

            $query = 'SELECT * FROM ncrm_tab';
            $params = array();
            if($presence) {
            ...
            for($i=0; $i<$noOfModules; ++$i) {
                ...
                Ncrm_Cache::set('module',$row['tabid'], $moduleModels[$row['tabid']]);
                Ncrm_Cache::set('module',$row['name'], $moduleModels[$row['tabid']]);
            }
            if(!$presence){
                Ncrm_Cache::set('ncrm', 'modules',$moduleModels);
            }
        }
	...
	public static function getEntityModules() {
		self::preModuleInitialize2();
		$moduleModels = Ncrm_Cache::get('ncrm','EntityModules');
        if(!$moduleModels){
            $presence = array(0, 2);
            ...
            Ncrm_Cache::set('ncrm','EntityModules',$moduleModels);
        }
		return $moduleModels;
	}
	...
	public static function getQuickCreateModules() {
		...
		$sql = 'SELECT DISTINCT ncrm_tab.*
					FROM ncrm_field
					INNER JOIN ncrm_tab ON ncrm_tab.tabid = ncrm_field.tabid
					WHERE quickcreate=0 AND ncrm_tab.presence != 1';
		$params = array();
	 ...
	 * @return <Array> - List of Ncrm_Module_Model instances
	 */
	...
	protected static function preModuleInitialize2() {
		if(!Ncrm_Cache::get('EntityField','all')){
            $db = PearDatabase::getInstance();
            // Initialize meta information - to speed up instance creation (Ncrm_ModuleBasic::initialize2)
            $result = $db->pquery('SELECT modulename,tablename,entityidfield,fieldname FROM ncrm_entityname', array());
            ...
            for($index = 0, $len = $db->num_rows($result); $index < $len; ++$index) {
                ...
                Ncrm_Cache::set('EntityField',$modulename,$entiyObj);
                Ncrm_Cache::set('EntityField','all',true);
            }
        }
	}
	...
	public static function getCleanInstance($moduleName){
		$modelClassName = Ncrm_Loader::getComponentClassName('Model', 'Module', $moduleName);
		$instance = new $modelClassName();
		return $instance;
	}
	...
	public function getDefaultCustomFilter() {
		...
		$result = $db->pquery("SELECT cvid FROM ncrm_customview WHERE setdefault = 1 AND entitytype = ?",
					array($this->getName()));
		if ($db->num_rows($result)) {
			return $db->query_result($result, 0, 'cvid');
		}
	...
	public function getComments($pagingModel) {
		...
		$result = $db->pquery('SELECT ncrm_crmentity.*, ncrm_modcomments.* FROM ncrm_modcomments
						...
						INNER JOIN ncrm_crmentity crmentity2 ON ncrm_modcomments.related_to = crmentity2.crmid
						 ...
						 '.$nonAdminAccessQuery.'
						ORDER BY ncrm_crmentity.createdtime DESC LIMIT ?, ?',
	...
	public function getHistory($pagingModel, $type=false) {
		...
		$result = $db->pquery('SELECT ncrm_modtracker_basic.*
								...
								ORDER BY ncrm_modtracker_basic.id DESC LIMIT ?, ?',
								array($this->getName(), $pagingModel->getStartIndex(), $pagingModel->getPageLimit()));
	...
	function getCalendarActivities($mode, $pagingModel, $user, $recordId = false) {
		...
		$query = "SELECT ncrm_crmentity.crmid, crmentity2.crmid AS parent_id, ncrm_crmentity.smownerid, ncrm_crmentity.setype, ncrm_activity.* FROM ncrm_activity
					...
					INNER JOIN ncrm_crmentity AS crmentity2 ON ncrm_seactivityrel.crmid = crmentity2.crmid AND crmentity2.deleted = 0 AND crmentity2.setype = ?
		...
		if ($recordId) {
			$query .= " AND ncrm_seactivityrel.crmid = ?";
	...
	public function getSpecificRelationQuery($relatedModule) {
		if($relatedModule == 'Documents'){
			return ' AND ncrm_notes.filestatus = 1 ';
		}
		return;
	}
	...
	public function getSettingLinks(){
		...
		if($this->hasSequenceNumberField()) {
			$settingsLinks[] = array(
				...
				'linkurl' => 'index.php?parent=Settings&module=Ncrm&view=CustomRecordNumbering&sourceModule='.$this->getName(),
				'linkicon' => ''
			);
		}
	 ...
	 * @return <Array> list of field models <Ncrm_Field_Model>
	 */
	...
	public function getSearchRecordsQuery($searchValue, $parentId=false, $parentModule=false) {
		return "SELECT * FROM ncrm_crmentity WHERE label LIKE '%$searchValue%' AND ncrm_crmentity.deleted = 0";
	}
	...
	public function searchRecord($searchValue, $parentId=false, $parentModule=false, $relatedModule=false) {
		if(!empty($searchValue) && empty($parentId) && empty($parentModule)) {
			$matchingRecords = Ncrm_Record_Model::getSearchResult($searchValue, $this->getName());
	...
	public function getRelationQuery($recordId, $functionName, $relatedModule) {
        ...
        if(count($relatedListFields) > 0) {
			...
			$selectColumnSql = 'SELECT DISTINCT ncrm_crmentity.crmid,'.$selectColumnSql;
	...
	public function vtJsonDependentModules() {
		vimport('~~/modules/com_ncrm_workflow/WorkflowComponents.php');
		$db = PearDatabase::getInstance();
	...
	public function getRelatedModuleRecordIds(Ncrm_Request $request, $recordIds = array()) {
		$db = PearDatabase::getInstance();
		...
		if(!empty($relatedModules)) {
			for ($i=0; $i<count($relatedModules); $i++) {
				...
				$sql = "SELECT ncrm_crmentity.crmid FROM ncrm_crmentity";
				if($tablename == 'ncrm_crmentityrel'){
					$sql .= " INNER JOIN $tablename ON ($tablename.relcrmid = ncrm_crmentity.crmid OR $tablename.crmid = ncrm_crmentity.crmid)
						WHERE ($tablename.crmid IN (".  generateQuestionMarks($recordIds).") OR $tablename.relcrmid IN (".  generateQuestionMarks($recordIds)."))";
					foreach ($recordIds as $key => $recordId) {
						array_push($params, $recordId);
					}
				} else {
					$sql .= " INNER JOIN $tablename ON $tablename.$tabIndex = ncrm_crmentity.crmid
						WHERE $tablename.$relIndex IN (".  generateQuestionMarks($recordIds).")";
				}

				$sql .=' AND ncrm_crmentity.deleted = 0';
				foreach ($recordIds as $key => $recordId) {
					array_push($params, $recordId);
				}
	...
	public function transferRecordsOwnership($transferOwnerId, $relatedModuleRecordIds){
		$db = PearDatabase::getInstance();
		$query = 'UPDATE ncrm_crmentity SET smownerid = ? WHERE crmid IN ('.  generateQuestionMarks($relatedModuleRecordIds).')';
