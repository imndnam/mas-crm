...
class Ncrm_ExportData_Action extends Ncrm_Mass_Action {
	 ...
	 * @param Ncrm_Request $request
	 */
	function ExportData(Ncrm_Request $request) {
		$db = PearDatabase::getInstance();
		$moduleName = $request->get('source_module');

		$this->moduleInstance = Ncrm_Module_Model::getInstance($moduleName);
	 ...
	 */
	function getExportQuery(Ncrm_Request $request) {
		...
		switch($mode) {
			...
			case 'ExportCurrentPage' :	$pagingModel = new Ncrm_Paging_Model();
	 ...
	 * @return <String>
	 */
	function getExportContentType(Ncrm_Request $request) {
	...
	function sanitizeValues($arr){
		...
		if(empty ($this->fieldArray)){
			...
			foreach($this->fieldArray as $fieldName => $fieldObj){
				...
				if($fieldObj->get('table') == 'ncrm_inventoryproductrel' && ($fieldName == 'discount_amount' || $fieldName == 'discount_percent')){
		...
		foreach($arr as $fieldName=>&$value){
			...
			if($fieldname != 'hdnTaxType' && ($uitype == 15 || $uitype == 16 || $uitype == 33)){
				...
				$value = Ncrm_Util_Helper::getOwnerName($value);
