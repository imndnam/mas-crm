 ...
 * All Rights Reserved.
 *************************************************************************************/
jQuery.Class("Ncrm_Base_Validator_Js",{
	...
	invokeValidation: function(field, rules, i, options){
		...
		var listOfValidators = Ncrm_Base_Validator_Js.getValidator(field);
	...
	getValidator: function(field){
		...
		if(Ncrm_Base_Validator_Js.moduleName == false && moduleEle.length > 0) {
			Ncrm_Base_Validator_Js.moduleName = moduleEle.val();
		}
		
		var fieldInstance = Ncrm_Field_Js.getInstance(fieldInfo);
		var validatorsOfType = Ncrm_Base_Validator_Js.getValidatorsFromFieldType(fieldInstance);
		...
		if(fieldData.hasOwnProperty(dataValidator)){
			...
			for(var key in specialValidators){
				...
				var validatorOfNames = Ncrm_Base_Validator_Js.getValidatorClassName(specialValidator.name);
				if(validatorOfNames != ""){
	...
	getValidatorsFromFieldType: function(fieldInstance){
		...
		validatorsOfType.push(Ncrm_Base_Validator_Js.getValidatorClassName(fieldType));
        return validatorsOfType;
	},
	
	getValidatorClassName: function(validatorName){
		var validatorsOfType = '';
		var className = Ncrm_Base_Validator_Js.getClassName(validatorName);
		var fallBackClassName = Ncrm_Base_Validator_Js.getFallBackClassName(validatorName);
	...
	getClassName: function(validatorName){
		if(Ncrm_Base_Validator_Js.moduleName != false) {
			var moduleName = Ncrm_Base_Validator_Js.moduleName;
		} else {
	...
	getFallBackClassName: function(validatorName){
		return "Ncrm_"+validatorName+"_Validator_Js";
	}
},{
	field: "",
	error: "",
