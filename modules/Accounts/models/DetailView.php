...
class Accounts_DetailView_Model extends Ncrm_DetailView_Model {
	...
	public function getDetailViewLinks($linkParams) {
		...
		if($currentUserModel->hasModulePermission($emailModuleModel->getId())) {
			$basicActionLink = array(
				...
				'linkurl' => 'javascript:Ncrm_Detail_Js.triggerSendEmail("index.php?module='.$this->getModule()->getName().
								'&view=MassActionAjax&mode=showComposeEmailForm&step=step1","Emails");',
				'linkicon' => ''
			);
			$linkModelList['DETAILVIEWBASIC'][] = Ncrm_Link_Model::getInstanceFromValues($basicActionLink);
		}
		...
		$SMSNotifierModuleModel = Ncrm_Module_Model::getInstance('SMSNotifier');
		if(!empty($SMSNotifierModuleModel) && $currentUserModel->hasModulePermission($SMSNotifierModuleModel->getId())) {
			$basicActionLink = array(
				'linktype' => 'DETAILVIEWBASIC',
				'linklabel' => 'LBL_SEND_SMS',
				'linkurl' => 'javascript:Ncrm_Detail_Js.triggerSendSms("index.php?module='.$this->getModule()->getName().
		...
		if($currentUserModel->hasModuleActionPermission($moduleModel->getId(), 'EditView')) {
			$massActionLink = array(
				...
				'linkurl' => 'javascript:Ncrm_Detail_Js.triggerTransferOwnership("index.php?module='.$moduleModel->getName().'&view=MassActionAjax&mode=transferOwnership")',
				'linkicon' => ''
			);
			$linkModelList['DETAILVIEW'][] = Ncrm_Link_Model::getInstanceFromValues($massActionLink);
		}

        foreach($CalendarActionLinks as $basicLink) {
			$linkModelList['DETAILVIEW'][] = Ncrm_Link_Model::getInstanceFromValues($basicLink);
		}
