...
class Accounts_Record_Model extends Ncrm_Record_Model {
	...
	function getAccountHierarchy() {
		...
		foreach($hierarchy['entries'] as $accountId => $accountInfo) {
			...
			if($matches != null) {
				...
				$recordModel = Ncrm_Record_Model::getCleanInstance('Accounts');
	...
	public function checkDuplicate() {
		...
		$query = "SELECT 1 FROM ncrm_crmentity WHERE setype = ? AND label = ? AND deleted = 0";
