...
class Install_Utils_Model {
	...
	/**
	 * Function returns the php.ini file settings required for installing ncrmCRM
	 * @return <Array>
	 */
	...
	/**
	 * Variable has the recommended php settings for smooth running of ncrmCRM
	 * @var <Array>
	 */
	...
	/**
	 * Returns the recommended php settings for ncrmCRM
	 * @return type
	 */
	...
	/**
	 * Function checks for ncrmCRM installation prerequisites
	 * @return <Array>
	 */
		...
		if($db_type) {
			...
			if(@$conn->Connect($db_hostname,$db_username,$db_password)) {
				...
				if(@$conn->Connect($db_hostname, $db_username, $db_password, $db_name)) {
					...
					if(!$db_utf8_support) {
						$db_utf8_support = Ncrm_Util_Helper::checkDbUTF8Support($conn);
					}
				}
				$conn->Close();
			}
	...
	public static function installModules() {
		...
		$moduleFolders = array('packages/ncrm/mandatory', 'packages/ncrm/optional');
