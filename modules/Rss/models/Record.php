...
class Rss_Record_Model extends Ncrm_Record_Model {
	...
	public function save($url) {
        ...
        $sql = "INSERT INTO ncrm_rss (rssid,rssurl,rsstitle) values (?,?,?)";
    ...
    public function delete() {
        ...
        $sql = 'DELETE FROM ncrm_rss where rssid = ?';
    ...
    public function makeDefault() {
        ...
        $sql = 'UPDATE ncrm_rss set starred = 0';
        $db->pquery($sql,array());
        
        $sql = 'UPDATE ncrm_rss set starred = 1 where rssid = ?';
        $db->pquery($sql,array($recordId));
    }
	...
	static public function getInstanceById($recordId, $qualifiedModuleName) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM ncrm_rss WHERE rssid = ?', array($recordId));
    ...
    public function getDefaultRss() {
        ...
        $result = $db->pquery('SELECT rssid FROM ncrm_rss where starred = 1', array());
        $recordId = $db->query_result($result,'0','rssid');
        if($recordId) {
            $this->setId($recordId);
        } else {
            $result = $db->pquery('SELECT rssid FROM ncrm_rss', array());
            $recordId = $db->query_result($result,'0','rssid');
            $this->setId($recordId);
        }
    }
