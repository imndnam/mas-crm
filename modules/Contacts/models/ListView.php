...
class Contacts_ListView_Model extends Ncrm_ListView_Model {
	...
	public function getListViewMassActions($linkParams) {
		...
		if($SMSNotifierModuleModel && $currentUserModel->hasModulePermission($SMSNotifierModuleModel->getId())) {
			$massActionLink = array(
				...
				'linkurl' => 'javascript:Ncrm_List_Js.triggerSendSms("index.php?module='.$this->getModule()->getName().'&view=MassActionAjax&mode=showSendSMSForm","SMSNotifier");',
				'linkicon' => ''
			);
			$massActionLinks['LISTVIEWMASSACTION'][] = Ncrm_Link_Model::getInstanceFromValues($massActionLink);
		}
		...
		if($currentUserModel->hasModuleActionPermission($moduleModel->getId(), 'EditView')) {
			$massActionLink = array(
				...
				'linkurl' => 'javascript:Ncrm_List_Js.triggerTransferOwnership("index.php?module='.$moduleModel->getName().'&view=MassActionAjax&mode=transferOwnership")',
				'linkicon' => ''
			);
			$massActionLinks['LISTVIEWMASSACTION'][] = Ncrm_Link_Model::getInstanceFromValues($massActionLink);
		}
