...
$query = 'UPDATE ncrm_currencies_seq SET id = (SELECT currencyid FROM ncrm_currencies ORDER BY currencyid DESC LIMIT 1)';
$adb->pquery($query, array());

$uniqId = $adb->getUniqueID('ncrm_currencies'); 
$result = $adb->pquery('SELECT 1 FROM ncrm_currencies WHERE currency_name = ?',array('CFP Franc')); 
 
if($adb->num_rows($result)<=0){ 
Migration_Index_View::ExecuteQuery('INSERT INTO ncrm_currencies VALUES (?,?,?,?)', array($uniqId, 'CFP Franc', 'XPF', 'F')); 
} 

//Adding new timezone (GMT+11:00) New Caledonia 
    $sortOrderResult = $adb->pquery("SELECT sortorderid FROM ncrm_time_zone WHERE time_zone = ?", array('Asia/Yakutsk'));
    if ($adb->num_rows($sortOrderResult)) {
        $sortOrderId = $adb->query_result($sortOrderResult, 0, 'sortorderid');
        $adb->pquery("UPDATE ncrm_time_zone SET sortorderid = (sortorderid + 1) WHERE sortorderid > ?", array($sortOrderId));
        Migration_Index_View::ExecuteQuery('INSERT INTO ncrm_time_zone (time_zone, sortorderid, presence) VALUES (?, ?, ?)', array('Etc/GMT-11', ($sortOrderId + 1), 1));
