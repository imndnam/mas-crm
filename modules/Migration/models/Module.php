...
class Migration_Module_Model extends Ncrm_Module_Model {
	
	public function getDBVersion(){
		$db = PearDatabase::getInstance();
		
		$result = $db->pquery('SELECT current_version FROM ncrm_version', array());
	...
	public function getLatestSourceVersion(){
		return vglobal('ncrm_current_version');
	}
	
	/**
	 * Function to update the latest ncrm version in db
	 * @return type
	 */
	public function updateNcrmVersion(){
		$db = PearDatabase::getInstance();
		$db->pquery('UPDATE ncrm_version SET current_version=?,old_version=?', array($this->getLatestSourceVersion(), $this->getDBVersion()));
		return true;
	}
