...
class Migration_Index_View extends Ncrm_Basic_View {
	...
	protected function step1(Ncrm_Request $request) {
	...
	protected function step2(Ncrm_Request $request){
		$moduleName = $request->getModule();
		$viewer = $this->getViewer($request);
	...
	public function applyDBChanges(){
		...
		//update ncrm version in db
		$migrationModuleModel->updateNcrmVersion();
	...
	public static function insertAdvFilter($queryid, $filters) {
		if ($queryid != "") {
			...
			self::ExecuteQuery('INSERT INTO ncrm_relcriteria_grouping VALUES(?,?,?,?)', array(1, $queryid, '', $conditionExpression));
		}
	}
