...
function getUserFldArray($fld_module,$roleid){
	...
	$query="select ncrm_field.fieldlabel,ncrm_field.columnname,ncrm_field.fieldname, ncrm_field.uitype" .
			" FROM ncrm_field inner join ncrm_picklist on ncrm_field.fieldname = ncrm_picklist.name" .
			" where (displaytype=1 and ncrm_field.tabid=? and ncrm_field.uitype in ('15','55','33','16') " .
			" or (ncrm_field.tabid=? and fieldname='salutationtype' and fieldname !='vendortype')) " .
...
function getPickListModules(){
	...
	$query = 'select distinct ncrm_field.fieldname,ncrm_field.tabid,ncrm_tab.tablabel, ncrm_tab.name as tabname,uitype from ncrm_field inner join ncrm_tab on ncrm_tab.tabid=ncrm_field.tabid where uitype IN (15,33) and ncrm_field.tabid != 29 and ncrm_tab.presence != 1 and ncrm_field.presence in (0,2) order by ncrm_field.tabid ASC';
	// END
...
function getrole2picklist(){
	global $adb;
	$query = "select rolename,roleid from ncrm_role where roleid not in('H1') order by roleid";
	$result = $adb->pquery($query, array());
	while($row = $adb->fetch_array($result)){
...
function getAllPickListValues($fieldName,$lang = Array() ){
	global $adb;
	$sql = 'SELECT * FROM ncrm_'.$adb->sql_escape_string($fieldName);
...
function getEditablePicklistValues($fieldName, $lang= array(), $adb){
	...
	$sql="select $fieldName from ncrm_$fieldName where presence=1 and $fieldName <> '--None--'";
	$res = $adb->query($sql);
...
function getNonEditablePicklistValues($fieldName, $lang=array(), $adb){
	$values = array();
	$fieldName = $adb->sql_escape_string($fieldName);
	$sql = "select $fieldName from ncrm_$fieldName where presence=0";
	$result = $adb->query($sql);
	$count = $adb->num_rows($result);
	for($i=0;$i<$count;$i++){
...
function getAssignedPicklistValues($tableName, $roleid, $adb, $lang=array()){
	...
	if($adb->num_rows($result)){
		...
		$sql = "SELECT distinct ".$adb->sql_escape_string($tableName)." FROM ". $adb->sql_escape_string("ncrm_$tableName")
				. " inner join ncrm_role2picklist on ".$adb->sql_escape_string("ncrm_$tableName").".picklist_valueid=ncrm_role2picklist.picklistvalueid"
