 ...
 */
class ModComments_Record_Model extends Ncrm_Record_Model {
	...
	public function getImagePath() {
		...
		if($commentor) {
			...
			if (!empty($customer) && $isMailConverterType != 1) {
                $recordModel = Ncrm_Record_Model::getInstanceById($customer);
	...
	public static function getInstanceById($record) {
		...
		$result = $db->pquery('SELECT ncrm_modcomments.*, ncrm_crmentity.smownerid,
					ncrm_crmentity.createdtime, ncrm_crmentity.modifiedtime FROM ncrm_modcomments
	...
	public function getParentRecordModel() {
		...
		return Ncrm_Record_Model::getInstanceById($parentRecordId);
	...
	public function getCommentedByModel() {
		$customer = $this->get('customer');
		if(!empty($customer)) {
			return Ncrm_Record_Model::getInstanceById($customer, 'Contacts');
		} else {
			$commentedBy = $this->get('smownerid');
			if($commentedBy) {
			    $commentedByModel = Ncrm_Record_Model::getInstanceById($commentedBy, 'Users');
			    if(empty($commentedByModel->entity->column_fields['user_name'])) {
				$activeAdmin = Users::getActiveAdminUser();
				$commentedByModel = Ncrm_Record_Model::getInstanceById($activeAdmin->id, 'Users');
			    }
	 ...
	 * @param <Ncrm_Paging_Model> - paging model
	...
	public static function getRecentComments($parentRecordId, $pagingModel){
		...
		$listView = Ncrm_ListView_Model::getInstance('ModComments');
		...
		$query = $query ." AND related_to = ? ORDER BY ncrm_crmentity.createdtime DESC
	...
	public static function getAllParentComments($parentId) {
		...
		$query = $query ." AND related_to = ? AND parent_comments = '' ORDER BY ncrm_crmentity.createdtime DESC";
	...
	public function getChildCommentsCount() {
		...
		$query = 'SELECT 1 FROM ncrm_modcomments WHERE parent_comments = ? AND related_to = ?';
