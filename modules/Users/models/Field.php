 ...
 */
class Users_Field_Model extends Ncrm_Field_Model {
	...
	public function getPicklistValues() {
		if($this->get('uitype') == 32) {
			return Ncrm_Language_Handler::getAllLanguages();
		}
        else if ($this->get('uitype') == '115') {
            $db = PearDatabase::getInstance();
            
            $query = 'SELECT '.$this->getFieldName().' FROM ncrm_'.$this->getFieldName();
    ...
    public function getDisplayValue($value, $recordId = false, $recordInstance = false) {
		 ...
		 if($this->get('uitype') == 32){
			return Ncrm_Language_Handler::getLanguageLabel($value);
		 }
