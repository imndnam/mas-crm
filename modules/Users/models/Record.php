...
class Users_Record_Model extends Ncrm_Record_Model {
	...
	public function getImageDetails() {
		...
		if ($recordId) {
			$query = "SELECT ncrm_attachments.* FROM ncrm_attachments
            LEFT JOIN ncrm_salesmanattachmentsrel ON ncrm_salesmanattachmentsrel.attachmentsid = ncrm_attachments.attachmentsid
            WHERE ncrm_salesmanattachmentsrel.smid=?";
	...
	public function getAccessibleUsers($private="",$module = false) {
		...
		$accessibleUser = Ncrm_Cache::get('ncrm-'.$this->getRole().'-'.$currentUserRoleModel->get('allowassignedrecordsto'), 'accessibleusers');
        if(empty($accessibleUser)) {
			...
			Ncrm_Cache::set('ncrm-'.$this->getRole().'-'.$currentUserRoleModel->get('allowassignedrecordsto'), 'accessibleusers',$accessibleUser);
		}
	...
	public function getAllUsersOnRoles($roles) {
		...
		$sql = 'SELECT userid FROM ncrm_user2role WHERE roleid IN ('.  generateQuestionMarks($roleIds).')';
		...
		if($noOfUsers > 0) {
			...
			$query = 'SELECT id, first_name, last_name FROM ncrm_users WHERE status = ? AND id IN ('.  generateQuestionMarks($userIds).')';
	...
	public function getAccessibleGroups($private="",$module = false) {
        ...
        $accessibleGroups = Ncrm_Cache::get('ncrm-'.$private, 'accessiblegroups');
        if(!$accessibleGroups){
            $accessibleGroups = get_group_array(false, "ACTIVE", "", $private,$module);
            Ncrm_Cache::set('ncrm-'.$private, 'accessiblegroups',$accessibleGroups);
        }
	...
	public function deleteImage($imageId) {
		...
		$checkResult = $db->pquery('SELECT smid FROM ncrm_salesmanattachmentsrel WHERE attachmentsid = ?', array($imageId));
		$smId = $db->query_result($checkResult, 0, 'smid');

		if ($this->getId() === $smId) {
			$db->pquery('DELETE FROM ncrm_attachments WHERE attachmentsid = ?', array($imageId));
			$db->pquery('DELETE FROM ncrm_salesmanattachmentsrel WHERE attachmentsid = ?', array($imageId));
			return true;
		}
		return false;
	}
	...
	function saveTagCloud() {
		$db = PearDatabase::getInstance();
		$db->pquery("UPDATE ncrm_homestuff SET visible = ? WHERE userid=? AND stufftype='Tag Cloud'",
	...
	public static function getUserGroups($userId){
		$db = PearDatabase::getInstance();
		$groupIds = array();
		$query = "SELECT groupid FROM ncrm_users2group WHERE userid=?";
    ...
    public static function getCount($onlyActive = false) {
        $db = PearDatabase::getInstance();
        $query = 'SELECT 1 FROM ncrm_users ';
        $params = array();
	...
	public static function getInstanceByName($userName) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT id FROM ncrm_users WHERE user_name = ?', array($userName));
	...
	public function delete() {
		...
		$db = PearDatabase::getInstance();
		$query = 'SELECT is_owner FROM ncrm_users WHERE id = ?';
	...
	public function getActiveAdminUsers() {
		...
		$sql = 'SELECT id FROM ncrm_users WHERE status=? AND is_admin=?';
	...
	public function getUserHash() {
		$db = PearDatabase::getInstance();
		$query = 'SELECT user_hash FROM ncrm_users WHERE id = ?';
        ...
        public function deleteUserPermanently($userId, $newOwnerId) {
                ...
                $sql = "UPDATE ncrm_crmentity SET smcreatorid=?,smownerid=? WHERE smcreatorid=? AND setype=?";
                $db->pquery($sql, array($newOwnerId, $newOwnerId, $userId,'ModComments'));
                
                //update history details in ncrm_modtracker_basic 
                $sql ="update ncrm_modtracker_basic set whodid=? where whodid=?"; 
                $db->pquery($sql, array($newOwnerId, $userId)); 

                //update comments details in ncrm_modcomments 
                $sql ="update ncrm_modcomments set userid=? where userid=?"; 
                $db->pquery($sql, array($newOwnerId, $userId));

                $sql = "DELETE FROM ncrm_users WHERE id=?";
