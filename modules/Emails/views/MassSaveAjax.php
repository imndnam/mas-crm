...
class Emails_MassSaveAjax_View extends Ncrm_Footer_View {
	 ...
	 */
	public function massSave(Ncrm_Request $request) {
        ...
        if(!empty($parentEmailId) && !empty ($recordId)) {
            $parentEmailModel = Ncrm_Record_Model::getInstanceById($parentEmailId);
		...
		if ($recordModel->checkUploadSize($documentIds)) {
			...
			if($flag == 'SENT') {
				...
				if ($status === true) {
					// This is needed to set ncrm_email_track table as it is used in email reporting
