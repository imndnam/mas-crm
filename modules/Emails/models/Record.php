...
class Emails_Record_Model extends Ncrm_Record_Model {
	...
	public function getDetailViewUrl() {
		...
		return 'Javascript:Ncrm_Index_Js.showEmailPreview("'.$this->getId().'","'.$parentId.'")';
	}
	...
	public function send() {
		...
		foreach($toEmailInfo as $id => $emails) {
			...
			$moduleLanguageStrings = Ncrm_Language_Handler::getModuleStringsFromFile($currentLanguage,$parentModule);
	...
	function getFromEmailAddress() {
		...
		$fromEmail = false;
		$result = $db->pquery('SELECT from_email_field FROM ncrm_systems WHERE server_type=?', array('email'));
	...
	function getAttachmentDetails() {
		...
		$attachmentRes = $db->pquery("SELECT * FROM ncrm_attachments
						INNER JOIN ncrm_seattachmentsrel ON ncrm_attachments.attachmentsid = ncrm_seattachmentsrel.attachmentsid
						WHERE ncrm_seattachmentsrel.crmid = ?", array($this->getId()));
	...
	public function getRelatedDocuments() {
		...
		$documentRes = $db->pquery("SELECT * FROM ncrm_senotesrel
						INNER JOIN ncrm_crmentity ON ncrm_senotesrel.notesid = ncrm_crmentity.crmid AND ncrm_senotesrel.crmid = ?
						INNER JOIN ncrm_notes ON ncrm_notes.notesid = ncrm_senotesrel.notesid
						INNER JOIN ncrm_seattachmentsrel ON ncrm_seattachmentsrel.crmid = ncrm_notes.notesid
						INNER JOIN ncrm_attachments ON ncrm_attachments.attachmentsid = ncrm_seattachmentsrel.attachmentsid
						WHERE ncrm_crmentity.deleted = 0", array($this->getId()));
	...
	public function saveDocumentDetails() {
		...
		for ($i=0; $i<$count; $i++) {
			$db->pquery("INSERT INTO ncrm_senotesrel(crmid, notesid) VALUES(?, ?)", array($record, $documentIds[$i]));
		}
	}
    ...
    public function deleteDocumentLink($idList = array()){
        $db = PearDatabase::getInstance();
        $query =  'DELETE FROM ncrm_senotesrel where crmid=?';
    ...
    public function deleteAttachment($emailAttachmentDetails = array()) {
        ...
        $db->pquery('UPDATE ncrm_crmentity SET deleted=0 WHERE crmid IN('.generateQuestionMarks($attachmentIdList).')',$attachmentIdList);
        $db->pquery('DELETE FROM ncrm_attachments WHERE attachmentsid IN('.generateQuestionMarks($attachmentIdList).')',$attachmentIdList);
        $db->pquery('DELETE FROM ncrm_seattachmentsrel WHERE crmid=? and attachmentsid IN('.generateQuestionMarks($attachmentIdList).')',
	...
	public function checkUploadSize($documentIds = false) {
		...
		if (!empty ($documentIds)) {
			$count = count($documentIds);
			for ($i=0; $i<$count; $i++) {
				$documentRecordModel = Ncrm_Record_Model::getInstanceById($documentIds[$i], 'Documents');
	...
	public function updateTrackDetails($parentId) {
		...
		$db->pquery("INSERT INTO ncrm_email_access(crmid, mailid, accessdate, accesstime) VALUES(?, ?, ?, ?)", array($parentId, $recordId, date('Y-m-d'), date('Y-m-d H:i:s')));

		$result = $db->pquery("SELECT 1 FROM ncrm_email_track WHERE crmid = ? AND mailid = ?", array($parentId, $recordId));
		if ($db->num_rows($result)>0) {
			$db->pquery("UPDATE ncrm_email_track SET access_count = access_count+1 WHERE crmid = ? AND mailid = ?", array($parentId, $recordId));
		} else {
			$db->pquery("INSERT INTO ncrm_email_track(crmid, mailid, access_count) values(?, ?, ?)", array($parentId, $recordId, 1));
		}
	}
	...
	public function getAccessCountValue($parentId) {
		...
		$result = $db->pquery("SELECT access_count FROM ncrm_email_track WHERE crmid = ? AND mailid = ?", array($parentId, $this->getId()));
	...
	public function isSentMail(){
		if(!array_key_exists('email_flag', $this->getData())){
			$db = PearDatabase::getInstance();
			$query = 'SELECT email_flag FROM ncrm_emaildetails WHERE emailid=?';
	...
	function getEntityType($id) {
		...
		if (!empty($id)) {
			...
			if(!in_array($relatedModule, $emailRelatedModules)){
				$sql = 'SELECT id FROM ncrm_users WHERE id=?';
				$result = $db->pquery($sql, array($id));
				if($db->num_rows($result) > 0){
					$relatedModule = 'Users';
				}
