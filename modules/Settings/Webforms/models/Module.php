...
class Settings_Webforms_Module_Model extends Settings_Ncrm_Module_Model {

	var $baseTable = 'ncrm_webforms';
	var $baseIndex = 'id';
	...
	public function deleteRecord($recordModel) {
		...
		$db->pquery("DELETE from ncrm_webforms_field WHERE webformid = ?", array($recordId));
		$db->pquery("DELETE from ncrm_webforms WHERE id = ?", array($recordId));
		return true;
	}
}
