...
class Settings_Currency_Record_Model extends Settings_Ncrm_Record_Model{
    ...
    public function isDisabledRestricted() {
        ...
        $query = 'SELECT 1 FROM ncrm_users WHERE currency_id = ?';
    ...
    public static function getAllNonMapped($includedIds = array()) {
        ...
        $query = 'SELECT ncrm_currencies.* FROM ncrm_currencies 
                    LEFT JOIN ncrm_currency_info ON ncrm_currency_info.currency_name = ncrm_currencies.currency_name
                    WHERE ncrm_currency_info.currency_name IS NULL or ncrm_currency_info.deleted=1';
        $params = array();
        if(!empty($includedIds)) {
            $params = $includedIds;
            $query .= ' or ncrm_currency_info.id IN('.generateQuestionMarks($includedIds).')';
        }
        $result = $db->pquery($query,$params);
