...
class Settings_SMSNotifier_Record_Model extends Settings_Ncrm_Record_Model {
	...
	public function save() {
		...
		if ($id) {
			$query = 'UPDATE ncrm_smsnotifier_servers SET providertype = ?, isactive = ?, username = ?, password = ?, parameters = ? WHERE id = ?';
			array_push($params, $id);
		} else {
			$query = 'INSERT INTO ncrm_smsnotifier_servers(providertype, isactive, username, password, parameters) VALUES(?, ?, ?, ?, ?)';
		}
	...
	static public function getInstanceById($recordId, $qualifiedModuleName) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM ncrm_smsnotifier_servers WHERE id = ?', array($recordId));
