...
class Settings_MailConverter_Module_Model extends Settings_Ncrm_Module_Model {
	...
	public function MailBoxExists() {
		...
		$result = $db->pquery("SELECT COUNT(*) AS count FROM ncrm_mailscanner", array());
	...
	public function getDefaultId() {
		$db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT MIN(scannerid) AS id FROM ncrm_mailscanner", array());
	...
	public function getMailboxes() {
		$mailBox = array();
		$db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT scannerid, scannername FROM ncrm_mailscanner", array());
	...
	public function getScannedFolders($id) {
		$folders = array();
		$db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT foldername FROM ncrm_mailscanner_folders WHERE scannerid=? AND enabled=1", array($id));
		$numOfRows = $db->num_rows($result);
		for ($i = 0; $i < $numOfRows; $i++) {
	...
	public function getFolders($id) {
		...
		$scannerInfo = new Ncrm_MailScannerInfo($scannerName);
	...
	public function getScannerName($id) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT scannername FROM ncrm_mailscanner WHERE scannerid=?", array($id));
	...
	public function updateFolders($scannerId, $folders) {
		...
		$db->pquery("DELETE FROM ncrm_mailscanner_folders WHERE scannerid=?", array($scannerId));
		foreach ($folders as $folder) {
			$db->pquery("INSERT INTO ncrm_mailscanner_folders VALUES(?,?,?,?,?,?)", array('', $scannerId, $folder, $lastScan, '0', '1'));
		}
	}
