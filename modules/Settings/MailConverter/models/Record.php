...
class Settings_MailConverter_Record_Model extends Settings_Ncrm_Record_Model {
	...
	public function hasRules() {
		...
		$result = $db->pquery('SELECT 1 FROM ncrm_mailscanner_rules WHERE scannerid = ?', array($this->getId()));
	...
	public function getRecordLinks() {
		...
		$links = array();
		if ($this->hasRules()) {
			$links[] = Ncrm_Link_Model::getInstanceFromValues(array(
					'linktype' => 'LISTVIEW',
	...
	public function delete() {
        ...
        $scanner = new Ncrm_MailScannerInfo(trim($this->getName()));
		$scanner->delete();
	}
	...
	public function save() {
        vimport('~~modules/Settings/MailConverter/handlers/MailScannerInfo.php');
		$scannerLatestInfo = new Ncrm_MailScannerInfo(false, false);
		...
		$isConnected = true;
		$scannerOldInfo = new Ncrm_MailScannerInfo($this->get('scannerOldName'));

		if(!$scannerOldInfo->compare($scannerLatestInfo)) {
            vimport('~~modules/Settings/MailConverter/handlers/MailBox.php');
			$mailBox = new Ncrm_MailBox($scannerLatestInfo);
	...
	public function scanNow() {
		...
		if ($isValid) {
			...
			$scannerInfo = new Ncrm_MailScannerInfo($this->getName());
			/** Start the scanning. */
			$scanner = new Ncrm_MailScanner($scannerInfo);
	...
	public function getUpdatedFoldersList() {
        vimport('~~modules/Settings/MailConverter/handlers/MailBox.php');
		$scannerInfo = new Ncrm_MailScannerInfo($this->getName());
		$mailBox = new Ncrm_MailBox($scannerInfo);
	...
	public function saveFolders() {
		...
		$updateQuery = "UPDATE ncrm_mailscanner_folders SET enabled = CASE folderid ";
	...
	public function updateSequence($sequencesList) {
		...
		$updateQuery = "UPDATE ncrm_mailscanner_rules SET sequence = CASE";
	...
	public static function getCleanInstance() {
		...
		return $recordModel->setModule(Settings_Ncrm_Module_Model::getInstance('Settings:MailConverter'));
	}
	...
	public static function getInstanceById($recordId) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM ncrm_mailscanner WHERE scannerid = ', array($recordId));
		if ($db->num_rows($result)) {
	...
	public static function getAll() {
		$db = PearDatabase::getInstance();
		$moduleModel = Settings_Ncrm_Module_Model::getInstance('Settings:MailConverter');

		$result = $db->pquery('SELECT * FROM ncrm_mailscanner', array());
	...
	public static function getCount() {
		...
		$result = $db->pquery('SELECT 1 FROM ncrm_mailscanner', array());
		$numOfRows = $db->num_rows($result);
		return $numOfRows;
	}
