...
class Settings_Workflows_EditTask_View extends Settings_Ncrm_Index_View {
	...
	public function process(Ncrm_Request $request) {
        ...
        if($moduleModel->getName() == 'Invoice' || $moduleModel->getName() == 'Quotes') {
            $metaVariables['Portal Pdf Url'] = '(general : (__NcrmMeta__) portalpdfurl)';
        }
		...
		if(!empty ($repeat_date)){
		    $repeat_date = Ncrm_Date_UIType::getDisplayDateValue($repeat_date);
		}
		...
		$fromEmailFieldOptions .= '<option value="$(general : (__NcrmMeta__) supportName)<$(general : (__NcrmMeta__) supportEmailId)>"
		...
		$assignedToValues[vtranslate('LBL_USERS', 'Ncrm')] = $userList;
		$assignedToValues[vtranslate('LBL_GROUPS', 'Ncrm')] = $groupList;
