...
class Settings_LayoutEditor_Module_Model extends Ncrm_Module_Model {
    ...
    public function addField($fieldType, $blockId, $params) {
        ...
        $max_fieldid = $db->getUniqueID("ncrm_field");
        ...
        if (isset($focus->customFieldTable)) {
            ...
            $tableName= 'ncrm_'.strtolower($moduleName).'cf';
        }
        ...
        $blockModel = Ncrm_Block_Model::getInstance($blockId, $this);
    ...
    public function getTypeDetailsForAddField($fieldType,$params) {
        switch ($fieldType) {
                ...
                Case 'Decimal' :
                               ...
                               // Fix for http://trac.ncrm.com/cgi-bin/trac.cgi/ticket/6363
                               $uichekdata='NN~O';
                               break;
               Case 'Percent' :
                               $uitype = 9;
    ...
    public function checkFIeldExists($fieldLabel) {
        ...
        $query = 'SELECT 1 FROM ncrm_field WHERE tabid IN ('.  generateQuestionMarks($tabId).') AND fieldlabel=?';
	...
	public static function getEntityModulesList() {
		...
		$query = 'SELECT name FROM ncrm_tab WHERE
	...
	public function getRelations() {
		if($this->relations === null) {
			$this->relations = Ncrm_Relation_Model::getAllRelations($this, false);
		}
