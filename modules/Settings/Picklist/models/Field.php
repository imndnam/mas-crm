...
class Settings_Picklist_Field_Model extends Ncrm_Field_Model {
    ...
    public function getPicklistValues($roleIdList, $groupMode='INTERSECTION') {
        ...
        $tableName = 'ncrm_'.$fieldName;
        $idColName = $fieldName.'id';
        $query = 'SELECT '.$fieldName;
        if($intersectionMode) {
            $query .= ',count(roleid) as rolecount ';
        }
        $query .= ' FROM  ncrm_role2picklist INNER JOIN '.$tableName.' ON ncrm_role2picklist.picklistvalueid = '.$tableName.'.picklist_valueid'.
	...
	public static function getEditablePicklistValues($fieldName){
        ...
        $query="SELECT $primaryKey ,$fieldName FROM ncrm_$fieldName WHERE presence=1 AND $fieldName <> '--None--'";
        $values = array();
	...
	public static function getNonEditablePicklistValues($fieldName){
        ...
        $query = "select $fieldName from ncrm_$fieldName where presence=0";
        $values = array();
        $result = $db->pquery($query, array());
        $num_rows = $db->num_rows($result);
        for($i=0; $i<$num_rows; $i++) {
