...
class Settings_ModuleManager_Module_Model extends Ncrm_Module_Model {

    public static function getNonVisibleModulesList() {
        return array('ModTracker','Webmails','Users','Mobile','Integration','WSAPP',
                     'ConfigEditor','FieldFormulas','NcrmBackup','CronTasks','Import' ,'Tooltip',
	 ...
	 * @return <Array of Ncrm_Module_Model>
	 */
	public static function getModulesSupportingSequenceNumbering() {
		$db = PearDatabase::getInstance();
		$sql="SELECT tabid, name FROM ncrm_tab WHERE isentitytype = 1 AND presence = 0 AND tabid IN
			(SELECT DISTINCT tabid FROM ncrm_field WHERE uitype = '4')";
