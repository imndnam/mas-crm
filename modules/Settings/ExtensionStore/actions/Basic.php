...
class Settings_ExtensionStore_Basic_Action extends Settings_Ncrm_IndexAjax_View {
    ...
    protected function postReview(Ncrm_Request $request) {
    ...
    protected function getDateString($date) {
        $dateString = Ncrm_Util_Helper::formatDateTimeIntoDayString($date);
        return $dateString;
    }


    protected function logoutMarketPlace(Ncrm_Request $request) {
         $modelInstance = $this->getModelInstance(); 
         $modelInstance->logoutMarketPlace($request);
    }


    protected function uninstallExtension(Ncrm_Request $request) {
        ...
        if($extnType == 'module'){
            ...
            if($languageInstance) {
                ...
                Ncrm_Language::deregister($langPrefix);
                //remove files
    ...
    protected function registerAccount(Ncrm_Request $request) {
        $options = array();
    ...
    protected function updateTrialMode(Ncrm_Request $request) {
        $response = new Ncrm_Response();
        $importedModuleName = $request->get('extensionName');
        $trial = $request->get('trial');
        $db = PearDatabase::getInstance();
        $db->pquery('UPDATE ncrm_tab SET trial = ? WHERE name = ?', array($trial, $importedModuleName));
        $response->setResult(array('success' => true, 'message' => 'Extension Store Installed'));
        $response->emit();
    }
    
    protected function updateCardDetails(Ncrm_Request $request) {
