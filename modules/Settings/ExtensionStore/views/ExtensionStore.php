...
class Settings_ExtensionStore_ExtensionStore_View extends Settings_Ncrm_Index_View {
    ...
    public function process(Ncrm_Request $request) {
        ...
        $loaderInfo =  $loaderRequired ? $loaderInstance->ncrm_extensionloader_suggest() : null;
    ...
    protected function searchExtension(Ncrm_Request $request) {
    ...
    protected function detail(Ncrm_Request $request) {
        $viewer = $this->getViewer($request);
        $qualifiedModuleName = $request->getModule(false);
    ...
    protected function installationLog(Ncrm_Request $request) {
        $viewer = $this->getViewer($request);
        global $Ncrm_Utils_Log;
        $viewer->assign('NCRM_UTILS_LOG', $Ncrm_Utils_Log);
        $Ncrm_Utils_Log = true;
        $qualifiedModuleName = $request->getModule(false);
            ...
            if ($package) {
                ...
                if (!$upgradeError) {
                    if (!$isLanguagePackage) {
                        $moduleModel = Ncrm_Module_Model::getInstance($importedModuleName);
        ...
        if ($extensionId && $extensionModel) {
            ...
            $viewer->assign('TARGET_MODULE_INSTANCE', Ncrm_Module_Model::getInstance($targetModuleName));
    ...
    protected function oneClickInstall(Ncrm_Request $request) {
        $viewer = $this->getViewer($request);
        global $Ncrm_Utils_Log;
        $viewer->assign('NCRM_UTILS_LOG', $Ncrm_Utils_Log);
        $Ncrm_Utils_Log = true;
