...
class Settings_ExtensionStore_Extension_Model extends Ncrm_Base_Model {
	...
	public function __construct() {
		parent::__construct();
		$this->EXTENSIONSTORE_LOOKUP_URL = 'https://marketplace.ncrm.com/api';
		global $site_URL;
		if (empty($site_URL)) {
    ...
    public static function getLanguageInstance($lang) {
        $sql = 'SELECT id,name,prefix FROM ncrm_language WHERE name = ?';
	...
	public function findListings($searchTerm=null, $searchType) {
         ...
         */
	public function logoutMarketPlace(Ncrm_Request $request) {
		    $sql = 'DELETE FROM ncrm_extnstore_users';
		    $db = PearDatabase::getInstance();
		    $db->pquery($sql, array());
	    }
	...
	public function getInstanceFromArray($listing) {
		...
		foreach ($listing as $key => $value) {
			...
			case 'minrange':   $key = 'ncrmVersion'; break;
			case 'maxrange':   $key = 'ncrmMaxVersion'; break;
    ...
    public static function getModuleFromExtnName($extnName) {
        $moduleModel = Ncrm_Module_Model::getInstance($extnName);
        if($moduleModel) {
            $moduleModel->set('extnType','module');
        }
        if(!$moduleModel) {
            if(self::getLanguageInstance($extnName)) {
               $moduleModel = new Ncrm_Module_Model();
       ...
       public function getCardDetails($cardId) {
        ...
        public function getLocationUrl($extensionId, $extensionName){
            ...
            if(is_admin($current_user)){
                ...
                return 'https://marketplace.ncrm.com/app/listings?id='.$extensionId;
            }
        }
}
