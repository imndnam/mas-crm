...
class Settings_Ncrm_ConfigModule_Model extends Settings_Ncrm_Module_Model {
	...
	public function getMenuItem() {
		$menuItem = Settings_Ncrm_MenuItem_Model::getInstance('LBL_CONFIG_EDITOR');
		return $menuItem;
	}
	...
	public function getEditViewUrl() {
		...
		return '?module=Ncrm&parent=Settings&view=ConfigEditorEdit&block='.$menuItem->get('blockid').'&fieldid='.$menuItem->get('fieldid');
	}
	...
	public function getDetailViewUrl() {
		$menuItem = $this->getMenuItem();
		return '?module=Ncrm&parent=Settings&view=ConfigEditorDetail&block='.$menuItem->get('blockid').'&fieldid='.$menuItem->get('fieldid');
	}
	...
	public function getPicklistValues($fieldName) {
		if ($fieldName === 'default_module') {
			...
			$query = 'SELECT name, tablabel FROM ncrm_tab WHERE presence IN (' . generateQuestionMarks($presence) . ') AND isentitytype = ? AND name NOT IN (' . generateQuestionMarks($restrictedModules) . ')';
            ...
            $db = PearDatabase::getInstance();
            $query = 'SELECT name,label FROM ncrm_layout';

            $result = $db->pquery($query, array());
            $numOfRows = $db->num_rows($result);

            $folders =  array('vlayout' => 'Vlayout');
            
            for ($i = 0; $i < $numOfRows; $i++) {
                $row = $db->query_result_rowdata($result, $i);
                $folders[$db->query_result($result, $i, 'name')] = $db->query_result($result, $i, 'label');
           }
            return $folders;
        }
...
//			'history_max_viewed'			=> array('label' => 'LBL_MAX_HISTORY_VIEWED',				'fieldType' => 'input'),
	 ...
	 * @return <Settings_Ncrm_ConfigModule_Model> $moduleModel
	 */
