...
class Settings_Ncrm_TaxRecord_Model extends Ncrm_Base_Model{
	...
	public function getCreateTaxUrl() {
		return '?module=Ncrm&parent=Settings&view=TaxAjax';
	}
	
	public function getEditTaxUrl() {
		return '?module=Ncrm&parent=Settings&view=TaxAjax&type='.$this->getType().'&taxid='.$this->getId();
	}
    ...
    public function addTax() {
        ...
        if($this->isShippingTax()) {
            $taxname = "shtax".$taxid;
            $query = "ALTER TABLE ncrm_inventoryshippingrel ADD COLUMN $taxname decimal(7,3) DEFAULT NULL";
        } else {
            $taxname = "tax".$taxid;
            $query = "ALTER TABLE ncrm_inventoryproductrel ADD COLUMN $taxname decimal(7,3) DEFAULT NULL";
        }
