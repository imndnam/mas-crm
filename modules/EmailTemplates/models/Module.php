...
class EmailTemplates_Module_Model extends Ncrm_Module_Model {
	...
	public function saveRecord($recordModel) {
		...
		if(empty($templateid)){
			$templateid = $db->getUniqueID('ncrm_emailtemplates');
			$sql = "INSERT INTO ncrm_emailtemplates(templatename, subject, description, body, deleted, templateid) VALUES (?,?,?,?,?,?)";
		}else{
			$sql = "UPDATE ncrm_emailtemplates SET templatename=?, subject=?, description=?, body=?, deleted=? WHERE templateid = ?";
		}
	...
	public function deleteRecord($recordModel) {
		...
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM ncrm_emailtemplates WHERE templateid = ? ', array($recordId));
	}
	...
	public function deleteAllRecords() {
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM ncrm_emailtemplates', array());
	}
	...
	public function getAllModuleList(){
		...
		$query = 'SELECT DISTINCT(name) AS modulename FROM ncrm_tab 
				  LEFT JOIN ncrm_field ON ncrm_field.tabid = ncrm_tab.tabid
				  WHERE (ncrm_field.uitype = ? AND ncrm_tab.presence = ?)';
	...
	public function getRecordIds($skipRecords){
		...
		$query = 'SELECT templateid FROM ncrm_emailtemplates WHERE templateid NOT IN ('.generateQuestionMarks($skipRecords).')';
