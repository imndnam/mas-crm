...
class Webforms {
	...
	function vtlib_handler($moduleName, $eventType) {
		...
		global $adb;
			...
			if($eventType == 'module.postinstall') {
				...
				// Mark the module as Standard module
				$this->updateSettings();
				$adb->pquery('UPDATE ncrm_tab SET customized=0 WHERE name=?', array($this->LBL_WEBFORMS));
			} else if($eventType == 'module.disabled') {
			// TODO Handle actions when this module is disabled.
				global $log,$adb;
				$adb->pquery('UPDATE ncrm_settings_field SET active= 1  WHERE  name= ?',array($this->LBL_WEBFORMS));
			} else if($eventType == 'module.enabled') {
			// TODO Handle actions when this module is enabled.
				global $log,$adb;
				$adb->pquery('UPDATE ncrm_settings_field SET active= 0  WHERE  name= ?',array($this->LBL_WEBFORMS));
	...
	function updateSettings(){
			...
			$result=$adb->pquery('SELECT 1 FROM ncrm_settings_field WHERE name=?',array($this->LBL_WEBFORMS));
