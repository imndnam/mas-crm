...
class Calendar_Import_View extends Ncrm_Import_View {
	 ...
	 */
	public function import(Ncrm_Request $request) {
	 ...
	 * @param Ncrm_Request $request
	 */
	public function importResult(Ncrm_Request $request) {
		...
		if (Import_Utils_Helper::validateFileUpload($request)) {
			...
			$calendarModel = Ncrm_Module_Model::getInstance($moduleName);
	 ...
	 * @param Ncrm_Request $request
	 */
	public function undoImport(Ncrm_Request $request) {
		$currentUserModel = Users_Record_Model::getCurrentUserModel();
		$moduleName = $request->getModule();
