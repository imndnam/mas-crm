...
Class Calendar_Edit_View extends Ncrm_Edit_View {
	...
	function Events($request, $moduleName) {
		...
		$eventModule = Ncrm_Module_Model::getInstance($moduleName);
		...
		foreach($requestFieldList as $fieldName=>$fieldValue){
            ...
            if (empty($record) && ($fieldName == 'date_start' || $fieldName == 'due_date') && !empty($fieldValue)) { 
                if($fieldName == 'date_start'){
                    $startTime = Ncrm_Time_UIType::getTimeValueWithSeconds($requestFieldList['time_start']);
                    $startDateTime = Ncrm_Datetime_UIType::getDBDateTimeValue($fieldValue." ".$startTime);
                    list($startDate, $startTime) = explode(' ', $startDateTime);
                    $fieldValue = Ncrm_Date_UIType::getDisplayDateValue($startDate);
                }else{
                    $endTime = Ncrm_Time_UIType::getTimeValueWithSeconds($requestFieldList['time_end']);
                    $endDateTime = Ncrm_Datetime_UIType::getDBDateTimeValue($fieldValue." ".$endTime);
                    list($endDate, $endTime) = explode(' ', $endDateTime);
                    $fieldValue = Ncrm_Date_UIType::getDisplayDateValue($endDate);
                }
            }
		...
		$viewer->assign('TOMORROWDATE', Ncrm_Date_UIType::getDisplayDateValue(date('Y-m-d', time()+86400)));
		...
		if(!empty($requestContactIdValue)) {
			$existingRelatedContacts[] = array('name' => Ncrm_Util_Helper::getRecordName($requestContactIdValue) ,'id' => $requestContactIdValue);
		}
