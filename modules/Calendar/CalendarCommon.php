...
/**
 * To get the lists of ncrm_users id who shared their calendar with specified user
 * @param $sharedid -- The shared user id :: Type integer
 * @returns $shared_ids -- a comma seperated ncrm_users id  :: Type string
 */
function getSharedCalendarId($sharedid)
{
	global $adb;
	$query = "SELECT * from ncrm_sharedcalendar where sharedid=?";
...
/**
 * Function to get the ncrm_activity details for mail body
	...
	if(!empty($only_for_user)) {
		if($only_for_user != 'ALL') {
			...
			if($only_for_user == $current_user->id) {
				...
				$qcondition = " AND ncrm_crmentity.smownerid IN (" . $user_group_ids .")";
			} else {
				$qcondition = " AND ncrm_crmentity.smownerid = "  . $adb->sql_escape_string($only_for_user);
			}
		}
	}
	return $qcondition;
