...
class Calendar_DragDropAjax_Action extends Calendar_SaveAjax_Action {
    ...
    public function updateDeltaOnResize(Ncrm_Request $request){
        ...
        if(isPermitted($moduleName, $actionname, $recordId) === 'no'){
            ...
            $record = Ncrm_Record_Model::getInstanceById($recordId, $moduleName);
    ...
    public function updateDeltaOnDrop(Ncrm_Request $request){
        $moduleName = $request->getModule();
        $activityType = $request->get('activitytype');
        $recordId = $request->get('id');
        $dayDelta = $request->get('dayDelta');
