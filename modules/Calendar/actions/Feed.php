...
class Calendar_Feed_Action extends Ncrm_BasicAjax_Action {
	...
	protected function pullEvents($start, $end, &$result, $userid = false,$color = null,$textColor = 'white') {
		...
		$moduleModel = Ncrm_Module_Model::getInstance('Events');
		if($userid){
			$focus = new Users();
			$focus->id = $userid;
		...
		$query.= " AND ncrm_activity.activitytype NOT IN ('Emails','Task') AND ";
        $hideCompleted = $currentUser->get('hidecompletedevents');
        if($hideCompleted)
            $query.= "ncrm_activity.eventstatus != 'HELD' AND ";
        ...
        $query.= " AND ncrm_crmentity.smownerid IN (".  generateQuestionMarks($params).")";
	...
	protected function pullTasks($start, $end, &$result, $color = null,$textColor = 'white') {
		...
		$moduleModel = Ncrm_Module_Model::getInstance('Calendar');
		...
		$query.= " AND ncrm_activity.activitytype = 'Task' AND ";
        $currentUser = Users_Record_Model::getCurrentUserModel();
        $hideCompleted = $currentUser->get('hidecompletedevents');
        if($hideCompleted)
            $query.= "ncrm_activity.status != 'Completed' AND ";
		$query.= " ((date_start >= ? AND due_date < ?) OR ( due_date >= ?))";
                $params = array($start,$end,$start);
        $params = array_merge($params, $userAndGroupIds);
		$query.= " AND ncrm_crmentity.smownerid IN (".generateQuestionMarks($userAndGroupIds).")";
	...
	protected  function pullContactsByBirthday($start, $end, &$result, $color = null,$textColor = 'white') {
		...
		$query = "SELECT firstname,lastname,birthday,crmid FROM ncrm_contactdetails";
		$query.= " INNER JOIN ncrm_contactsubdetails ON ncrm_contactdetails.contactid = ncrm_contactsubdetails.contactsubscriptionid";
		$query.= " INNER JOIN ncrm_crmentity ON ncrm_contactdetails.contactid = ncrm_crmentity.crmid";
		$query.= " WHERE ncrm_crmentity.deleted=0 AND smownerid IN (".  generateQuestionMarks($userAndGroupIds) .") AND";
	...
	protected function pullProjects($start, $end, &$result, $color = null,$textColor = 'white') {
		...
		$query = "SELECT projectname, startdate, targetenddate, crmid FROM ncrm_project";
		$query.= " INNER JOIN ncrm_crmentity ON ncrm_project.projectid = ncrm_crmentity.crmid";
		$query.= " WHERE ncrm_crmentity.deleted=0 AND smownerid IN (". generateQuestionMarks($userAndGroupIds) .") AND ";
	...
	protected function pullProjectTasks($start, $end, &$result, $color = null,$textColor = 'white') {
		...
		$query = "SELECT projecttaskname, startdate, enddate, crmid FROM ncrm_projecttask";
		$query.= " INNER JOIN ncrm_crmentity ON ncrm_projecttask.projecttaskid = ncrm_crmentity.crmid";
		$query.= " WHERE ncrm_crmentity.deleted=0 AND ";
