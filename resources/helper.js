...
jQuery.Class("Ncrm_Helper_Js",{
	...
	requestToShowComposeEmailForm : function(selectedId,fieldname,fieldmodule){
		...
		var emailsMassEditInstance = Ncrm_Helper_Js.getEmailMassEditInstance();
	...
	getInternalMailer  : function(selectedId,fieldname,fieldmodule){
		var module = 'Emails';
		var cacheResponse = Ncrm_Helper_Js.checkServerConfigResponseCache;
		var  checkServerConfigPostOperations = function (data) {
			if(data == true){
				Ncrm_Helper_Js.requestToShowComposeEmailForm(selectedId,fieldname,fieldmodule);
			} else {
				alert(app.vtranslate('JS_EMAIL_SERVER_CONFIGURATION'));
			}
		}
		if(cacheResponse === ''){
			var checkServerConfig = Ncrm_Helper_Js.checkServerConfig(module);
			checkServerConfig.then(function(data){
				Ncrm_Helper_Js.checkServerConfigResponseCache = data;
				checkServerConfigPostOperations(Ncrm_Helper_Js.checkServerConfigResponseCache);
			});
		} else {
			checkServerConfigPostOperations(Ncrm_Helper_Js.checkServerConfigResponseCache);
		}
