...
class Ncrm_Loader {
	 ...
	 * @throws AppException
	 */
	public static function getComponentClassName($componentType, $componentName, $moduleName='Ncrm') {
		...
		$fallBackModuleDir = $fallBackModuleClassPath = 'Ncrm';
		...
		if(strpos($moduleName, ':') > 0) {
			...
			if($baseModule == 'Settings')  $baseModule = 'Settings:Ncrm';
		...
		$moduleSpecificComponentFilePath = Ncrm_Loader::resolveNameToPath('modules.'.$moduleDir.'.'.$componentTypeDirectory.'.'.$componentName);
		...
		if(!empty($firstFallBackDir) && !empty($firstFallBackClassPath)) {
			$fallBackComponentFilePath = Ncrm_Loader::resolveNameToPath('modules.'.$firstFallBackDir.'.'.$componentTypeDirectory.'.'.$componentName);
		...
		if(!empty($secondFallBackModuleDir) && !empty($secondFallBackModuleClassPath)) {
			$fallBackComponentFilePath = Ncrm_Loader::resolveNameToPath('modules.'.$secondFallBackModuleDir.'.'.$componentTypeDirectory.'.'.$componentName);
		...
		$fallBackComponentFilePath = Ncrm_Loader::resolveNameToPath('modules.'.$fallBackModuleDir.'.'.$componentTypeDirectory.'.'.$componentName);
	...
	public static function autoLoad($className) {
		...
		if($noOfParts > 2) {
            ...
            return Ncrm_Loader::includeOnce($filePath);
		}
		return false;
	}
}
...
function vimport($qualifiedName) {
	return Ncrm_Loader::includeOnce($qualifiedName);
}

function vimport_try($qualifiedName) {
	return Ncrm_Loader::includeOnce($qualifiedName, true);
}

function vimport_path($qualifiedName) {
	return Ncrm_Loader::includePath($qualifiedName);
}

spl_autoload_register('Ncrm_Loader::autoLoad');
