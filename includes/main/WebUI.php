...
class Ncrm_WebUI extends Ncrm_EntryPoint {
	 ...
	 * @throws AppException
	 */
	protected function checkLogin (Ncrm_Request $request) {
		if (!$this->hasLogin()) {
            ...
            if($return_params && !$_SESSION['return_params']) {
                ...
                Ncrm_Session::set('return_params', $return_params);
            }
	...
	function getLogin() {
		...
		if (!$user && isset($_SESSION['authenticated_user_id'])) {
			$userid = Ncrm_Session::get('AUTHUSERID', $_SESSION['authenticated_user_id']);
	...
	function process (Ncrm_Request $request) {
		...
		if ($currentUser && $qualifiedModuleName) {
			$moduleLanguageStrings = Ncrm_Language_Handler::getModuleStringsFromFile($currentLanguage,$qualifiedModuleName);
		...
		try {
			...
			$handlerClass = Ncrm_Loader::getComponentClassName($componentType, $componentName, $qualifiedModuleName);
            ...
            if ($handler) {
				...
				$skipList = array('Users', 'Home', 'CustomView', 'Import', 'Export', 'Inventory', 'Ncrm','PriceBooks','Migration','Install');
