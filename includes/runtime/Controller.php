 ...
 */
abstract class Ncrm_Controller {
	...
	abstract function getViewer(Ncrm_Request $request);
 ...
 */
abstract class Ncrm_Action_Controller extends Ncrm_Controller {
	function __construct() {
		parent::__construct();
	}

	function getViewer(Ncrm_Request $request) {
 ...
 */
abstract class Ncrm_View_Controller extends Ncrm_Action_Controller {
	...
	function getViewer(Ncrm_Request $request) {
		if(!$this->viewer) {
			...
			$viewer->assign('NCRM_VERSION', $ncrm_current_version);
	 ...
	 * @return <array> - array of Ncrm_JsScript_Model
	 */
	function getHeaderScripts(Ncrm_Request $request){
		$headerScriptInstances = array();
		$languageHandlerShortName = Ncrm_Language_Handler::getShortLanguageName();
	...
	function checkAndConvertJsScripts($jsFileNames) {
		...
		foreach($jsFileNames as $jsFileName) {
			...
			$jsScript = new Ncrm_JsScript_Model();
			...
			$completeFilePath = Ncrm_Loader::resolveNameToPath($jsFileName, $fileExtension);

			if(file_exists($completeFilePath)) {
				if (strpos($jsFileName, '~') === 0) {
					$filePath = ltrim(ltrim($jsFileName, '~'), '/');
					// if ~~ (reference is outside ncrm6 folder)
				...
				$fallBackFilePath = Ncrm_Loader::resolveNameToPath(Ncrm_JavaScript::getBaseJavaScriptPath().'/'.$jsFileName, 'js');
				if(file_exists($fallBackFilePath)) {
					$filePath = str_replace('.','/', $jsFileName) . '.js';
					$jsScriptInstances[$jsFileName] = $jsScript->set('src', Ncrm_JavaScript::getFilePath($filePath));
				}
			}
		}
	 ...
	 * @return <Array of Ncrm_CssScript_Model>
	 *
	...
	function checkAndConvertCssStyles($cssFileNames, $fileExtension='css') {
		...
		foreach($cssFileNames as $cssFileName) {
			...
			$completeFilePath = Ncrm_Loader::resolveNameToPath($cssFileName, $fileExtension);
			$filePath = NULL;
			if(file_exists($completeFilePath)) {
				if (strpos($cssFileName, '~') === 0) {
					...
					$filePath = Ncrm_Theme::getStylePath($filePath);
				}
	 ...
	 * @param Ncrm_Request $request
	 */
	function getJSLanguageStrings(Ncrm_Request $request) {
		$moduleName = $request->getModule(false);
		return Ncrm_Language_Handler::export($moduleName, 'jsLanguageStrings');
	}
}