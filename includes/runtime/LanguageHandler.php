...
class Ncrm_Language_Handler {
     ...
     */
    public static function getModuleStringsFromFile($language, $module='Ncrm'){
        $module = str_replace(':', '.', $module);
        if(empty(self::$languageContainer[$language][$module])){
            $qualifiedName = 'languages.'.$language.'.'.$module;
            $file = Ncrm_Loader::resolveNameToPath($qualifiedName);
    ...
    public static function getLanguageLabel($name) {
        ...
        $languageResult = $db->pquery('SELECT label FROM ncrm_language WHERE prefix = ?', array($name));
...
function vtranslate($key, $moduleName = '') {
    $args = func_get_args();
    $formattedString = call_user_func_array(array('Ncrm_Language_Handler', 'getTranslatedString'), $args);
    array_shift($args);
    array_shift($args);
...
function vJSTranslate($key, $moduleName = '') {
    $args = func_get_args();
    return call_user_func_array(array('Ncrm_Language_Handler', 'getJSTranslatedString'), $args);
}
