 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/ncrm_crm/ncrmcrm/data/CRMEntity.php,v 1.16 2005/04/29 04:21:31 mickie Exp $
...
class CRMEntity {
	...
	static function isBulkSaveMode() {
		global $NCRM_BULK_SAVE_MODE;
		if (isset($NCRM_BULK_SAVE_MODE) && $NCRM_BULK_SAVE_MODE) {
			return true;
		}
		return false;
	}
	...
	function uploadAndSaveFile($id, $module, $file_details) {
		...
		if ($save_file == 'true' && $upload_status == 'true') {
			//This is only to update the attached filename in the ncrm_notes ncrm_table for the Notes module
			if ($module == 'Contacts' || $module == 'Products') {
			...
			if ($module == 'Contacts') {
				$att_sql = "select ncrm_seattachmentsrel.attachmentsid  from ncrm_seattachmentsrel inner join ncrm_crmentity on ncrm_crmentity.crmid=ncrm_seattachmentsrel.attachmentsid where ncrm_crmentity.setype='Contacts Image' and ncrm_seattachmentsrel.crmid=?";
				...
				if ($attachmentsid != '') {
					...
					$crm_delquery = "delete from ncrm_crmentity where crmid=?";
					$adb->pquery($crm_delquery, array($attachmentsid));
					$sql5 = 'insert into ncrm_seattachmentsrel values(?,?)';
	...
	/** Function to insert values in the ncrm_crmentity for the specified module
	...
	function insertIntoCrmEntity($module, $fileid = '') {
		...
		if ($this->mode == 'edit') {
			...
			if ($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0) {
				$sql = "update ncrm_crmentity set smownerid=?,modifiedby=?,description=?, modifiedtime=? where crmid=?";
				$params = array($ownerid, $current_user->id, $description_val, $adb->formatDate($date_var, true), $this->id);
			} else {
				$profileList = getCurrentUserProfileList();
				$perm_qry = "SELECT columnname FROM ncrm_field INNER JOIN ncrm_profile2field ON ncrm_profile2field.fieldid = ncrm_field.fieldid INNER JOIN ncrm_def_org_field ON ncrm_def_org_field.fieldid = ncrm_field.fieldid WHERE ncrm_field.tabid = ? AND ncrm_profile2field.visible = 0 AND ncrm_profile2field.readonly = 0 AND ncrm_profile2field.profileid IN (" . generateQuestionMarks($profileList) . ") AND ncrm_def_org_field.visible = 0 and ncrm_field.tablename='ncrm_crmentity' and ncrm_field.displaytype in (1,3) and ncrm_field.presence in (0,2);";
				...
				if (is_array($columname) && in_array("description", $columname)) {
					...
					$sql = "update ncrm_crmentity set smownerid=?,modifiedby=?, modifiedtime=? where crmid=?";
			...
			$description_val = from_html($this->column_fields['description'], ($insertion_mode == 'edit') ? true : false);
			$sql = "insert into ncrm_crmentity (crmid,smcreatorid,smownerid,setype,description,modifiedby,createdtime,modifiedtime) values(?,?,?,?,?,?,?,?)";
	...
	function insertIntoEntityTable($table_name, $module, $fileid = '') {
		...
		$log->info("function insertIntoEntityTable " . $module . ' ncrm_table name ' . $table_name);
		global $adb;
		$insertion_mode = $this->mode;

		//Checkin whether an entry is already is present in the ncrm_table to update
		...
		if ($insertion_mode == 'edit') {
			...
			require('user_privileges/user_privileges_' . $current_user->id . '.php');
			if ($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0) {
				$sql = "select * from ncrm_field where tabid in (" . generateQuestionMarks($tabid) . ") and tablename=? and displaytype in (1,3) and presence in (0,2) group by columnname";
				...
				if (count($profileList) > 0) {
					$sql = "SELECT *
			  			FROM ncrm_field
			  			INNER JOIN ncrm_profile2field
			  			ON ncrm_profile2field.fieldid = ncrm_field.fieldid
			  			INNER JOIN ncrm_def_org_field
			  			ON ncrm_def_org_field.fieldid = ncrm_field.fieldid
			  			WHERE ncrm_field.tabid = ?
			  			AND ncrm_profile2field.visible = 0 AND ncrm_profile2field.readonly = 0
			  			AND ncrm_profile2field.profileid IN (" . generateQuestionMarks($profileList) . ")
			  			AND ncrm_def_org_field.visible = 0 and ncrm_field.tablename=? and ncrm_field.displaytype in (1,3) and ncrm_field.presence in (0,2) group by columnname";
			...
			if ($table_index_column == 'id' && $table_name == 'ncrm_users') {
				$currentuser_id = $adb->getUniqueID("ncrm_users");
				$this->id = $currentuser_id;
			}
			$column = array($table_index_column);
			$value = array($this->id);
			$sql = "select * from ncrm_field where tabid=? and tablename=? and displaytype in (1,3,4) and ncrm_field.presence in (0,2)";
			$params = array($tabid, $table_name);
		}
		...
		for ($i = 0; $i < $noofrows; $i++) {
			...
			if (isset($this->column_fields[$fieldname])) {
				if ($uitype == 56) {
					...
					if ($this->column_fields[$fieldname] == null) {
						$fileQuery = $adb->pquery("SELECT filename from ncrm_notes WHERE notesid = ?", array($this->id));
						$fldvalue = null;
						if (isset($fileQuery)) {
					...
					if (empty($fldvalue)) {
						$query = "SELECT email1 FROM ncrm_users WHERE id = ?";
			...
			if ($insertion_mode == 'edit') {
				if ($table_name != 'ncrm_ticketcomments' && $uitype != 4) {
		...
		if ($insertion_mode == 'edit') {
			if ($module == 'Potentials') {
				$dbquery = 'select sales_stage from ncrm_potential where potentialid = ?';
				...
				if ($sales_stage != $_REQUEST['sales_stage'] && $_REQUEST['sales_stage'] != '') {
					...
					$sql = "insert into ncrm_potstagehistory values(?,?,?,?,?,?,?,?)";
	 ...
	 * The function will get the attachmentsid for the given entityid from ncrm_seattachmentsrel table and get the attachmentsname from ncrm_attachments table
	...
	function getOldFileName($notesid) {
		...
		global $adb;
		$query1 = "select * from ncrm_seattachmentsrel where crmid=?";
		$result = $adb->pquery($query1, array($notesid));
		$noofrows = $adb->num_rows($result);
		if ($noofrows != 0)
			$attachmentid = $adb->query_result($result, 0, 'attachmentsid');
		if ($attachmentid != '') {
			$query2 = "select * from ncrm_attachments where attachmentsid=?";
	...
	function retrieve_entity_info($record, $module) {
		...
		if (isset($this->multirow_tables)) {
			...
			$multirow_tables = array(
				'ncrm_campaignrelstatus',
				'ncrm_attachments',
				//'ncrm_inventoryproductrel',
				//'ncrm_cntactivityrel',
				'ncrm_email_track'
			);
		}
		...
		if ($cachedModuleFields === false) {
			...
			$sql0 = "SELECT fieldname, fieldid, fieldlabel, columnname, tablename, uitype, typeofdata,presence FROM ncrm_field WHERE tabid=?";
		...
		if ($cachedModuleFields) {
			...
			foreach ($cachedModuleFields as $fieldinfo) {
				...
				// Added to avoid picking shipping tax fields for Inventory modules, the shipping tax detail are stored in ncrm_inventoryshippingrel
				// table, but in ncrm_field table we have set tablename as ncrm_inventoryproductrel.
			...
			$column_clause .= 'ncrm_crmentity.deleted';

			if (isset($required_tables['ncrm_crmentity'])) {
				$from_clause  = ' ncrm_crmentity';
				unset($required_tables['ncrm_crmentity']);
				foreach ($required_tables as $tablename => $tableindex) {
					...
					$from_clause .= sprintf(' %s %s ON %s.%s=%s.%s', $join_type,
						$tablename, $tablename, $tableindex, 'ncrm_crmentity', 'crmid');
				}
			}

			$where_clause .= ' ncrm_crmentity.crmid=?';
			$params[] = $record;
	...
	function save($module_name, $fileid = '') {
		...
		$em->triggerEvent("ncrm.entity.beforesave.modifiable", $entityData);
		$em->triggerEvent("ncrm.entity.beforesave", $entityData);
		$em->triggerEvent("ncrm.entity.beforesave.final", $entityData);
		}
		...
		if($em) {
		//Event triggering code
		$em->triggerEvent("ncrm.entity.aftersave", $entityData);
		$em->triggerEvent("ncrm.entity.aftersave.final", $entityData);
		//Event triggering code ends
	}
...
//Added by Richie
	...
	function mark_deleted($id) {
		global $current_user;
		$date_var = date("Y-m-d H:i:s");
		$query = "UPDATE ncrm_crmentity set deleted=1,modifiedtime=?,modifiedby=? where crmid=?";
	...
	// the keys are user defined, and don't directly map to the bean's ncrm_fields
	// the value is the method name within that bean that will do extra
	// processing for that ncrm_field. example: 'full_name'=>'get_names_from_full_name'
	...
	/**
	 * Function to check if the custom ncrm_field ncrm_table exists
	...
	/**
	 * function to construct the query to fetch the custom ncrm_fields
	 * return the query to fetch the custom ncrm_fields
	 */
	function constructCustomQueryAddendum($tablename, $module) {
		global $adb;
		$tabid = getTabid($module);
		$sql1 = "select columnname,fieldlabel from ncrm_field where generatedtype=2 and tabid=? and ncrm_field.presence in (0,2)";
	...
	function track_view($user_id, $current_module, $id = '') {
		$this->log->debug("About to call ncrm_tracker (user_id, module_name, item_id)($user_id, $current_module, $this->id)");
	...
	function initRequiredFields($module) {
		...
		$tabid = getTabId($module);
		$sql = "select * from ncrm_field where tabid= ? and typeofdata like '%M%' and uitype not in ('53','70') and ncrm_field.presence in (0,2)";
	...
	function trash($module, $id) {
		...
		if(!self::isBulkSaveMode()) {
            ...
            $em->triggerEvent("ncrm.entity.beforedelete", $entityData);
        }
		...
		$sql_recentviewed = 'DELETE FROM ncrm_tracker WHERE user_id = ? AND item_id = ?';
		$this->db->pquery($sql_recentviewed, array($current_user->id, $id));

		if($em){
            $em->triggerEvent("ncrm.entity.afterdelete", $entityData);
        }
	}
	...
	function unlinkDependencies($module, $id) {
		...
		$fieldRes = $this->db->pquery('SELECT tabid, tablename, columnname FROM ncrm_field WHERE fieldid IN (
			SELECT fieldid FROM ncrm_fieldmodulerel WHERE relmodule=?)', array($module));
		...
		for ($i = 0; $i < $numOfFields; $i++) {
			...
			if ($tableName == 'ncrm_inventoryproductrel') {
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		$query = 'DELETE FROM ncrm_crmentityrel WHERE (crmid=? AND relmodule=? AND relcrmid=?) OR (relcrmid=? AND module=? AND crmid=?)';
		$params = array($id, $return_module, $return_id, $id, $return_module, $return_id);
		$this->db->pquery($query, $params);

		$fieldRes = $this->db->pquery('SELECT tabid, tablename, columnname FROM ncrm_field WHERE fieldid IN (
			SELECT fieldid FROM ncrm_fieldmodulerel WHERE module=? AND relmodule=?)', array($currentModule, $return_module));
		$numOfFields = $this->db->num_rows($fieldRes);
		for ($i = 0; $i < $numOfFields; $i++) {
			$tabId = $this->db->query_result($fieldRes, $i, 'tabid');
			$tableName = $this->db->query_result($fieldRes, $i, 'tablename');
	...
	function restore($module, $id) {
		...
		$date_var = date("Y-m-d H:i:s");
		$query = 'UPDATE ncrm_crmentity SET deleted=0,modifiedtime=?,modifiedby=? WHERE crmid = ?';
		...
		$entityData = VTEntityData::fromCRMEntity($this);
		//Event triggering code
		$em->triggerEvent("ncrm.entity.afterrestore", $entityData);
		//Event triggering code ends
	...
	function restoreRelatedRecords($module, $record) {
		...
		$result = $this->db->pquery('SELECT * FROM ncrm_relatedlists_rb WHERE entityid = ?', array($record));
		...
		for ($i = 0; $i < $numRows; $i++) {
			...
			if (strtoupper($action) == RB_RECORD_UPDATED) {
				...
				if ($rel_table == 'ncrm_crmentity' && $rel_column == 'deleted') {
				...
				if ($rel_table == 'ncrm_seproductrel') {
		...
		$this->db->pquery('DELETE FROM ncrm_relatedlists_rb WHERE entityid = ?', array($record));
	}
	...
	function initSortByField($module) {
		...
		$sql = "SELECT columnname FROM ncrm_field " .
				" WHERE (fieldname not like '%\_id' OR fieldname in ('assigned_user_id'))" .
				" AND tabid in (" . generateQuestionMarks($tabid) . ") and ncrm_field.presence in (0,2)";
		$params = array($tabid);
	...
	function setModuleSeqNumber($mode, $module, $req_str = '', $req_no = '') {
		...
		if ($mode == "configure" && $req_no != '') {
			$check = $adb->pquery("select cur_id from ncrm_modentity_num where semodule=? and prefix = ?", array($module, $req_str));
			if ($adb->num_rows($check) == 0) {
				$numid = $adb->getUniqueId("ncrm_modentity_num");
				$active = $adb->pquery("select num_id from ncrm_modentity_num where semodule=? and active=1", array($module));
				$adb->pquery("UPDATE ncrm_modentity_num SET active=0 where num_id=?", array($adb->query_result($active, 0, 'num_id')));

				$adb->pquery("INSERT into ncrm_modentity_num values(?,?,?,?,?,?)", array($numid, $module, $req_str, $req_no, $req_no, 1));
				return true;
				...
				if ($req_no < $num_check) {
					...
					$adb->pquery("UPDATE ncrm_modentity_num SET active=0 where active=1 and semodule=?", array($module));
					$adb->pquery("UPDATE ncrm_modentity_num SET cur_id=?, active = 1 where prefix=? and semodule=?", array($req_no, $req_str, $module));
					return true;
				}
			}
		} else if ($mode == "increment") {
			//when we save new invoice we will increment the invoice id and write
			$check = $adb->pquery("select cur_id,prefix from ncrm_modentity_num where semodule=? and active = 1", array($module));
			...
			$req_no.= $temp . ($curid + 1);
			$adb->pquery("UPDATE ncrm_modentity_num SET cur_id=? where cur_id=? and active=1 AND semodule=?", array($req_no, $curid, $module));
	...
	function isModuleSequenceConfigured($module) {
		...
		$result = $adb->pquery('SELECT 1 FROM ncrm_modentity_num WHERE semodule = ? AND active = 1', array($module));
	...
	function updateMissingSeqNumber($module) {
		...
		$tabid = getTabid($module);
		$fieldinfo = $adb->pquery("SELECT * FROM ncrm_field WHERE tabid = ? AND uitype = 4", Array($tabid));
			...
			if ($fld_table == $this->table_name) {
				...
				if ($records && $adb->num_rows($records)) {
					...
					if ($old_cur_id != $cur_id) {
						$adb->pquery("UPDATE ncrm_modentity_num set cur_id=? where semodule=? and active=1", Array($cur_id, $module));
					}
				}
			} else {
	...
	function get_attachments($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		...
		$userNameSql = getSqlForNameInDisplayFormat(array('first_name'=>'ncrm_users.first_name',
														'last_name' => 'ncrm_users.last_name'), 'Users');
		$query = "select case when (ncrm_users.user_name not like '') then $userNameSql else ncrm_groups.groupname end as user_name," .
				...
				ncrm_seattachmentsrel.attachmentsid attachmentsid, ncrm_crmentity.smownerid smownerid, ncrm_notes.notesid crmid,
				ncrm_notes.notecontent description,ncrm_notes.*
				from ncrm_notes
	...
	function isViewed($crmid = false) {
		...
		if ($crmid) {
			global $adb;
			$result = $adb->pquery("SELECT viewedtime,modifiedtime,smcreatorid,smownerid,modifiedby FROM ncrm_crmentity WHERE crmid=?", Array($crmid));
	...
	function markAsViewed($userid) {
		global $adb;
		$adb->pquery("UPDATE ncrm_crmentity set viewedtime=? WHERE crmid=? AND smownerid=?", Array(date('Y-m-d H:i:s', time()), $this->id, $userid));
	}
	...
	function save_related_module($module, $crmid, $with_module, $with_crmid) {
		...
		foreach ($with_crmid as $relcrmid) {
			...
			if ($with_module == 'Documents') {
				$checkpresence = $adb->pquery("SELECT crmid FROM ncrm_senotesrel WHERE crmid = ? AND notesid = ?", Array($crmid, $relcrmid));
				// Relation already exists? No need to add again
				if ($checkpresence && $adb->num_rows($checkpresence))
					continue;

				$adb->pquery("INSERT INTO ncrm_senotesrel(crmid, notesid) VALUES(?,?)", array($crmid, $relcrmid));
			} else {
				$checkpresence = $adb->pquery("SELECT crmid FROM ncrm_crmentityrel WHERE
					crmid = ? AND module = ? AND relcrmid = ? AND relmodule = ?", Array($crmid, $module, $relcrmid, $with_module));
				// Relation already exists? No need to add again
				if ($checkpresence && $adb->num_rows($checkpresence))
					continue;

				$adb->pquery("INSERT INTO ncrm_crmentityrel(crmid, module, relcrmid, relmodule) VALUES(?,?,?,?)", Array($crmid, $module, $relcrmid, $with_module));
			}
		}
	}
	...
	function delete_related_module($module, $crmid, $with_module, $with_crmid) {
		...
		foreach ($with_crmid as $relcrmid) {
			...
			if ($with_module == 'Documents') {
				$adb->pquery("DELETE FROM ncrm_senotesrel WHERE crmid=? AND notesid=?", Array($crmid, $relcrmid));
			} else {
				$adb->pquery("DELETE FROM ncrm_crmentityrel WHERE (crmid=? AND module=? AND relcrmid=? AND relmodule=?) OR (relcrmid=? AND relmodule=? AND crmid=? AND module=?)",
	...
	function get_related_list($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		...
		$query .= " INNER JOIN ncrm_crmentityrel ON (ncrm_crmentityrel.relcrmid = ncrm_crmentity.crmid OR ncrm_crmentityrel.crmid = ncrm_crmentity.crmid)";
		$query .= $more_relation;
		...
		$query .= " WHERE ncrm_crmentity.deleted = 0 AND (ncrm_crmentityrel.crmid = $id OR ncrm_crmentityrel.relcrmid = $id)";
	...
	function get_dependents_list($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		...
		if ($numOfFields > 0) {
			...
			$query .= " WHERE ncrm_crmentity.deleted = 0 AND $this->table_name.$this->table_index = $id";
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		foreach ($transferEntityIds as $transferId) {
			...
			$relatedRecords = $adb->pquery("SELECT relcrmid, relmodule FROM ncrm_crmentityrel WHERE crmid=? AND module=?" .
					" AND relcrmid NOT IN (SELECT relcrmid FROM ncrm_crmentityrel WHERE crmid=? AND module=?)", array($transferId, $module, $entityId, $module));
			...
			for ($i = 0; $i < $numOfRecords; $i++) {
				...
				$adb->pquery("UPDATE ncrm_crmentityrel SET crmid=? WHERE relcrmid=? AND relmodule=? AND crmid=? AND module=?", array($entityId, $relcrmid, $relmodule, $transferId, $module));
			}
			...
			$parentRecords = $adb->pquery("SELECT crmid, module FROM ncrm_crmentityrel WHERE relcrmid=? AND relmodule=?" .
					" AND crmid NOT IN (SELECT crmid FROM ncrm_crmentityrel WHERE relcrmid=? AND relmodule=?)", array($transferId, $module, $entityId, $module));
			...
			for ($i = 0; $i < $numOfRecords; $i++) {
				...
				$adb->pquery("UPDATE ncrm_crmentityrel SET relcrmid=? WHERE crmid=? AND module=? AND relcrmid=? AND relmodule=?", array($entityId, $parcrmid, $parmodule, $transferId, $module));
			}
			$adb->pquery("UPDATE ncrm_modcomments SET related_to = ? WHERE related_to = ?", array($entityId, $transferId));
		}
	...
	function generateReportsQuery($module, $queryPlanner) {
		...
		$fields_query = $adb->pquery("SELECT ncrm_field.fieldname,ncrm_field.tablename,ncrm_field.fieldid from ncrm_field INNER JOIN ncrm_tab on ncrm_tab.name = ? WHERE ncrm_tab.tabid=ncrm_field.tabid AND ncrm_field.uitype IN (10) and ncrm_field.presence in (0,2)", array($module));
		...
		if ($adb->num_rows($fields_query) > 0) {
			for ($i = 0; $i < $adb->num_rows($fields_query); $i++) {
				...
				$ui10_modules_query = $adb->pquery("SELECT relmodule FROM ncrm_fieldmodulerel WHERE fieldid=?", array($field_id));

				if ($adb->num_rows($ui10_modules_query) > 0) {

					// Capture the forward table dependencies due to dynamic related-field
					$crmentityRelModuleFieldTable = "ncrm_crmentityRel$module$field_id";
					...
					if ($queryPlanner->requireTable($crmentityRelModuleFieldTable, $matrix)) {
						$relquery.= " left join ncrm_crmentity as $crmentityRelModuleFieldTable on $crmentityRelModuleFieldTable.crmid = $tab_name.$field_name and ncrm_crmentityRel$module$field_id.deleted=0";
					}
		...
		$query = "from $moduletable inner join ncrm_crmentity on ncrm_crmentity.crmid=$moduletable.$moduleindex";
	...
	function generateReportsSecQuery($module, $secmodule,$queryPlanner) {
		...
		$fields_query = $adb->pquery("SELECT ncrm_field.fieldname,ncrm_field.tablename,ncrm_field.fieldid from ncrm_field INNER JOIN ncrm_tab on ncrm_tab.name = ? WHERE ncrm_tab.tabid=ncrm_field.tabid AND ncrm_field.uitype IN (10) and ncrm_field.presence in (0,2)", array($secmodule));
		...
		if ($adb->num_rows($fields_query) > 0) {
			for ($i = 0; $i < $adb->num_rows($fields_query); $i++) {
				...
				if ($adb->num_rows($ui10_modules_query) > 0) {
					// Capture the forward table dependencies due to dynamic related-field
					$crmentityRelSecModuleTable = "ncrm_crmentityRel$secmodule$field_id";
					...
					if ($queryPlanner->requireTable($crmentityRelSecModuleTable, $matrix)) {
						$relquery .= " left join ncrm_crmentity as $crmentityRelSecModuleTable on $crmentityRelSecModuleTable.crmid = $tab_name.$field_name and $crmentityRelSecModuleTable.deleted=0";
					}
					for ($j = 0; $j < $adb->num_rows($ui10_modules_query); $j++) {
						$rel_mod = $adb->query_result($ui10_modules_query, $j, 'relmodule');
						$rel_obj = CRMEntity::getInstance($rel_mod);
		...
		$matrix->setDependency("ncrm_crmentity$secmodule", array("ncrm_groups$secmodule", "ncrm_users$secmodule", "ncrm_lastModifiedBy$secmodule"));
		$matrix->addDependency($tablename, "ncrm_crmentity$secmodule");
		...
		if ($queryPlanner->requireTable("ncrm_crmentity$secmodule", $matrix)) {
			$query .= " left join ncrm_crmentity as ncrm_crmentity$secmodule on ncrm_crmentity$secmodule.crmid = $tablename.$tableindex AND ncrm_crmentity$secmodule.deleted=0";
		}

		// Add the pre-joined custom table query
		$query .= " ".$cfquery;

		if ($queryPlanner->requireTable("ncrm_groups$secmodule")) {
			$query .= " left join ncrm_groups as ncrm_groups" . $secmodule . " on ncrm_groups" . $secmodule . ".groupid = ncrm_crmentity$secmodule.smownerid";
		}
		if ($queryPlanner->requireTable("ncrm_users$secmodule")) {
			$query .= " left join ncrm_users as ncrm_users" . $secmodule . " on ncrm_users" . $secmodule . ".id = ncrm_crmentity$secmodule.smownerid";
		}
		if ($queryPlanner->requireTable("ncrm_lastModifiedBy$secmodule")) {
			$query .= " left join ncrm_users as ncrm_lastModifiedBy" . $secmodule . " on ncrm_lastModifiedBy" . $secmodule . ".id = ncrm_crmentity" . $secmodule . ".modifiedby";
		}
	...
	function getListViewSecurityParameter($module) {
		...
		if($is_admin==false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2] == 1
			...
			$sec_query .= " and (ncrm_crmentity.smownerid in($current_user->id) or ncrm_crmentity.smownerid
					in (select ncrm_user2role.userid from ncrm_user2role
							...
							where ncrm_role.parentrole like '" . $current_user_parent_role_seq . "::%') or ncrm_crmentity.smownerid
					in(select shareduserid from ncrm_tmp_read_user_sharing_per
						where userid=" . $current_user->id . " and tabid=" . $tabid . ") or (";
			if (sizeof($current_user_groups) > 0) {
				$sec_query .= " ncrm_groups.groupid in (" . implode(",", $current_user_groups) . ") or ";
			}
			$sec_query .= " ncrm_groups.groupid in(select ncrm_tmp_read_group_sharing_per.sharedgroupid
						from ncrm_tmp_read_group_sharing_per where userid=" . $current_user->id . " and tabid=" . $tabid . "))) ";
		}
		return $sec_query;
	}
	...
	function getRelationQuery($module, $secmodule, $table_name, $column_name, $queryPlanner) {
		...
		$secQuery = "select $table_name.* from $table_name inner join ncrm_crmentity on " .
				"ncrm_crmentity.crmid=$table_name.$column_name and ncrm_crmentity.deleted=0";

		$secQueryTempTableQuery = $queryPlanner->registerTempTable($secQuery, array($column_name, $fields[1], $prifieldname));

		$query = '';
		if ($pritablename == 'ncrm_crmentityrel') {
			$condition = "($table_name.$column_name={$tmpname}.{$secfieldname} " .
					"OR $table_name.$column_name={$tmpname}.{$prifieldname})";
			$query = " left join ncrm_crmentityrel as $tmpname ON ($condvalue={$tmpname}.{$secfieldname} " .
			...
			$condition = "$table_name.$column_name=$tmpname.$secfieldname";
            if($pritablename == 'ncrm_seactivityrel') {
                ...
                $query = " left join $pritablename as $tmpname ON ($sectablename.$sectableindex=$tmpname.$prifieldname
                    AND $tmpname.activityid IN (SELECT crmid FROM ncrm_crmentity WHERE setype='$tmpModule' AND deleted = 0))";
            } else if($pritablename == 'ncrm_senotesrel') {
                    $query = " left join $pritablename as $tmpname ON ($sectablename.$sectableindex=$tmpname.$prifieldname
                    AND $tmpname.notesid IN (SELECT crmid FROM ncrm_crmentity WHERE setype='Documents' AND deleted = 0))";
            } else if($pritablename == 'ncrm_inventoryproductrel' && ($module =="Products" || $module =="Services") && ($secmodule == "Invoice" || $secmodule == "SalesOrder" || $secmodule == "PurchaseOrder" || $secmodule == "Quotes")) {
                /** In ncrm_inventoryproductrel table, we'll have same product related to quotes/invoice/salesorder/purchaseorder
                ...
                $query = " left join $pritablename as $tmpname ON ($sectablename.$sectableindex=$tmpname.$prifieldname AND $tmpname.id in 
                        (select crmid from ncrm_crmentity where setype='$secmodule' and deleted=0))";
            } else {
                $query = " left join $pritablename as $tmpname ON ($sectablename.$sectableindex=$tmpname.$prifieldname)";
            }
            ...
            // To get all the Contacts from ncrm_contpotentialrel table
            $condition .= " OR $table_name.potentialid = ncrm_contpotentialrel.potentialid";
            $query .= " left join ncrm_contpotentialrel on  ncrm_contpotentialrel.contactid = ncrm_contactdetails.contactid";
        }else if($module == "Potentials" && $secmodule == "Contacts"){
            // To get all the Potentials from ncrm_contpotentialrel table
            $condition .= " OR $table_name.contactid = ncrm_contpotentialrel.contactid";
            $query .= " left join ncrm_contpotentialrel on ncrm_potential.potentialid = ncrm_contpotentialrel.potentialid";
        }
	...
	function add_related_to($module, $fieldname) {
		...
		$query = "SELECT ncrm_crmentity.deleted, $focus1->table_name.*
					FROM $focus1->table_name
					INNER JOIN ncrm_crmentity ON ncrm_crmentity.crmid=$focus1->table_name.$focus1->table_index
						where $entityName=? and ncrm_crmentity.deleted=0";
	...
	function filterInactiveFields($module) {
		...
		if ($cachedModuleFields) {
			foreach ($cachedModuleFields as $fieldinfo) {
				...
				$tableName = str_replace("ncrm_", "", $fieldinfo['tablename']);
	...
	function buildSearchQueryForFieldTypes($uitypes, $value=false) {
		...
		$query .=" INNER JOIN ncrm_crmentity ON $this->table_name.$this->table_index = ncrm_crmentity.crmid AND deleted = 0 ";
		...
		if($this->table_name == 'ncrm_leaddetails') {
	...
	function getNonAdminUserAccessQuery($user, $parentRole, $userGroups) {
		$query = "(SELECT $user->id as id) UNION (SELECT ncrm_user2role.userid AS userid FROM " .
				"ncrm_user2role INNER JOIN ncrm_users ON ncrm_users.id=ncrm_user2role.userid " .
				"INNER JOIN ncrm_role ON ncrm_role.roleid=ncrm_user2role.roleid WHERE " .
				"ncrm_role.parentrole like '$parentRole::%')";
		if (count($userGroups) > 0) {
			$query .= " UNION (SELECT groupid FROM ncrm_groups where" .
	...
	function getNonAdminModuleAccessQuery($module, $user) {
		...
		if (!empty($sharingRuleInfo) && (count($sharingRuleInfo['ROLE']) > 0 ||
			...
			$query = " (SELECT shareduserid FROM ncrm_tmp_read_user_sharing_per " .
					"WHERE userid=$user->id AND tabid=$tabId) UNION (SELECT " .
					"ncrm_tmp_read_group_sharing_per.sharedgroupid FROM " .
					"ncrm_tmp_read_group_sharing_per WHERE userid=$user->id AND tabid=$tabId)";
		}
		return $query;
	}
		...
		if ($is_admin == false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2]
            ...
            // for secondary module we should join the records even if record is not there(primary module without related record)
                if($scope == ''){
                    ...
                    $query = " INNER JOIN $tableName $tableName$scope ON $tableName$scope.id = " .
                            "ncrm_crmentity$scope.smownerid OR ncrm_crmentity$scope.smownerid IS NULL";
                }
            }
		return $query;
	}
	...
	function setRelationTables($secmodule) {
		$rel_tables = array(
			"Documents" => array("ncrm_senotesrel" => array("crmid", "notesid"),
	...
	function trackLinkedInfo($module, $crmid, $with_module, $with_crmid) {
		...
		$adb->pquery('UPDATE ncrm_crmentity SET modifiedtime = ?, modifiedby = ? WHERE crmid = ?', array($currentTime, $current_user->id, $crmid));
