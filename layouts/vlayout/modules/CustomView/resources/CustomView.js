...
var Ncrm_CustomView_Js = {
	...
	loadFilterView : function(url) {
		...
		AppConnector.request(url).then(
			function(data){
				...
				Ncrm_CustomView_Js.registerEvents();
				Ncrm_CustomView_Js.advanceFilterInstance = Ncrm_AdvanceFilter_Js.getInstance(jQuery('.filterContainer',contents));
			},
			function(error,err){
	...
	getContentsContainer : function() {
		if(Ncrm_CustomView_Js.contentsCotainer == false) {
			Ncrm_CustomView_Js.contentsCotainer = jQuery('div.contentsDiv');
		}
		return Ncrm_CustomView_Js.contentsCotainer;
	},

	getColumnListSelect2Element : function() {
		return Ncrm_CustomView_Js.columnListSelect2Element;
	},
	...
	getColumnSelectElement : function() {
		if(Ncrm_CustomView_Js.columnSelectElement == false) {
			Ncrm_CustomView_Js.columnSelectElement = jQuery('#viewColumnsSelect');
		}
		return Ncrm_CustomView_Js.columnSelectElement;
	},
	...
	getSelectedColumnsList : function() {
		if(Ncrm_CustomView_Js.selectedColumnsList == false) {
			Ncrm_CustomView_Js.selectedColumnsList = jQuery('#selectedColumnsList');
		}
		return Ncrm_CustomView_Js.selectedColumnsList;
	},
	...
	makeColumnListSortable : function() {
		var select2Element = Ncrm_CustomView_Js.getColumnListSelect2Element();
		...
		chozenChoiceElement.sortable({
                ...
                start: function() { Ncrm_CustomView_Js.getSelectedColumnsList().select2("onSortStart"); },
                update: function() { Ncrm_CustomView_Js.getSelectedColumnsList().select2("onSortEnd"); }
            });
	},
	...
	getSelectedColumns : function() {
		var columnListSelectElement = Ncrm_CustomView_Js.getColumnSelectElement();
		var select2Element = Ncrm_CustomView_Js.getColumnListSelect2Element();
	...
	arrangeSelectChoicesInOrder : function() {
		var contentsContainer = Ncrm_CustomView_Js.getContentsContainer();
		var chosenElement = Ncrm_CustomView_Js.getColumnListSelect2Element();
		var choicesContainer = chosenElement.find('ul.select2-choices');
		var choicesList = choicesContainer.find('li.select2-search-choice');
		var coulmnListSelectElement = Ncrm_CustomView_Js.getColumnSelectElement();
	...
	saveAndViewFilter : function(){
		Ncrm_CustomView_Js.saveFilter().then(
			function(response){
				if (response.success) {
					...
					Ncrm_Helper_Js.showPnotify(params);
				}
			},
			function(error) {
	...
	registerSelect2ElementForColumnsSelection : function() {
		var selectElement = Ncrm_CustomView_Js.getColumnSelectElement();
		app.changeSelectElementView(selectElement, 'select2', {maximumSelectionSize: 12,dropdownCss : {'z-index' : 0}});
	},

	registerEvents: function(){
		Ncrm_CustomView_Js.registerSelect2ElementForColumnsSelection();
		var contentsContainer = Ncrm_CustomView_Js.getContentsContainer();
		jQuery('.stndrdFilterDateSelect').datepicker();
		jQuery('.chzn-select').chosen();

		var select2Element = app.getSelect2ElementFromSelect(Ncrm_CustomView_Js.getColumnSelectElement());
		Ncrm_CustomView_Js.columnListSelect2Element = select2Element;

		//To arrange the chosen choices in the order that is selected
		Ncrm_CustomView_Js.arrangeSelectChoicesInOrder();

		jQuery("#standardDateFilter").change(function(){
			Ncrm_CustomView_Js.loadDateFilterValues();
		});

		Ncrm_CustomView_Js.makeColumnListSortable();

		jQuery("#CustomView").submit(function(e) {
			var selectElement = Ncrm_CustomView_Js.getColumnSelectElement();
			var select2Element = app.getSelect2ElementFromSelect(selectElement);
			var result = Ncrm_MultiSelect_Validator_Js.invokeValidation(selectElement);
			if(result != true){
			...
			if(result == true){
				...
				var advfilterlist = Ncrm_CustomView_Js.advanceFilterInstance.getValues();
				jQuery('#advfilterlist').val(JSON.stringify(advfilterlist));
				jQuery('input[name="columnslist"]', contentsContainer).val(JSON.stringify(Ncrm_CustomView_Js.getSelectedColumns()));
				Ncrm_CustomView_Js.saveAndViewFilter();
				return false;
			} else {
