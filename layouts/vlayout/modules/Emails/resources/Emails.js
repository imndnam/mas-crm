...
//		return aDeferred.promise();
	...
	//configured ckeditor toolbar for ncrm
	var Ncrm_ckeditor_toolbar = 
	[
	...
	getComposeEmailForm : function(form){
		...
		AppConnector.request(massSendEmailUrl).then(
			function(data) {
				...
				Ncrm_Emails_Js.loadCkEditor();
				Ncrm_Emails_Js.registerEvents();
			},
			function(error,err){
	...
	registerComposeEmailStep1Events : function(){
		jQuery('.SendEmailFormStep1').on('submit',"#SendEmailFormStep1",function(e){
			...
			var result = Ncrm_MultiSelect_Validator_Js.invokeValidation(chosenElement);
			if(result == true){
				Ncrm_Emails_Js.getComposeEmailForm(form);
			} else {
	...
	registerEvents : function(){
		...
		composeEmailContainer.on('submit','#SendEmailFormStep2',function(e){
			var form = jQuery(e.currentTarget);
			Ncrm_Emails_Js.SendEmailStep2(form);
			e.preventDefault();
		});
