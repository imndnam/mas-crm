...
Ncrm_Detail_Js("Leads_Detail_Js",{
	...
	displayConvertLeadModel : function(data, buttonElement) {
		...
		if(errorElement.length != '0') {
			...
			var callBackFunction = function(data){
				var editViewObj = Ncrm_Edit_Js.getInstance();
	...
	registerConvertLeadEvents : function() {
		...
		container.on('hidden', '.accordion-body', function(e){
			...
			currentTarget.closest('.convertLeadModules').find('.iconArrow').removeClass('fa-chevron-up').addClass('fa-chevron-down');
		}).on('shown', '.accordion-body', function(e){
			var currentTarget = jQuery(e.currentTarget);
			currentTarget.closest('.convertLeadModules').find('.iconArrow').removeClass('fa-chevron-down').addClass('fa-chevron-up');
		});
    ...
    updateConvertLeadvalue : function() {
		var thisInstance = Ncrm_Detail_Js.getInstance();
