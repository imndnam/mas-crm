...
Reports_Detail_Js("Reports_ChartDetail_Js",{
	...
	registerEventForChartGeneration : function() {
		...
		try {
			...
			jQuery('#chartcontent').trigger(Ncrm_Widget_Js.widgetPostLoadEvent);
		} catch(error) {
			console.log("error");
			console.log(error);
	...
	registerEventForModifyCondition : function() {
		jQuery('button[name=modify_condition]').on('click', function() {
			...
			if(classValue == 'icon-chevron-right') {
				icon.removeClass('icon-chevron-right').addClass('fa-chevron-down');
				filter.show('slow');
			} else {
				icon.removeClass('fa-chevron-down').addClass('icon-chevron-right');
				filter.hide('slow');
			}
			return false;
		});
...
Ncrm_Pie_Widget_Js('Report_Piechart_Js',{},{
...
Ncrm_Barchat_Widget_Js('Report_Verticalbarchart_Js', {},{
