		...
		<div class="listViewActionsDiv row-fluid">
			<span class="btn-toolbar span4">
				...
				{foreach item=LISTVIEW_BASICACTION from=$LISTVIEW_LINKS['LISTVIEWBASIC']}
						...
						<span class="btn-group">
							...
							<ul class="dropdown-menu">
								{foreach item="childLink" from=$childLinks}
									<li id="{$MODULE}_listView_basicAction_{Ncrm_Util_Helper::replaceSpaceWithUnderScores($childLink->getLabel())}">
					...
					{else}
						<span class="btn-group">
							<button id="{$MODULE}_listView_basicAction_{Ncrm_Util_Helper::replaceSpaceWithUnderScores($LISTVIEW_BASICACTION->getLabel())}" class="btn addButton" {if stripos($LISTVIEW_BASICACTION->getUrl(), 'javascript:')===0}onclick='{$LISTVIEW_BASICACTION->getUrl()|substr:strlen("javascript:")};'{else} onclick='window.location.href="{$LISTVIEW_BASICACTION->getUrl()}"'{/if}><i class="icon-plus"></i>&nbsp;<strong>{vtranslate($LISTVIEW_BASICACTION->getLabel(), $MODULE)}</strong></button>
						</span>
					{/if}
				{/foreach}
			</span>
