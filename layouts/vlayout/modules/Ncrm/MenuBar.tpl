...
{strip}
	...
	<div class="navbar" id="topMenus" style="overflow: hidden; height: 50px;">
			...
			<div class="menuBar row-fluid">
				...
				<div class="span9">
					<ul class="nav modulesList" id="largeNav">
						<li class="tabs">
							<a class="alignMiddle {if $MODULE eq 'Home'} selected {/if}" href="{$HOME_MODULE_MODEL->getDefaultUrl()}">
					...
					<ul class="nav" id="commonMoreMenu">
						<li class="dropdown" id="moreMenu">
							<a class="dropdown-toggle" data-toggle="dropdown" href="#moreMenu">
                                {*<strong>{vtranslate('LBL_MORE',$MODULE)}&nbsp;</strong>*}
								<div class="more-dots clearfix">
							...
							<div class="dropdown-menu moreMenus" {if ($NUMBER_OF_PARENT_TABS <= 2) && ($NUMBER_OF_PARENT_TABS != 0)}style="width: 30em;"{elseif $NUMBER_OF_PARENT_TABS == 0}style="width: 10em;"{/if}>
									...
									{if $smarty.foreach.more.index % 4 == 0}
										...
										<span class="{$SPAN_CLASS}">
											<strong class="group-name">
												<span class="fa fa-{if $parent eq 'MARKETING_AND_SALES'}shopping-bag{elseif $parent eq 'SUPPORT'}life-ring{elseif $parent eq 'INVENTORY'}cubes{elseif $parent eq 'TOOLS'}wrench{/if}"></span>
												&nbsp;{vtranslate("LBL_$parent",$moduleName)}
											</strong>{*<hr>*}
											{foreach key=moduleName item=moduleModel from=$moduleList}
												{assign var='translatedModuleLabel' value=vtranslate($moduleModel->get('label'),$moduleName)}
								...
								{if $USER_MODEL->isAdminUser()}
									<div class="config-menu">
										<a id="menubar_item_moduleManager"
										   href="index.php?module=MenuEditor&parent=Settings&view=Index"
										   class="pull-right" title="Customize modules">
											<span class="fa fa-cog"></span>&nbsp;
											{*{vtranslate('LBL_CUSTOMIZE_MAIN_MENU',$MODULE)}*}
										</a>
									</div>
									{*<div class="row-fluid">*}
				...
				<div class="span3 marginLeftZero pull-right" id="headerLinks">
					<span id="headerLinksBig" class="pull-right headerLinksContainer">
						{*<span class="dropdown span settingIcons">
							...
							<ul class="dropdown-menu themeMenuContainer">
								<div id="themeContainer">
									{assign var=COUNTER value=0}
									{assign var=THEMES_LIST value=Ncrm_Theme::getAllSkins()}
						...
						{*{foreach key=index item=obj from=$HEADER_LINKS}*}
										...
										{*{foreach key=index item=obj from=$childLinks}*}
											{*{if $obj->getLabel() eq NULL}*}
												...
												{*{else}*}
													...
													{*{/if}*}
												{*<li>*}
														{*<a target="{$obj->target}" id="menubar_item_right_{Ncrm_Util_Helper::replaceSpaceWithUnderScores($label)}" {if $label=='Switch to old look'}switchLook{/if} href="{$href}" {$onclick}>{vtranslate($label,$MODULE)}</a>*}
												{*</li>*}
											{*{/if}*}
										{*{/foreach}*}
									{*</ul>*}
							...
							{*</span>*}
						<div class="span">
							<a class="dropdown-toggle enable-search" data-toggle="dropdown" href="#" title="Search...">
								<span class="fa fa-search"></span>
							</a>
							<div class="navbar-search pull-right">
								<input type="text" class="search-query" id="globalSearchValue" placeholder="Search...">
								<a class="close-search"><span class="fa fa-times"></span></a>
							</div>
						</div>

						<span class="dropdown span quick-create-icon">
							<a class="dropdown-toggle" data-toggle="dropdown" href="#" id="menubar_quickCreate" title="Quick Create">
								<span class="fa fa-plus"></span>
							</a>
							<ul class="dropdown-menu dropdownStyles pull-right" id="quickCreateModules">
								{foreach key=moduleName item=moduleModel from=$MENUS}
                                    {if $moduleModel->isPermitted('EditView')}
                                        ...
                                        {if $quickCreateModule == '1'}
											<li><a id="menubar_quickCreate_{$moduleModel->getName()}"
												   class="quickCreateModule" href="javascript:void(0)"
												   data-name="{$moduleModel->getName()}"
												   data-url="{$moduleModel->getQuickCreateUrl()}">
													{vtranslate($singularLabel,$moduleName)}</a></li>
                                        {/if}
                                    {/if}
                                {/foreach}
							</ul>
						...
						<span class="dropdown span settingIcons">
							...
							<ul class="dropdown-menu dropdownStyles pull-right config-menu">
								{if $USER_MODEL->isAdminUser()}
									<li><a href="?module=Ncrm&parent=Settings&view=Index">Setup</a></li>
									<li><a href="?module=Users&parent=Settings&view=List">Manage users</a></li>
                                {/if}
								{*<li role="separator" class="divider"></li>*}
								<li class="profile">
									{assign var="profileImages" value=$USER_MODEL->getImageDetails()}
                                    {assign var="profileImage" value=$profileImages[0]}
									<div class="profile-item-container clearfix">
										<div class="profile-picture"
											 style="background-image: url({$profileImage.path}_{$profileImage.orgname});">
										</div>
										<div class="profile-actions">
											<strong class="profile-username">{$USER_MODEL->getDisplayName()}</strong>
											<div class="actions">
												<a class="settings" href="{$USER_MODEL->getPreferenceDetailViewUrl()}">Settings</a>
												<a class="logout" href="?module=Users&parent=Settings&action=Logout">Log out</a>
											</div>
										</div>
									</div>
								</li>
