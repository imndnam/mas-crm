...
{strip}
    <div class="quickLinksDiv">
        {foreach item=SIDEBARLINK from=$QUICK_LINKS['SIDEBARLINK']}
            ...
            {if $SIDEBARLINK->getLabel() eq 'LBL_RECORDS_LIST'}
                {if $CURRENT_LINK_VIEW neq 'view=List'}
                    <p onclick="window.location.href='{$SIDEBARLINK->getUrl()}'"
                       id="{$MODULE}_sideBar_link_{Ncrm_Util_Helper::replaceSpaceWithUnderScores($SIDEBARLINK->getLabel())}"
                       class="{if $LINK_LIST_VIEW and $IS_LINK_MODULE_NAME}selectedQuickLink {else}unSelectedQuickLink{/if}">
                        <a class="quickLinks back-to-list" href="{$SIDEBARLINK->getUrl()}">
                            <span class="fa fa-long-arrow-left"></span> Back to list
                        </a>
                    </p>
                {/if}
            {else}
