...
var Ncrm_Index_Js = {
	...
	registerWidgetsEvents : function() {
		...
		widgets.on({
				shown: function(e) {
					...
					Ncrm_Index_Js.loadWidgets(widgetContainer);
	...
	loadWidgets : function(widgetContainer, open) {
		...
		AppConnector.request(listViewWidgetParams).then(
			...
			if(typeof open == 'undefined') open = true;
				...
				jQuery('.bodyContents').trigger('Ncrm.Widget.Load.'+label,jQuery(widgetContainer));
			}
		);
	},
	...
	showComposeEmailPopup : function(params, cb){
		...
		Ncrm_Helper_Js.checkServerConfig(currentModule).then(function(data){
			if(data == true){
	...
	registerActivityReminder : function() {
		...
		if(activityReminder != '') {
			...
			if((currentTime + activityReminder) > nextActivityReminderCheck) {
				Ncrm_Index_Js.requestReminder();
				setTimeout('Ncrm_Index_Js.requestReminder()', activityReminder);
		...
		AppConnector.request(url).then(function(data){
			if(data.success && data.result) {
				for(i=0; i< data.result.length; i++) {
					...
					Ncrm_Index_Js.showReminderPopup(record);
				}
			}
		});
	},
	...
	showReminderPopup : function(record) {
		...
		var notify = Ncrm_Helper_Js.showPnotify(params);
	...
	registerResizeEvent: function(){
		...
		$(window).bind('resizeEnd', function() {
			Ncrm_Index_Js.adjustTopMenuBarItems();
		});
	},
	/**
	...
	registerTooltipEvents: function() {
		...
		function registerToolTipDestroy() {
			jQuery('button[name="vtTooltipClose"]').on('click', function(e){
				...
				lastPopover.popover('hide');
				// Fix suggested http://code.ncrm.com/ncrm/ncrmcrm/issues/43
	...
	registerShowHideLeftPanelEvent : function() {
		jQuery('#toggleButton').click(function(e){
			...
			if (leftPanel.attr('class').indexOf(' hide') == -1) {
				...
				tButtonImage.removeClass('fa-chevron-left').addClass("fa-chevron-right");
			} else {
                var leftPanelshow = 0;
				leftPanel.removeClass('hide');
				rightPanel.removeClass('span12').addClass('span10');
				tButtonImage.removeClass('fa-chevron-right').addClass("fa-chevron-left");
			}
            var params = {
                'module' : 'Users',
                'action' : 'IndexAjax',
	...
	registerEvents : function(){
		Ncrm_Index_Js.registerWidgetsEvents();
		Ncrm_Index_Js.loadWidgetsOnLoad();
		Ncrm_Index_Js.registerActivityReminder();
		Ncrm_Index_Js.adjustTopMenuBarItems();
		Ncrm_Index_Js.registerPostAjaxEvents();
		Ncrm_Index_Js.changeSkin();
		Ncrm_Index_Js.registerShowHideLeftPanelEvent();
		Ncrm_Index_Js.registerResizeEvent();
	},

	registerPostAjaxEvents: function() {
		Ncrm_Index_Js.registerTooltipEvents();
	}
}
