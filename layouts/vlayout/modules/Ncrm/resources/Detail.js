...
jQuery.Class("Ncrm_Detail_Js",{

    detailInstance : false,

	getInstance: function(){
        if( Ncrm_Detail_Js.detailInstance == false ){
            var module = app.getModuleName();
            var view = app.getViewName();
            var moduleClassName = module+"_"+view+"_Js";
            var fallbackClassName = Ncrm_Detail_Js;
            if(typeof window[moduleClassName] != 'undefined'){
                var instance = new window[moduleClassName]();
            }else{
                var instance = new fallbackClassName();
            }
            Ncrm_Detail_Js.detailInstance = instance;
        }
        return Ncrm_Detail_Js.detailInstance;
	},
	...
	triggerSendEmail : function(detailActionUrl, module){
        Ncrm_Helper_Js.checkServerConfig(module).then(function(data){
			if(data == true){
                var currentInstance = Ncrm_Detail_Js.getInstance();
                ...
                Ncrm_Index_Js.showComposeEmailPopup(params);
			} else {
    ...
    triggerDetailViewAction : function(detailActionUrl, callBackFunction){
		var detailInstance = Ncrm_Detail_Js.getInstance();
    ...
    triggerSendSms : function(detailActionUrl, module) {
        Ncrm_Helper_Js.checkServerConfig(module).then(function(data){
			if(data == true){
                Ncrm_Detail_Js.triggerDetailViewAction(detailActionUrl);
			} else {
    ...
    deleteRecord : function(deleteRecordActionUrl) {
		...
		Ncrm_Helper_Js.showConfirmationBox({'message' : message}).then(function(data) {
				AppConnector.request(deleteRecordActionUrl+'&ajaxDelete=true').then(
				function(data){
					if(data.success == true){
						window.location.href = data.result;
					}else{
						Ncrm_Helper_Js.showPnotify(data.error.message);
					}
				});
			},
			function(error, err){
	...
	fieldUpdatedEvent : 'Ncrm.Field.Updated',
	widgetPostLoad : 'Ncrm.Widget.PostLoad',
	...
	fieldPreSave : 'Ncrm.Field.PreSave',
	...
	loadRelatedList : function(pageNumber){
		var relatedListInstance = new Ncrm_RelatedList_Js(this.getRecordId(), app.getModuleName(), this.getSelectedTab(), this.getRelatedModuleName());
	...
	registerEventForRelatedList : function(){
		...
		detailContentsHolder.on('click', 'a.relationDelete', function(e){
			e.stopImmediatePropagation();
			var element = jQuery(e.currentTarget);
			var instance = Ncrm_Detail_Js.getInstance();
	...
	ajaxEditHandling : function(currentTdElement) {
			...
			var saveHandler = function(e) {
                ...
                var fieldInfo = Ncrm_Field_Js.getInstance(fieldElement.data('fieldinfo'));
                ...
                if((""+previousValue) == (""+ajaxEditNewValue)) { // Normalize(99!="099") Fix http://code.ncrm.com/ncrm/ncrmcrm/issues/16 
			...
			var callbackFunction = function() {
				...
				if(previousValue == ajaxEditNewValue) {
					...
					currentTarget.show();
                                        ...
                                        //If validation fails  
                                        if(errorExists) {  
                                                Ncrm_Helper_Js.addClickOutSideEvent(currentDiv, callbackFunction);
                                                                return;   
                                        }
	...
	addRelationBetweenRecords : function(relatedModule, relatedModuleRecordId){
		...
		var thisInstance = this;
		var selectedTabElement = thisInstance.getSelectedTab();
		var relatedController = new Ncrm_RelatedList_Js(thisInstance.getRecordId(), app.getModuleName(), selectedTabElement, relatedModule);
	...
	registerPostTagCloudWidgetLoad : function() {
		var thisInstance = this;
		app.getContentsContainer().on('Ncrm.Widget.Load.LBL_TAG_CLOUD',function(e,data){
	...
	registerEvents : function(){
		...
		detailContentsHolder.on('click', '.relatedPopup', function(e){
			var editViewObj = new Ncrm_Edit_Js();
