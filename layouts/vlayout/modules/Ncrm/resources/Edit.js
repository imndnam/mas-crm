...
jQuery.Class("Ncrm_Edit_Js",{

	//Event that will triggered when reference field is selected
	referenceSelectionEvent : 'Ncrm.Reference.Selection',

	//Event that will triggered when reference field is selected
	referenceDeSelectionEvent : 'Ncrm.Reference.DeSelection',

	//Event that will triggered before saving the record
	recordPreSave : 'Ncrm.Record.PreSave',

	refrenceMultiSelectionEvent : 'Ncrm.MultiReference.Selection',

	preReferencePopUpOpenEvent : 'Ncrm.Referece.Popup.Pre',

	editInstance : false,

    postReferenceSelectionEvent: 'Ncrm.PostReference.Selection',
	...
	getInstanceByModuleName : function(moduleName){
		...
		if(parentModule == 'Settings'){
			...
			var fallbackClassName = parentModule+"_Ncrm_Edit_Js";
			if(typeof window[fallbackClassName] == 'undefined') {
				fallbackClassName = "Ncrm_Edit_Js";
			}
		} else {
			moduleClassName = moduleName+"_Edit_Js";
			fallbackClassName = "Ncrm_Edit_Js";
	...
	getInstance: function(){
		if(Ncrm_Edit_Js.editInstance == false){
			var instance = Ncrm_Edit_Js.getInstanceByModuleName();
			Ncrm_Edit_Js.editInstance = instance;
			return instance;
		}
		return Ncrm_Edit_Js.editInstance;
	}
	...
	openPopUp : function(e){
		...
		var prePopupOpenEvent = jQuery.Event(Ncrm_Edit_Js.preReferencePopUpOpenEvent);
		...
		popupInstance.show(params,function(data){
			...
			if(isMultiple) {
                    sourceFieldElement.trigger(Ncrm_Edit_Js.refrenceMultiSelectionEvent,{'data':dataList});
			}
                sourceFieldElement.trigger(Ncrm_Edit_Js.postReferenceSelectionEvent,{'data':responseData});
		});
	},
	...
	setReferenceFieldValue : function(container, params) {
		...
		fieldElement.trigger(Ncrm_Edit_Js.referenceSelectionEvent, {'source_module' : popupReferenceModule, 'record' : id, 'selectedName' : selectedName});
	...
	registerAutoCompleteFields : function(container) {
		...
		container.find('input.autoComplete').autocomplete({
			...
			'select' : function(event, ui ){
                ...
                fieldElement.trigger(Ncrm_Edit_Js.postReferenceSelectionEvent,{'data':selectedItemData});
			},
	...
	registerClearReferenceSelectionEvent : function(container) {
		container.find('.clearReferenceSelection').on('click', function(e){
			...
			element.trigger(Ncrm_Edit_Js.referenceDeSelectionEvent);
			e.preventDefault();
		})
	},
	...
	referenceCreateHandler : function(container) {
		...
		if(quickCreateNode.length <= 0) {
			Ncrm_Helper_Js.showPnotify(app.vtranslate('JS_NO_CREATE_OR_NOT_QUICK_CREATE_ENABLED'))
		}
	...
	registerEventForImageDelete : function(){
		...
		formElement.find('.imageDelete').on('click',function(e){
			...
			if(exisitingImages.length < 1 && mandatoryStatus){
				...
				imageUploadElement.attr('data-validation-engine','validate[required,funcCall[Ncrm_Base_Validator_Js.invokeValidation]]');
		...
		editViewForm.submit(function(e){
			...
			if(typeof editViewForm.data('submit') != "undefined") {
				...
				if(editViewForm.validationEngine('validate')) {
					...
					var recordPreSaveEvent = jQuery.Event(Ncrm_Edit_Js.recordPreSave);
	...
	registerRecordAccessCheckEvent : function(form) {
		...
		form.on(Ncrm_Edit_Js.recordPreSave, function(e, data) {
