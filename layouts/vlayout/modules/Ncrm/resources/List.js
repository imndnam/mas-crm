...
jQuery.Class("Ncrm_List_Js",{

	listInstance : false,

	getRelatedModulesContainer : false,

	massEditPreSave : 'Ncrm.MassEdit.PreSave',

   getInstance: function(){
		if(Ncrm_List_Js.listInstance == false){
			...
			if(parentModule == 'Settings'){
				...
				var fallbackClassName = parentModule+"_Ncrm_List_Js";
				if(typeof window[fallbackClassName] == 'undefined') {
					fallbackClassName = "Ncrm_List_Js";
				}
			} else {
				moduleClassName = module+"_List_Js";
				fallbackClassName = "Ncrm_List_Js";
			...
			Ncrm_List_Js.listInstance = instance;
			return instance;
		}
		return Ncrm_List_Js.listInstance;
	},
		/*
	...
	triggerSendEmail : function(massActionUrl, module, params){
		var listInstance = Ncrm_List_Js.getInstance();
		...
		if(validationResult != true){
			...
			Ncrm_Index_Js.showComposeEmailPopup(actionParams);
		} else {
	...
	triggerSendSms : function(massActionUrl, module){
		...
		if(validationResult != true){
			...
			Ncrm_Helper_Js.checkServerConfig(module).then(function(data){
				...
				if(data == true){
					Ncrm_List_Js.triggerMassAction(massActionUrl);
				} else {
		...
		if(validationResult != true){
			...
			Ncrm_Helper_Js.showConfirmationBox({'message' : message}).then(
				function(e) {
                    ...
                    var listViewInstance = Ncrm_List_Js.getInstance();
	...
	triggerMassEdit : function(massEditUrl) {
		...
		Ncrm_List_Js.triggerMassAction(massEditUrl, function(container){
			...
			var editInstance = Ncrm_Edit_Js.getInstance();
	...
	showDuplicateSearchForm : function(url) {
		...
		app.showModalWindow("", url, function() {
			progressIndicatorElement.progressIndicator({'mode' : 'hide'});
			Ncrm_List_Js.registerDuplicateSearchButtonEvent();
		});
	},
		...
		AppConnector.requestPjax(urlParams).then(
			function(data){
				...
				thisInstance.calculatePages().then(function(data){
					...
					Ncrm_Helper_Js.showHorizontalTopScrollBar();
	...
	massActionSave : function(form, isMassEdit){
		...
		if(isMassEdit) {
			...
			if(fieldsChanged == false){
				Ncrm_Helper_Js.showPnotify(app.vtranslate('NONE_OF_THE_FIELD_VALUES_ARE_CHANGED_IN_MASS_EDIT'));
				form.find('[name="saveButton"]').removeAttr('disabled');
				aDeferred.reject();
				return aDeferred.promise();
			}
			//on submit form trigger the massEditPreSave event
			var massEditPreSaveEvent = jQuery.Event(Ncrm_List_Js.massEditPreSave);
	...
	registerRecordAccessCheckEvent : function(form) {
		...
		form.on(Ncrm_List_Js.massEditPreSave, function(e) {
				...
				if(assignedToSelectElement.data('recordaccessconfirmation') == true) {
					...
					if(assignedToSelectElement.data('recordaccessconfirmationprogress') != true) {
						...
						if(recordAccess == false) {
							...
							Ncrm_Helper_Js.showConfirmationBox({
								'message' : message
							}).then(
								function(e) {
	...
	registerPageNavigationEvents : function(){
		...
		jQuery('#listViewPageJump').on('click',function(e){
            if(typeof Ncrm_WholeNumberGreaterThanZero_Validator_Js.invokeValidation(jQuery('#pageToJump'))!= 'undefined') {
		...
		jQuery('#listViewPageJumpDropDown').on('click','li',function(e){
			...
			if(e.which == 13){
				...
				var response = Ncrm_WholeNumberGreaterThanZero_Validator_Js.invokeValidation(element);
	...
	*/
	...
	registerCreateFilterClickEvent : function(){
		...
		jQuery('#createFilter').on('click',function(event){
			...
			Ncrm_CustomView_Js.loadFilterView(createUrl);
		});
	},
	...
	registerEditFilterClickEvent : function(){
		...
		if(listViewFilterBlock != false){
			listViewFilterBlock.on('mouseup','li i.editFilter',function(event){
				...
				Ncrm_CustomView_Js.loadFilterView(editUrl);
	...
	registerDeleteRecordClickEvent: function(){
		...
		listViewContentDiv.on('click','.deleteRecordButton',function(e){
			...
			Ncrm_List_Js.deleteRecord(recordId);
			e.stopPropagation();
		});
	},
	/*
	...
	registerReferenceFieldsForValidation : function(form){
		...
		form.find('.sourceField').on(Ncrm_Edit_Js.referenceSelectionEvent,function(e,params){
			var element = jQuery(e.currentTarget);
		...
		form.find('.clearReferenceSelection').on(Ncrm_Edit_Js.referenceDeSelectionEvent,function(e){
