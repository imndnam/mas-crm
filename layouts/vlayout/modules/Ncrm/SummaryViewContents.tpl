			...
			<tr class="summaryViewEntries">
				...
				<td class="fieldValue" style="width:65%">
                    <div class="row-fluid">
                        ...
                        {if $FIELD_MODEL->isEditable() eq 'true' && ($FIELD_MODEL->getFieldDataType()!=Ncrm_Field_Model::REFERENCE_TYPE) && $IS_AJAX_ENABLED && $FIELD_MODEL->isAjaxEditable() eq 'true' && $FIELD_MODEL->get('uitype') neq 69}
...
<div class="row-fluid">
	<div class="span4 toggleViewByMode">
		...
		<a {*type="button"*} class="{*btn *}changeDetailViewMode cursorPointer">
			<span>{vtranslate('LBL_SHOW_FULL_DETAILS',$MODULE_NAME)}</span>&nbsp;
	...
	<div class="span8">
		<div class="pull-right">
			<div>
				<p>
					<small>
						{vtranslate('LBL_CREATED_ON',$MODULE_NAME)} {Ncrm_Util_Helper::formatDateTimeIntoDayString($RECORD->get('createdtime'))}
					</small>
				</p>
			</div>
			<div>
				<p>
					<small>
						{vtranslate('LBL_MODIFIED_ON',$MODULE_NAME)} {Ncrm_Util_Helper::formatDateTimeIntoDayString($RECORD->get('modifiedtime'))}
					</small>
				</p>
			</div>
		</div>
