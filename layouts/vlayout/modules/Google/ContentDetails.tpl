...
<div class="row-fluid paddingLeftRight10px">
    {foreach from=$RECORDS item=RECORD key=type }
        {if $type eq 'ncrm'}
            <div class='row-fluid'>
                ...
                {if $RECORD['more']}
					<div class="row-fluid"><span style='background:#FFFBCF;' class="span11" title="{vtranslate('LBL_MORE_NCRM',$MODULE_NAME)}">{vtranslate('LBL_MORE_NCRM',$MODULE_NAME)}</span>
                {/if}    
            </div>
         {else}
            <div class='row-fluid'> 
    ...
    {/foreach}
    <div class='row-fluid'>
        {if $SYNCTIME}<p class="muted span12"><small title="{Ncrm_Util_Helper::formatDateTimeIntoDayString($SYNCTIME)}">{vtranslate('LBL_SYNCRONIZED',$MODULE_NAME)} : {Ncrm_Util_Helper::formatDateDiffInStrings($SYNCTIME)}</small></p>{/if}
    </div>
{if $NORECORDS}
