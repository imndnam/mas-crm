...
include_once 'vtlib/Ncrm/PDF/models/Model.php';
include_once 'vtlib/Ncrm/PDF/inventory/HeaderViewer.php';
include_once 'vtlib/Ncrm/PDF/inventory/FooterViewer.php';
include_once 'vtlib/Ncrm/PDF/inventory/ContentViewer.php';
include_once 'vtlib/Ncrm/PDF/inventory/ContentViewer2.php';
include_once 'vtlib/Ncrm/PDF/viewers/PagerViewer.php';
include_once 'vtlib/Ncrm/PDF/PDFGenerator.php';
include_once 'data/CRMEntity.php';

class Ncrm_InventoryPDFController {
	...
	function getPDFGenerator() {
		return new Ncrm_PDF_Generator();
	}

	function getContentViewer() {
		if($this->focusColumnValue('hdnTaxType') == "individual") {
			$contentViewer = new Ncrm_PDF_InventoryContentViewer();
		} else {
			$contentViewer = new Ncrm_PDF_InventoryTaxGroupContentViewer();
		}
	...
	function getHeaderViewer() {
		$headerViewer = new Ncrm_PDF_InventoryHeaderViewer();
		$headerViewer->setModel($this->buildHeaderModel());
		return $headerViewer;
	}

	function getFooterViewer() {
		$footerViewer = new Ncrm_PDF_InventoryFooterViewer();
	...
	function getPagerViewer() {
		$pagerViewer = new Ncrm_PDF_PagerViewer();
	...
	function buildContentModels() {
		...
		foreach($associated_products as $productLineItem) {
			...
			$contentModel = new Ncrm_PDF_Model();
	...
	function buildSummaryModel() {
		...
		$summaryModel = new Ncrm_PDF_Model();
	...
	function buildHeaderModel() {
		$headerModel = new Ncrm_PDF_Model();
	...
	function buildHeaderModelColumnLeft() {
		...
		// Company information
		$result = $adb->pquery("SELECT * FROM ncrm_organizationdetails", array());
	...
	function buildFooterModel() {
		$footerModel = new Ncrm_PDF_Model();
		$footerModel->set(Ncrm_PDF_InventoryFooterViewer::$DESCRIPTION_DATA_KEY, from_html($this->focusColumnValue('description')));
		$footerModel->set(Ncrm_PDF_InventoryFooterViewer::$TERMSANDCONDITION_DATA_KEY, from_html($this->focusColumnValue('terms_conditions')));
		return $footerModel;
	}

	function buildFooterLabelModel() {
		$labelModel = new Ncrm_PDF_Model();
		$labelModel->set(Ncrm_PDF_InventoryFooterViewer::$DESCRIPTION_LABEL_KEY, getTranslatedString('Description',$this->moduleName));
		$labelModel->set(Ncrm_PDF_InventoryFooterViewer::$TERMSANDCONDITION_LABEL_KEY, getTranslatedString('Terms & Conditions',$this->moduleName));
		return $labelModel;
	}
	...
	function buildWatermarkModel() {
		$watermarkModel = new Ncrm_PDF_Model();
	...
	function buildCurrencySymbol() {
		...
		if(!empty($currencyId)) {
			$result = $adb->pquery("SELECT currency_symbol FROM ncrm_currency_info WHERE id=?", array($currencyId));
