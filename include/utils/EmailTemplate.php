...
class EmailTemplate {
	...
	public function process($params) {
		...
		if (count($variableList) > 0) {
			...
			if (count($tableList) > 0 && count($columnList) > 0) {
				...
				if($module == 'Leads' && !in_array('ncrm_leaddetails', $tableList)){
					$sql .=' INNER JOIN ncrm_leaddetails ON ncrm_leaddetails.leadid = ncrm_crmentity.crmid';
				}
				...
				foreach ($it as $row) {
					foreach ($fieldList as $field) {
						     $moduleModel = Ncrm_Module_Model::getInstance($module); 
 	                         $fieldModel = Ncrm_Field_Model::getInstance($field, $moduleModel); 
	...
	public function isActive($field, $mod) {
		global $adb;
		$tabid = getTabid($mod);
		$query = 'select * from ncrm_field where fieldname = ?  and tabid = ? and presence in (0,2)';
