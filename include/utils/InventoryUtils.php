 ...
 * Param $ext_prod_arr - existing ncrm_products
...
function sendPrdStckMail($product_id,$upd_qty,$prod_name,$qtyinstk,$qty,$module)
{
	...
	{	
		...
		$query = "select * from ncrm_inventorynotification where notificationname=?";
...
function getPrdReOrderLevel($product_id)
{
	...
	global $adb;
	$query1 = "SELECT reorderlevel FROM ncrm_products WHERE productid = ?";
	$result=$adb->pquery($query1, array($product_id));
 ...
 *	return int   $taxid - taxid corresponding to the Tax type from ncrm_inventorytaxinfo ncrm_table
 */
function getTaxId($type)
{
	...
	$res = $adb->pquery("SELECT taxid FROM ncrm_inventorytaxinfo WHERE taxname=?", array($type));
 ...
 *	@param string $type       - tax type (VAT or Sales or Service)
 *	return int $taxpercentage - taxpercentage corresponding to the Tax type from ncrm_inventorytaxinfo ncrm_table
 */
function getTaxPercentage($type)
{
	...
	$res = $adb->pquery("SELECT percentage FROM ncrm_inventorytaxinfo WHERE taxname = ?", array($type));
...
function getProductTaxPercentage($type,$productid,$default='')
{
	...
	$res = $adb->pquery("SELECT taxpercentage
			FROM ncrm_inventorytaxinfo
			INNER JOIN ncrm_producttaxrel
				ON ncrm_inventorytaxinfo.taxid = ncrm_producttaxrel.taxid
			WHERE ncrm_producttaxrel.productid = ?
			AND ncrm_inventorytaxinfo.taxname = ?", array($productid, $type));
...
function addInventoryHistory($module, $id, $relatedname, $total, $history_fldval)
{
	...
	$history_table_array = Array(
					"PurchaseOrder"=>"ncrm_postatushistory",
					"SalesOrder"=>"ncrm_sostatushistory",
					"Quotes"=>"ncrm_quotestagehistory",
					"Invoice"=>"ncrm_invoicestatushistory"
				    );
...
function getAllTaxes($available='all', $sh='',$mode='',$id='')
{
	...
	$taxtypes = Array();
	if($sh != '' && $sh == 'sh') {
		$tablename = 'ncrm_shippingtaxinfo';
		$value_table='ncrm_inventoryshippingrel';
		if($mode == 'edit' && id != '') {
		...
		$tablename = 'ncrm_inventorytaxinfo';
		$value_table='ncrm_inventoryproductrel';
		if($mode == 'edit' && $id != '' ) {
...
function getTaxDetailsForProduct($productid, $available='all')
{
	...
	if($productid != '')
	{
		...
		if($available != 'all' && $available == 'available')
		{
			$where = ' and ncrm_inventorytaxinfo.deleted=0';
		}
		if($available != 'all' && $available == 'available_associated')
		{
			$query = "SELECT ncrm_producttaxrel.*, ncrm_inventorytaxinfo.* FROM ncrm_inventorytaxinfo left JOIN ncrm_producttaxrel ON ncrm_inventorytaxinfo.taxid = ncrm_producttaxrel.taxid WHERE ncrm_producttaxrel.productid = ? or ncrm_inventorytaxinfo.deleted=0 GROUP BY ncrm_inventorytaxinfo.taxid";
		}
		else
		{
			$query = "SELECT ncrm_producttaxrel.*, ncrm_inventorytaxinfo.* FROM ncrm_inventorytaxinfo INNER JOIN ncrm_producttaxrel ON ncrm_inventorytaxinfo.taxid = ncrm_producttaxrel.taxid WHERE ncrm_producttaxrel.productid = ? $where";
		}
...
function deleteInventoryProductDetails($focus)
{
	...
	$product_info = $adb->pquery("SELECT productid, quantity, sequence_no, incrementondel from ncrm_inventoryproductrel WHERE id=?",array($focus->id));
	...
	for($index = 0;$index <$numrows;$index++){
		...
		if($incrementondel){
			...
			$sub_prod_query = $adb->pquery("SELECT productid from ncrm_inventorysubproductrel WHERE id=? AND sequence_no=?",array($focus->id,$sequence_no));
    ...
    $adb->pquery("delete from ncrm_inventoryproductrel where id=?", array($focus->id));
    $adb->pquery("delete from ncrm_inventorysubproductrel where id=?", array($focus->id));
    $adb->pquery("delete from ncrm_inventoryshippingrel where id=?", array($focus->id));
...
function updateInventoryProductRel($entity) {
	...
	if($statusChanged) {
		if($recordDetails[$statusFieldName] == $statusFieldValue) {
			$adb->pquery("UPDATE ncrm_inventoryproductrel SET incrementondel=0 WHERE id=?",array($entity_id));
	...
	if($updateInventoryProductRel_deduct_stock) {
		$adb->pquery("UPDATE ncrm_inventoryproductrel SET incrementondel=1 WHERE id=?",array($entity_id));

		$product_info = $adb->pquery("SELECT productid,sequence_no, quantity from ncrm_inventoryproductrel WHERE id=?",array($entity_id));
		$numrows = $adb->num_rows($product_info);
		for($index = 0;$index <$numrows;$index++) {
			$productid = $adb->query_result($product_info,$index,'productid');
			$qty = $adb->query_result($product_info,$index,'quantity');
			...
			$sub_prod_query = $adb->pquery("SELECT productid from ncrm_inventorysubproductrel WHERE id=? AND sequence_no=?",array($entity_id,$sequence_no));
			if($adb->num_rows($sub_prod_query)>0) {
				for($j=0;$j<$adb->num_rows($sub_prod_query);$j++) {
					$sub_prod_id = $adb->query_result($sub_prod_query,$j,"productid");
...
function saveInventoryProductDetails(&$focus, $module, $update_prod_stock='false', $updateDemand='')
{
	...
	for($i=1; $i<=$tot_no_prod; $i++)
	{
		...
		/*else{
			$desc_duery = "select ncrm_crmentity.description AS product_description from ncrm_crmentity where ncrm_crmentity.crmid=?";
		...
		$query ="insert into ncrm_inventoryproductrel(id, productid, sequence_no, quantity, listprice, comment, description) values(?,?,?,?,?,?,?)";
		...
		if (!empty($sub_prod_str)) {
			...
			for($j=0;$j<count($sub_prod);$j++){
				$query ="insert into ncrm_inventorysubproductrel(id, sequence_no, productid) values(?,?,?)";
		...
		$updatequery = "update ncrm_inventoryproductrel set ";
		$updateparams = array();
	...
	//to save the S&H tax details in ncrm_inventoryshippingrel table
	...
	$sh_query = "insert into ncrm_inventoryshippingrel($sh_query_fields) values($sh_query_values)";
...
function getInventoryTaxType($module, $id)
{
	...
	$inv_table_array = Array('PurchaseOrder'=>'ncrm_purchaseorder','SalesOrder'=>'ncrm_salesorder','Quotes'=>'ncrm_quotes','Invoice'=>'ncrm_invoice');
...
function getInventoryCurrencyInfo($module, $id)
{
	...
	$res = $adb->pquery("select currency_id, $inventory_table.conversion_rate as conv_rate, ncrm_currency_info.* from $inventory_table
						inner join ncrm_currency_info on $inventory_table.currency_id = ncrm_currency_info.id
...
function getInventoryProductTaxValue($id, $productid, $taxname)
{
	...
	$res = $adb->pquery("select $taxname from ncrm_inventoryproductrel where id = ? and productid = ?", array($id, $productid));
...
function getInventorySHTaxPercent($id, $taxname)
{
	...
	$res = $adb->pquery("select $taxname from ncrm_inventoryshippingrel where id= ?", array($id));
...
function getAllCurrencies($available='available') {
	...
	$sql = "select * from ncrm_currency_info";
	if ($available != 'all') {
...
function getPriceDetailsForProduct($productid, $unit_price, $available='available', $itemtype='Products')
{
	...
	if($productid != '')
	{
		...
		if ($available == 'available_associated') {
			$query = "select ncrm_currency_info.*, ncrm_productcurrencyrel.converted_price, ncrm_productcurrencyrel.actual_price
					from ncrm_currency_info
					inner join ncrm_productcurrencyrel on ncrm_currency_info.id = ncrm_productcurrencyrel.currencyid
					where ncrm_currency_info.currency_status = 'Active' and ncrm_currency_info.deleted=0
					and ncrm_productcurrencyrel.productid = ? and ncrm_currency_info.id != ?";
			$params = array($productid, $product_currency_id);
		} else { // Edit View
			$query = "select ncrm_currency_info.*, ncrm_productcurrencyrel.converted_price, ncrm_productcurrencyrel.actual_price
					from ncrm_currency_info
					left join ncrm_productcurrencyrel
					on ncrm_currency_info.id = ncrm_productcurrencyrel.currencyid and ncrm_productcurrencyrel.productid = ?
					where ncrm_currency_info.currency_status = 'Active' and ncrm_currency_info.deleted=0";
			$params = array($productid);
		}
	...
	else
	{
		if($available == 'available') { // Create View
			...
			$query = "select ncrm_currency_info.* from ncrm_currency_info
					where ncrm_currency_info.currency_status = 'Active' and ncrm_currency_info.deleted=0";
			$params = array();
...
function getProductBaseCurrency($productid,$module='Products') {
	global $adb, $log;
	if ($module == 'Services') {
		$sql = "select currency_id from ncrm_service where serviceid=?";
	} else {
		$sql = "select currency_id from ncrm_products where productid=?";
	}
	$params = array($productid);
...
function getBaseConversionRateForProduct($productid, $mode='edit', $module='Products') {
	...
	if ($mode == 'edit') {
		if ($module == 'Services') {
			$sql = "select conversion_rate from ncrm_service inner join ncrm_currency_info
					on ncrm_service.currency_id = ncrm_currency_info.id where ncrm_service.serviceid=?";
		} else {
			$sql = "select conversion_rate from ncrm_products inner join ncrm_currency_info
					on ncrm_products.currency_id = ncrm_currency_info.id where ncrm_products.productid=?";
		}
		$params = array($productid);
	} else {
		$sql = "select conversion_rate from ncrm_currency_info where id=?";
...
function getPricesForProducts($currencyid, $product_ids, $module='Products') {
	...
	if (count($product_ids) > 0) {
		if ($module == 'Services') {
			$query = "SELECT ncrm_currency_info.id, ncrm_currency_info.conversion_rate, " .
					"ncrm_service.serviceid AS productid, ncrm_service.unit_price, " .
					"ncrm_productcurrencyrel.actual_price " .
					"FROM (ncrm_currency_info, ncrm_service) " .
					"left join ncrm_productcurrencyrel on ncrm_service.serviceid = ncrm_productcurrencyrel.productid " .
					"and ncrm_currency_info.id = ncrm_productcurrencyrel.currencyid " .
					"where ncrm_service.serviceid in (". generateQuestionMarks($product_ids) .") and ncrm_currency_info.id = ?";
		} else {
			$query = "SELECT ncrm_currency_info.id, ncrm_currency_info.conversion_rate, " .
					"ncrm_products.productid, ncrm_products.unit_price, " .
					"ncrm_productcurrencyrel.actual_price " .
					"FROM (ncrm_currency_info, ncrm_products) " .
					"left join ncrm_productcurrencyrel on ncrm_products.productid = ncrm_productcurrencyrel.productid " .
					"and ncrm_currency_info.id = ncrm_productcurrencyrel.currencyid " .
					"where ncrm_products.productid in (". generateQuestionMarks($product_ids) .") and ncrm_currency_info.id = ?";
		}
...
function getPriceBookCurrency($pricebook_id) {
	global $adb;
	$result = $adb->pquery("select currency_id from ncrm_pricebook where pricebookid=?", array($pricebook_id));
...
function deductProductsFromStock($recordId) {
	global $adb;
	$adb->pquery("UPDATE ncrm_inventoryproductrel SET incrementondel=1 WHERE id=?",array($recordId));

	$product_info = $adb->pquery("SELECT productid,sequence_no, quantity from ncrm_inventoryproductrel WHERE id=?",array($recordId));
	$numrows = $adb->num_rows($product_info);
	for($index = 0;$index <$numrows;$index++) {
		$productid = $adb->query_result($product_info,$index,'productid');
		$qty = $adb->query_result($product_info,$index,'quantity');
		...
		$upd_qty = $qtyinstk-$qty;
		updateProductQty($productid, $upd_qty);
		$sub_prod_query = $adb->pquery("SELECT productid from ncrm_inventorysubproductrel WHERE id=? AND sequence_no=?",array($recordId,$sequence_no));
		if($adb->num_rows($sub_prod_query)>0) {
			for($j=0;$j<$adb->num_rows($sub_prod_query);$j++) {
				$sub_prod_id = $adb->query_result($sub_prod_query,$j,"productid");
				$sqtyinstk= getPrdQtyInStck($sub_prod_id);
		...
		for ($j = 0; $j < $count; ++$j) {
			...
			if ($subjectRow['productid'] == '' || $subjectRow['quantity'] == '' || $subjectRow['listprice'] == '') {
				...
				foreach ($fieldMapping as $fieldName => $index) {
					if($moduleFields[$fieldName]->getTableName() == 'ncrm_inventoryproductrel') {
...
function importRecord($obj, $inventoryFieldData, $lineItemDetails) {
	...
	$webserviceObject = NcrmWebserviceObject::fromName($adb, $moduleName);
	$inventoryOperation = new NcrmInventoryOperation($webserviceObject, $obj->user, $adb, $log);
...
function undoLastImport($obj, $user) {
	...
	if(!is_admin($user) && $user->id != $owner->id) {
		$viewer = new Ncrm_Viewer();
		$viewer->view('OperationNotPermitted.tpl', 'Ncrm');
		exit;
	}
...
function getInventoryFieldsForExport($tableName) {
	...
	$sql .= 'ncrm_currency_info.currency_name as "Currency" ';
...
function getCurrencyId($fieldValue) {
	...
	$sql = 'SELECT id FROM ncrm_currency_info WHERE currency_name = ? AND deleted = 0';
...
function getLineItemFields(){
	...
	$sql = 'SELECT DISTINCT columnname FROM ncrm_field WHERE tablename=?';
	$result = $adb->pquery($sql, array('ncrm_inventoryproductrel'));
