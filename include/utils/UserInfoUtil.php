...
function getMailServerInfo($user)
{
	...
	global $adb;
        $sql = "select * from ncrm_mail_accounts where status=1 and user_id=?";
...
/** To get the Role of the specified user
  ...
  * @returns  ncrm_roleid :: Type String
 */
function fetchUserRole($userid)
{
	...
	global $adb;
	$sql = "select roleid from ncrm_user2role where userid=?";
...
/** Function to get all the ncrm_tab utility action permission for the specified ncrm_profile
...
function getTabsUtilityActionPermission($profileid)
{
	...
	$check = Array();
	$temp_tabid = Array();
	$sql1 = "select * from ncrm_profile2utility where profileid=? order by(tabid)";
...
function getDefaultSharingEditAction()
{
	...
	$sql= "select * from ncrm_def_org_share where editstatus=0";
...
function getDefaultSharingAction()
{
	...
	$sql= "select * from ncrm_def_org_share where editstatus in(0,1)";
	$result = $adb->pquery($sql, array());
	$permissionRow=$adb->fetch_array($result);
	do
	{
...
function getAllDefaultSharingAction()
{
	...
	$copy=Array();
	//retreiving the standard permissions
	$sql= "select * from ncrm_def_org_share";
	$result = $adb->pquery($sql, array());
...
/** Function to update user to ncrm_role mapping based on the userid
...
$log->debug("Entering updateUser2RoleMapping(".$roleid.",".$userid.") method ...");
  ...
  $sqlcheck = "select * from ncrm_user2role where userid=?";
  $resultcheck = $adb->pquery($sqlcheck, array($userid));
  if($adb->num_rows($resultcheck) == 1)
  {
  	$sqldelete = "delete from ncrm_user2role where userid=?";
	$delparams = array($userid);
  	$result_delete = $adb->pquery($sqldelete, $delparams);
  }
  $sql = "insert into ncrm_user2role(userid,roleid) values(?,?)";
...
/** Function to get the ncrm_role name from the ncrm_roleid
  * @param $roleid -- Role Id:: Type varchar
...
function getRoleName($roleid)
{
	...
	global $adb;
	$sql1 = "select * from ncrm_role where roleid=?";
...
function isPermitted($module,$actionname,$record_id='')
{
	...
	if(vtlib_isModuleActive($checkModule)){
		...
		//If no actionid, then allow action is ncrm_tab permission is available
		if($actionid === '')
		{
		...
		//Checking for ncrm_tab permission
		if($profileTabsPermission[$tabid] !=0)
		{
			$permission = "no";
			$log->debug("Exiting isPermitted method ...");
...
/** Function to get the Profile Global Information for the specified ncrm_profileid
  * @param $profileid -- Profile Id:: Type integer
...
$log->debug("Entering getProfileGlobalPermission(".$profileid.") method ...");
  global $adb;
  $sql = "select * from ncrm_profile2globalpermissions where profileid=?" ;
...
/** Function to get the Profile Tab Permissions for the specified ncrm_profileid
  * @param $profileid -- Profile Id:: Type integer
...
$log->debug("Entering getProfileTabsPermission(".$profileid.") method ...");
  global $adb;
  $sql = "select * from ncrm_profile2tab where profileid=?" ;
  $result = $adb->pquery($sql, array($profileid));
  $num_rows = $adb->num_rows($result);
...
/** Function to get the Profile Action Permissions for the specified ncrm_profileid
  * @param $profileid -- Profile Id:: Type integer
...
$log->debug("Entering getProfileActionPermission(".$profileid.") method ...");
	...
	$check = Array();
	$temp_tabid = Array();
	$sql1 = "select * from ncrm_profile2standardpermissions where profileid=?";
	$result1 = $adb->pquery($sql1, array($profileid));
        $num_rows1 = $adb->num_rows($result1);
        for($i=0; $i<$num_rows1; $i++)
        {
...
/** Function to get the Standard and Utility Profile Action Permissions for the specified ncrm_profileid
  * @param $profileid -- Profile Id:: Type integer
  * @returns Profile Tabs Action Permission Array in the following format:
  *    $tabActionPermission = Array($tabid1=>Array(actionid1=>permission, actionid2=>permission,...,actionidn=>permission),
  *                        $tabid2=>Array(actionid1=>permission, actionid2=>permission,...,actionidn=>permission),
...
/** Function to get all  the ncrm_role information
  * @returns $allRoleDetailArray-- Array will contain the details of all the ncrm_roles. RoleId will be the key:: Type array
 */
...
$log->debug("Entering getAllRoleDetails() method ...");
	...
	for($i=0; $i<$num_rows;$i++)
	{
		...
		$query1="select * from ncrm_role where parentrole like ? and depth=?";
...
/** Function to get the ncrm_role information of the specified ncrm_role
...
function getRoleInformation($roleid)
{
	...
	global $adb;
	$query = "select * from ncrm_role where roleid=?";
...
/** Function to get the ncrm_role related ncrm_users
  * @param $roleid -- RoleId :: Type varchar
...
function getRoleUsers($roleId)
{
	...
	global $adb;
	$query = "select ncrm_user2role.*,ncrm_users.* from ncrm_user2role inner join ncrm_users on ncrm_users.id=ncrm_user2role.userid where roleid=?";
	$result = $adb->pquery($query, array($roleId));
	$num_rows=$adb->num_rows($result);
...
/** Function to get the ncrm_role related user ids
  * @param $roleid -- RoleId :: Type varchar
...
function getRoleUserIds($roleId)
{
	...
	global $adb;
	$query = "select ncrm_user2role.*,ncrm_users.user_name from ncrm_user2role inner join ncrm_users on ncrm_users.id=ncrm_user2role.userid where roleid=?";
	$result = $adb->pquery($query, array($roleId));
	$num_rows=$adb->num_rows($result);
	$roleRelatedUsers=Array();
	for($i=0; $i<$num_rows; $i++)
...
/** Function to get the ncrm_role and subordinate ncrm_users
  * @param $roleid -- RoleId :: Type varchar
...
function getRoleAndSubordinateUsers($roleId)
{
	...
	$query = "select ncrm_user2role.*,ncrm_users.user_name from ncrm_user2role inner join ncrm_users on ncrm_users.id=ncrm_user2role.userid inner join ncrm_role on ncrm_role.roleid=ncrm_user2role.roleid where ncrm_role.parentrole like ?";
...
/** Function to get the ncrm_role and subordinate Information for the specified ncrm_roleId
  * @param $roleid -- RoleId :: Type varchar
...
function getRoleAndSubordinatesInformation($roleId)
{
	...
	$query="select * from ncrm_role where parentrole like ? order by parentrole asc";
...
/** Function to get the ncrm_role and subordinate ncrm_role ids
  * @param $roleid -- RoleId :: Type varchar
...
/** Function to delete the ncrm_role related sharing rules
  * @param $roleid -- RoleId :: Type varchar
 */
function deleteRoleRelatedSharingRules($roleId)
{
	...
	$log->debug("Entering deleteRoleRelatedSharingRules(".$roleId.") method ...");
        global $adb;
        $dataShareTableColArr=Array('ncrm_datashare_grp2role'=>'to_roleid',
                                    'ncrm_datashare_grp2rs'=>'to_roleandsubid',
                                    'ncrm_datashare_role2group'=>'share_roleid',
                                    'ncrm_datashare_role2role'=>'share_roleid::to_roleid',
                                    'ncrm_datashare_role2rs'=>'share_roleid::to_roleandsubid',
                                    'ncrm_datashare_rs2grp'=>'share_roleandsubid',
                                    'ncrm_datashare_rs2role'=>'share_roleandsubid::to_roleid',
                                    'ncrm_datashare_rs2rs'=>'share_roleandsubid::to_roleandsubid');
...
function deleteGroupRelatedSharingRules($grpId)
{
	...
	$log->debug("Entering deleteGroupRelatedSharingRules(".$grpId.") method ...");
        ...
        global $adb;
        $dataShareTableColArr=Array('ncrm_datashare_grp2grp'=>'share_groupid::to_groupid',
                                    'ncrm_datashare_grp2role'=>'share_groupid',
                                    'ncrm_datashare_grp2rs'=>'share_groupid',
                                    'ncrm_datashare_role2group'=>'to_groupid',
                                    'ncrm_datashare_rs2grp'=>'to_groupid');
...
/** Function to get userid and username of all ncrm_users
...
function getAllUserName()
{
	...
	global $adb;
	$query="select * from ncrm_users where deleted=0";
	$result = $adb->pquery($query, array());
	$num_rows=$adb->num_rows($result);
	$user_details=Array();
	for($i=0;$i<$num_rows;$i++)
...
/** Function to get groupid and groupname of all ncrm_groups
...
function deleteSharingRule($shareid)
{
	...
	global $adb;
	$query2="select * from ncrm_datashare_module_rel where shareid=?";
	...
	$query4="delete from ncrm_datashare_module_rel where shareid=?";
	$adb->pquery($query4, array($shareid));

	//deleting the releated module sharing permission
	$query5="delete from ncrm_datashare_relatedmodule_permission where shareid=?";
...
function getDataShareTableName()
{
	...
	$dataShareTableColArr=Array('GRP::GRP'=>'ncrm_datashare_grp2grp',
				    'GRP::ROLE'=>'ncrm_datashare_grp2role',
				    'GRP::RS'=>'ncrm_datashare_grp2rs',
				    'ROLE::GRP'=>'ncrm_datashare_role2group',
				    'ROLE::ROLE'=>'ncrm_datashare_role2role',
				    'ROLE::RS'=>'ncrm_datashare_role2rs',
				    'RS::GRP'=>'ncrm_datashare_rs2grp',
				    'RS::ROLE'=>'ncrm_datashare_rs2role',
				    'RS::RS'=>'ncrm_datashare_rs2rs');
...
/** This function is to retreive the ncrm_profiles associated with the  the specified user
  * It takes the following input parameters:
  *     $userid -- The User Id:: Type Integer
  *This function will return the ncrm_profiles associated to the specified ncrm_users in an Array in the following format:
...
function getUserProfile($userId)
{
	...
	$profArr=Array();
	$sql1 = "select profileid from ncrm_role2profile where roleid=?";
...
/** To retreive the global permission of the specifed user from the various ncrm_profiles associated with the user
  * @param $userid -- The User Id:: Type Integer
...
/** To retreive the ncrm_tab permissions of the specifed user from the various ncrm_profiles associated with the user
  * @param $userid -- The User Id:: Type Integer
  * @returns  user global permission  array in the following format:
...
/** To retreive the ncrm_tab acion permissions of the specifed user from the various ncrm_profiles associated with the user
  * @param $userid -- The User Id:: Type Integer
  * @returns  user global permission  array in the following format:
...
/** To retreive the parent ncrm_role of the specified ncrm_role
  * @param $roleid -- The Role Id:: Type varchar
  * @returns  parent ncrm_role array in the following format:
...
/** To retreive the subordinate ncrm_roles of the specified parent ncrm_role
  * @param $roleid -- The Role Id:: Type varchar
  * @returns  subordinate ncrm_role array in the following format:
...
/** To retreive the subordinate ncrm_roles and ncrm_users of the specified parent ncrm_role
  * @param $roleid -- The Role Id:: Type varchar
  * @returns  subordinate ncrm_role array in the following format:
  *     $subordinateRoleUserArray=(roleid1=>Array(userid1,userid2,userid3),
                               ncrm_roleid2=>Array(userid1,userid2,userid3)
				                |
						|
			       ncrm_roleidn=>Array(userid1,userid2,userid3));
 */
...
function getWriteSharingGroupsList($module)
{
	...
	$grp_array=Array();
	$tabid=getTabid($module);
	$query = "select sharedgroupid from ncrm_tmp_write_group_sharing_per where userid=? and tabid=?";
...
function getListViewSecurityParameter($module)
{
	...
	{
		$sec_query .= " and (
						ncrm_crmentity.smownerid in($current_user->id)
						or ncrm_crmentity.smownerid in(select ncrm_user2role.userid from ncrm_user2role inner join ncrm_users on ncrm_users.id=ncrm_user2role.userid inner join ncrm_role on ncrm_role.roleid=ncrm_user2role.roleid where ncrm_role.parentrole like '".$current_user_parent_role_seq."::%')
						or ncrm_crmentity.smownerid in(select shareduserid from ncrm_tmp_read_user_sharing_per where userid=".$current_user->id." and tabid=".$tabid.")
						or (";
                        ...
                        if(sizeof($current_user_groups) > 0)
                        {
                         ...
                         $sec_query .= " ncrm_groups.groupid in(select ncrm_tmp_read_group_sharing_per.sharedgroupid from ncrm_tmp_read_group_sharing_per where userid=".$current_user->id." and tabid=".$tabid."))) ";
	}
	elseif($module == 'Accounts')
	{
		$sec_query .= " and (ncrm_crmentity.smownerid in($current_user->id) " .
				"or ncrm_crmentity.smownerid in(select ncrm_user2role.userid from ncrm_user2role inner join ncrm_users on ncrm_users.id=ncrm_user2role.userid inner join ncrm_role on ncrm_role.roleid=ncrm_user2role.roleid where ncrm_role.parentrole like '".$current_user_parent_role_seq."::%') " .
				"or ncrm_crmentity.smownerid in(select shareduserid from ncrm_tmp_read_user_sharing_per where userid=".$current_user->id." and tabid=".$tabid.") or (";
	...
	{
		$sec_query .= " and (ncrm_crmentity.smownerid in($current_user->id) or ncrm_crmentity.smownerid in(select ncrm_user2role.userid from ncrm_user2role inner join ncrm_users on ncrm_users.id=ncrm_user2role.userid inner join ncrm_role on ncrm_role.roleid=ncrm_user2role.roleid where ncrm_role.parentrole like '".$current_user_parent_role_seq."::%') or ncrm_crmentity.smownerid in(select shareduserid from ncrm_tmp_read_user_sharing_per where userid=".$current_user->id." and tabid=".$tabid.") ";
	...
	{
		$sec_query .= " and ncrm_crmentity.smownerid=".$current_user->id." ";
	...
	elseif($module == 'Calendar')
	{
		...
		if(isset($shared_ids) && $shared_ids != '')
			$condition = " or (ncrm_crmentity.smownerid in($shared_ids) and ncrm_activity.visibility = 'Public')";
		else
			$condition = null;
		$sec_query .= " and (ncrm_crmentity.smownerid in($current_user->id) $condition or ncrm_crmentity.smownerid in(select ncrm_user2role.userid from ncrm_user2role inner join ncrm_users on ncrm_users.id=ncrm_user2role.userid inner join ncrm_role on ncrm_role.roleid=ncrm_user2role.roleid where ncrm_role.parentrole like '".$current_user_parent_role_seq."::%')";

		if(sizeof($current_user_groups) > 0)
		{
			$sec_query .= " or ((ncrm_groups.groupid in (". implode(",", $current_user_groups) .")))";
		}
		$sec_query .= ")";
	}
...
function get_current_user_access_groups($module)
{
	...
	$query ="select groupname,groupid from ncrm_groups";
	$params = array();
...
function getGrpId($groupname)
{
	...
	global $adb;
    $groupid = Ncrm_Cache::get('group',$groupname);
    if(!$groupid && $groupid !== 0){
        $result = $adb->pquery("select groupid from ncrm_groups where groupname=?", array($groupname));
        $groupid = ($adb->num_rows($result) > 0) ? $adb->query_result($result,0,'groupid') : 0;
        Ncrm_Cache::set('group',$groupname,$groupid);
    }
	$log->debug("Exiting getGrpId method ...");
	return $groupid;
}

/** Function to check permission to access a ncrm_field for a given user
...
function getFieldVisibilityPermission($fld_module, $userid, $fieldname, $accessmode='readonly')
{
	...
	{
        //get ncrm_profile list using userid
        ...
        $tabid = getTabid($fld_module);
			...
			if($accessmode == 'readonly') {
				$query="SELECT ncrm_profile2field.visible FROM ncrm_field INNER JOIN ncrm_profile2field ON ncrm_profile2field.fieldid=ncrm_field.fieldid INNER JOIN ncrm_def_org_field ON ncrm_def_org_field.fieldid=ncrm_field.fieldid WHERE ncrm_field.tabid=? AND ncrm_profile2field.visible=0 AND ncrm_def_org_field.visible=0  AND ncrm_profile2field.profileid in (". generateQuestionMarks($profilelist) .") AND ncrm_field.fieldname= ? and ncrm_field.presence in (0,2) GROUP BY ncrm_field.fieldid";
                } else {
				$query="SELECT ncrm_profile2field.visible FROM ncrm_field INNER JOIN ncrm_profile2field ON ncrm_profile2field.fieldid=ncrm_field.fieldid INNER JOIN ncrm_def_org_field ON ncrm_def_org_field.fieldid=ncrm_field.fieldid WHERE ncrm_field.tabid=? AND ncrm_profile2field.visible=0 AND ncrm_profile2field.readonly=0 AND ncrm_def_org_field.visible=0  AND ncrm_profile2field.profileid in (". generateQuestionMarks($profilelist) .") AND ncrm_field.fieldname= ? and ncrm_field.presence in (0,2) GROUP BY ncrm_field.fieldid";
                }
                $params = array($tabid, $profilelist, $fieldname);

            } else {
			if($accessmode == 'readonly') {
				$query="SELECT ncrm_profile2field.visible FROM ncrm_field INNER JOIN ncrm_profile2field ON ncrm_profile2field.fieldid=ncrm_field.fieldid INNER JOIN ncrm_def_org_field ON ncrm_def_org_field.fieldid=ncrm_field.fieldid WHERE ncrm_field.tabid=? AND ncrm_profile2field.visible=0 AND ncrm_def_org_field.visible=0  AND ncrm_field.fieldname= ? and ncrm_field.presence in (0,2) GROUP BY ncrm_field.fieldid";
                } else {
				$query="SELECT ncrm_profile2field.visible FROM ncrm_field INNER JOIN ncrm_profile2field ON ncrm_profile2field.fieldid=ncrm_field.fieldid INNER JOIN ncrm_def_org_field ON ncrm_def_org_field.fieldid=ncrm_field.fieldid WHERE ncrm_field.tabid=? AND ncrm_profile2field.visible=0 AND ncrm_profile2field.readonly=0 AND ncrm_def_org_field.visible=0  AND ncrm_field.fieldname= ? and ncrm_field.presence in (0,2) GROUP BY ncrm_field.fieldid";
                }
...
function getColumnVisibilityPermission($userid, $columnname, $module, $accessmode='readonly')
{
	...
	if($cacheFieldInfo === false) {
		$res = $adb->pquery("select fieldname from ncrm_field where tabid=? and columnname=? and ncrm_field.presence in (0,2)", array($tabid, $columnname));
...
/** Function to recalculate the Sharing Rules for all the ncrm_users
  * This function will recalculate all the sharing rules for all the ncrm_users in the Organization and will write them in flat ncrm_files
  *
 */
function RecalculateSharingRules()
{
	...
	$query="select id from ncrm_users where deleted=0";
	$result=$adb->pquery($query, array());
	$num_rows=$adb->num_rows($result);
	for($i=0;$i<$num_rows;$i++)
	{
...
function getSharingModuleList($eliminateModules=false)
{
	...
	$query = "SELECT name FROM ncrm_tab WHERE presence=0 AND ownedby = 0 AND isentitytype = 1";
...
function isCalendarPermittedBySharing($recordId)
{
	...
	$permission = 'no';
	$query = "SELECT ncrm_sharedcalendar.sharedid, ncrm_users.calendarsharedtype FROM ncrm_sharedcalendar RIGHT JOIN ncrm_users ON ncrm_sharedcalendar.userid=ncrm_users.id and status='Active'
				WHERE ncrm_users.id IN(SELECT smownerid FROM ncrm_activity INNER JOIN ncrm_crmentity ON ncrm_crmentity.crmid=ncrm_activity.activityid
