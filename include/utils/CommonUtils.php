...
function is_admin($user) {
	return Ncrm_Functions::userIsAdministrator($user);
}

function parse_calendardate($local_format) {
	return Ncrm_Functions::currentUserJSDateFormat($local_format);
}

function from_html($string, $encode = true) {
	return Ncrm_Functions::fromHTML($string, $encode);
}

function fck_from_html($string) {
	return Ncrm_Functions::fromHTML_FCK($string);
}

function popup_from_html($string, $encode = true) {
	return Ncrm_Functions::fromHTML_Popup($string, $encode);
}

function fetchCurrency($id) {
	return Ncrm_Functions::userCurrencyId($id);
}

function getCurrencyName($currencyid, $show_symbol = true) {
	return Ncrm_Functions::getCurrencyName($currencyid, $show_symbol);
}

function getTabid($module) {
	return Ncrm_Functions::getModuleId($module);
	}

function getFieldid($tabid, $fieldname, $onlyactive = true) {
	return Ncrm_Functions::getModuleFieldId($tabid, $fieldname, $onlyactive);
}

function getTabOwnedBy($module) {
	return Ncrm_Functions::getModuleOwner($module);
}

function getSalesEntityType($crmid) {
	return Ncrm_Functions::getCRMRecordType($crmid);
}

function getAccountName($account_id) {
	return Ncrm_Functions::getCRMRecordLabel($account_id);
}

function getProductName($product_id) {
	return Ncrm_Functions::getCRMRecordLabel($product_id);
}

function getPotentialName($potential_id) {
	return Ncrm_Functions::getCRMRecordLabel($potential_id);
}

function getContactName($contact_id) {
	return Ncrm_Functions::getCRMRecordLabel($contact_id);
}

function getFullNameFromQResult($result, $row_count, $module) {
	return Ncrm_Deprecated::getFullNameFromQResult($result, $row_count, $module);
}

function getFullNameFromArray($module, $fieldValues) {
	return Ncrm_Deprecated::getFullNameFromArray($module, $fieldValues);
}

function getCampaignName($campaign_id) {
	return Ncrm_Functions::getCRMRecordLabel($campaign_id);
}

function getVendorName($vendor_id) {
	return Ncrm_Functions::getCRMRecordLabel($vendor_id);
}

function getQuoteName($quote_id) {
	return Ncrm_Functions::getCRMRecordLabel($quote_id);
}

function getPriceBookName($pricebookid) {
	return Ncrm_Functions::getCRMRecordLabel($pricebookid);
}

function getSoName($so_id) {
	return Ncrm_Functions::getCRMRecordLabel($so_id);
}

function getGroupName($groupid) {
	return Ncrm_Functions::getGroupName($groupid);
}

function getUserName($userid) {
	return Ncrm_Functions::getUserName($userid);
}

function getUserFullName($userid) {
	return Ncrm_Functions::getUserRecordLabel($userid);
}

function getParentName($parent_id) {
	return Ncrm_Functions::getCRMRecordLabel($parent_id);
	}

function getValidDisplayDate($cur_date_val) {
	return Ncrm_Functions::currentUserDisplayDate($cur_date_val);
}

function getNewDisplayDate() {
	return Ncrm_Functions::currentUserDisplayDateNew();
}

/** This function returns the conversion rate and ncrm_currency symbol
 * in array format for a given id.
 * param $id - ncrm_currency id.
 */
function getCurrencySymbolandCRate($id) {
	return Ncrm_Functions::getCurrencySymbolandRate($id);
}
...
function getTermsandConditions() {
	return Ncrm_Functions::getInventoryTermsAndCondition();
}
 ...
 */
function br2nl($str) {
	return Ncrm_Functions::br2nl($str);
}
...
function getBlockId($tabid, $label) {
	return Ncrm_Deprecated::getBlockId($tabid, $label);
}
...
function getParentTabFromModule($module) {
	return Ncrm_Deprecated::getParentTabFromModule($module);
}

/**
 * This function is used to get the Parent Tab name for a given module.
 * Takes no parameter but gets the ncrm_parenttab value from form request
 * This returns value string type
 */
function getParentTab() {
	return Ncrm_Deprecated::getParentTab();
}
...
function setObjectValuesFromRequest($focus) {
	return Ncrm_Deprecated::copyValuesFromRequest($focus);
}

function create_tab_data_file() {
	return Ncrm_Deprecated::createModuleMetaFile();
}

/**
 * Function to write the ncrm_parenttabid and name to a flat file parent_tabdata.txt so that the data
 * is obtained from the file instead of repeated queries
 * returns null
 */
function create_parenttab_data_file() {
	return Ncrm_Deprecated::createModuleGroupMetaFile();
}

function getEntityName($module, $ids_list, $compute=true) {
	if ($compute) {
		return Ncrm_Functions::computeCRMRecordLabels($module, $ids_list);
	} else {
		return Ncrm_Functions::getCRMRecordLabels($module, $ids_list);
	}
}
...
function decideFilePath() {
	return Ncrm_Functions::initStorageFileDirectory();
}

/**
 * 	This function is used to check whether the attached file is a image file or not
 * 	@param string $file_details  - ncrm_files array which contains all the uploaded file details
...
function validateImageFile($file_details) {
	return Ncrm_Functions::validateImage($file_details);
}
...
function getTemplateDetails($templateid) {
	return Ncrm_Deprecated::getTemplateDetails($templateid);
}
...
function getMergedDescription($description, $id, $parent_type) {
	return Ncrm_Functions::getMergedDescription($description, $id, $parent_type);
}
...
function getSingleFieldValue($tablename, $fieldname, $idname, $id) {
	return Ncrm_Functions::getSingleFieldValue($tablename, $fieldname, $idname, $id);
}
...
function get_announcements() {
	return Ncrm_Deprecated::getAnnouncements();
}
...
function getrecurringObjValue() {
	return Ncrm_Functions::getRecurringObjValue();
}

function getTranslatedString($str, $module = 'Ncrm') {
	return Ncrm_Functions::getTranslatedString($str, $module);
}
...
function getTranslatedCurrencyString($str) {
	return Ncrm_Deprecated::getTranslatedCurrencyString($str);
	}

function getTicketComments($ticketid) {
	return Ncrm_Functions::getTicketComments($ticketid);
}

function makeRandomPassword() {
	return Ncrm_Functions::generateRandomPassword();
}
...
function getCvIdOfAll($module) {
	return Ncrm_Deprecated::getIdOfCustomViewByNameAll($module);
}
...
function getTagCloudView($id = "") {
	return Ncrm_Functions::getTagCloudView($id);
}
...
function SaveTagCloudView($id = "") {
	return Ncrm_Deprecated::SaveTagCloudView($id);
}
 ...
 * *	        -------------------+------------+----------------------+------------------+
 * *		Potential Name     | I~O        | ncrm_quotes        | potentialid      |
 * *
 * *     Then put an entry in $new_field_details  like this:
 * *
 * *				"ncrm_quotes:potentialid"=>"V",
 * *
...
function ChangeTypeOfData_Filter($table_name, $column_name, $type_of_data) {
	return Ncrm_Functions::transformFieldTypeOfData($table_name, $column_name, $type_of_data);
}
...
function clear_smarty_cache($path = null) {
	Ncrm_Deprecated::clearSmartyCompiledFiles($path);
}
...
function get_smarty_compiled_file($template_file, $path = null) {
	return Ncrm_Deprecated::getSmartyCompiledTemplateFile($template_file, $path);
}

/** Function to carry out all the necessary actions after migration */
function perform_post_migration_activities() {
	Ncrm_Deprecated::postApplicationMigrationTasks();
}
...
function getPickListValues($tablename, $roleid) {
	return Ncrm_Functions::getPickListValuesFromTableForRole($tablename, $roleid);
}
...
function checkFileAccessForInclusion($filepath) {
	Ncrm_Deprecated::checkFileAccessForInclusion($filepath);
}

/** Function to check the file deletion within the deletable (safe) directories*/
function checkFileAccessForDeletion($filepath) {
	Ncrm_Deprecated::checkFileAccessForDeletion($filepath);
}

/** Function to check the file access is made within web root directory. */
function checkFileAccess($filepath) {
	Ncrm_Deprecated::checkFileAccess($filepath);
}
...
function isFileAccessible($filepath) {
	return Ncrm_Deprecated::isFileAccessible($filepath);
}
...
function getActivityType($id) {
	return Ncrm_Functions::getActivityType($id);
}

/** Function to get owner name either user or group */
function getOwnerName($id) {
	return Ncrm_Functions::getOwnerRecordLabel($id);
}

/** Function to get owner name either user or group */
function getOwnerNameList($idList) {
	return Ncrm_Functions::getOwnerRecordLabels($idList);
}
...
function getSettingsBlockId($label) {
	return Ncrm_Deprecated::getSettingsBlockId($label);
}
...
function getEntityField($module) {
	return Ncrm_Functions::getEntityModuleSQLColumnString($module);
}

/**
 * this function returns the entity information for a given module; for e.g. for Contacts module
 * it returns the information of tablename, modulename, fieldsname and id gets from ncrm_entityname
 * @param string $module - the module name
 * @return array $data - the entity information for the module
 */
function getEntityFieldNames($module) {
	return Ncrm_Functions::getEntityModuleInfoFieldsFormatted($module);
}
...
function getEntityFieldNameDisplay($module, $fieldsName, $fieldValues) {
	return Ncrm_Deprecated::getCurrentUserEntityFieldNameDisplay($module, $fieldsName, $fieldValues);
}

// ncrm cache utility
require_once('include/utils/VTCacheUtils.php');

// vtlib customization: Extended NCRM utlitiy functions
require_once('include/utils/VtlibUtils.php');

// END

function vt_suppressHTMLTags($string) {
	return Ncrm_Functions::suppressHTMLTags($string);
}

function getSqlForNameInDisplayFormat($input, $module, $glue = ' ') {
	return Ncrm_Deprecated::getSqlForNameInDisplayFormat($input, $module, $glue);
}

function getModuleSequenceNumber($module, $recordId) {
	return Ncrm_Deprecated::getModuleSequenceNumber($module, $recordId);
	}

function getInvoiceStatus($invoiceId) {
	return Ncrm_Functions::getInvoiceStatus($invoiceId);
}

function decimalFormat($value){
	return Ncrm_Functions::formatDecimal($value);
}

function updateRecordLabel($module,$recordId){
	return Ncrm_Functions::updateCRMRecordLabel($module, $recordId);
}


function get_group_options() {
return Ncrm_Functions::get_group_options();
}
?>
