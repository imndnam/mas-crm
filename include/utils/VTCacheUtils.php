...
class VTCacheUtils {
	...
	static function updateFieldInfo($tabid, $fieldname, $fieldid, $fieldlabel,
        ...
        Ncrm_Cache::set('fieldInfo', $tabid, self::$_fieldinfo_cache[$tabid]);
	}
	static function lookupFieldInfo($tabid, $fieldname) {
        $fieldInfo = Ncrm_Cache::get('fieldInfo', $tabid);
        ...
        $field = Ncrm_Cache::get('field-'.$tabid,$fieldname);
        if($field){
            $cacheField = array(
                'tabid' => $tabid,
                'fieldid' => $field->getId(),
	...
	static function lookupFieldInfo_Module($module, $presencein = array('0', '2')) {
        ...
        $fieldInfo = Ncrm_Cache::get('ModuleFields',$tabid);
        if($fieldInfo){
	...
	static function updateEntityNameInfo($module, $data) {
        ...
        Ncrm_Cache::set('EntityInfo', $module, self::$_module_entityname_cache[$module]);
	}
	static function lookupEntityNameInfo($module) {
        $entityNames = Ncrm_Cache::get('EntityInfo', $module);
        if($entityNames){
            return $entityNames;
