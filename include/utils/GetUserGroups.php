...
class GetUserGroups { 
	...
	function getAllUserGroups($userid)
	{
		...
		$query="select * from ncrm_users2group where userid=?";
		...
		//Retreiving from the ncrm_user2role
		$query="select * from ncrm_group2role where roleid=?";
		...
		$query="select * from ncrm_group2rs where roleandsubid in (". generateQuestionMarks($parentRolelist) .")";
