...
require_once 'vtlib/Ncrm/Functions.php';
require_once 'vtlib/Ncrm/Deprecated.php';
...
function get_user_array($add_blank=true, $status="Active", $assigned_user="",$private="",$module=false)
{
	...
	if($user_array == null)
	{
		...
		$temp_result = Array();
		// Including deleted ncrm_users for now.
		if (empty($status)) {
				$query = "SELECT id, user_name from ncrm_users";
				$params = array();
		}
		else {
				if($private == 'private')
				{
					...
					$query = "select id as id,user_name as user_name,first_name,last_name from ncrm_users where id=? and status='Active' union select ncrm_user2role.userid as id,ncrm_users.user_name as user_name ,
							  ncrm_users.first_name as first_name ,ncrm_users.last_name as last_name
							  from ncrm_user2role inner join ncrm_users on ncrm_users.id=ncrm_user2role.userid inner join ncrm_role on ncrm_role.roleid=ncrm_user2role.roleid where ncrm_role.parentrole like ? and status='Active' union
							  select shareduserid as id,ncrm_users.user_name as user_name ,
							  ncrm_users.first_name as first_name ,ncrm_users.last_name as last_name  from ncrm_tmp_write_user_sharing_per inner join ncrm_users on ncrm_users.id=ncrm_tmp_write_user_sharing_per.shareduserid where status='Active' and ncrm_tmp_write_user_sharing_per.userid=? and ncrm_tmp_write_user_sharing_per.tabid=?";
					$params = array($current_user->id, $current_user_parent_role_seq."::%", $current_user->id, getTabid($module));
				}
				else
				{
					$log->debug("Sharing is Public. All ncrm_users should be listed");
					$query = "SELECT id, user_name,first_name,last_name from ncrm_users WHERE status=?";
					$params = array($status);
				}
		}
...
function get_group_array($add_blank=true, $status="Active", $assigned_user="",$private="",$module = false)
{
	...
	if($group_array == null)
	{
		...
		// Including deleted ncrm_users for now.
		$log->debug("Sharing is Public. All ncrm_users should be listed");
		$query = "SELECT groupid, groupname from ncrm_groups";
		$params = array();
		...
		if($private == 'private'){
			...
			if(count($current_user_groups) != 0) {
				$query .= " OR ncrm_groups.groupid in (".generateQuestionMarks($current_user_groups).")";
				array_push($params, $current_user_groups);
			}
			$log->debug("Sharing is Private. Only the current user should be listed");
			$query .= " union select ncrm_group2role.groupid as groupid,ncrm_groups.groupname as groupname from ncrm_group2role inner join ncrm_groups on ncrm_groups.groupid=ncrm_group2role.groupid inner join ncrm_role on ncrm_role.roleid=ncrm_group2role.roleid where ncrm_role.parentrole like ?";
			array_push($params, $current_user_parent_role_seq."::%");

			if(count($current_user_groups) != 0) {
				$query .= " union select ncrm_groups.groupid as groupid,ncrm_groups.groupname as groupname from ncrm_groups inner join ncrm_group2rs on ncrm_groups.groupid=ncrm_group2rs.groupid where ncrm_group2rs.roleandsubid in (".generateQuestionMarks($parent_roles).")";
				array_push($params, $parent_roles);
			}

			$query .= " union select sharedgroupid as groupid,ncrm_groups.groupname as groupname from ncrm_tmp_write_group_sharing_per inner join ncrm_groups on ncrm_groups.groupid=ncrm_tmp_write_group_sharing_per.sharedgroupid where ncrm_tmp_write_group_sharing_per.userid=?";
			array_push($params, $current_user->id);

			$query .= " and ncrm_tmp_write_group_sharing_per.tabid=?";
...
function return_app_list_strings_language($language) {
	return Ncrm_Deprecated::return_app_list_strings_language($language);
}
...
function return_module_language($language, $module) {
	return Ncrm_Deprecated::getModuleTranslationStrings($language, $module);
}
...
function return_specified_module_language($language, $module) {
	return Ncrm_Deprecated::return_app_list_strings_language($language, $module);
}
...
{
        $startstring=$string;
	$cachedresult=Ncrm_Cache::get('to_html',$startstring);
	if($cachedresult){
		return $cachedresult;
	}
	...
	if(is_string($string))
	{
		...
		// In ncrm5 ajax request are treated specially and the data is encoded
		if ($doconvert == true)
		{
			if($inUTF8)
	...
	Ncrm_Cache::set('to_html', $startstring, $string);
	return $string;
}
...
function getTabname($tabid)
{
	...
	if (!isset($cache[$tabid])) {
		$sql = "select tablabel from ncrm_tab where tabid=?";
...
{
	return Ncrm_Functions::getModuleName($tabid);
}
...
function getColumnFields($module)
{
	...
	if($cachedModuleFields === false) {
		...
		FROM ncrm_field WHERE tabid in (" . generateQuestionMarks($tabid) . ")";
...
function getUserEmail($userid)
{
	...
	$log->info("in getUserEmail ".$userid);
        ...
        if($userid != '')
        {
                $sql = "select email1 from ncrm_users where id=?";
...
function getUserId_Ol($username)
{
	...
	global $adb;
	$sql = "select id from ncrm_users where user_name=?";
...
function getActionid($action)
{
	...
	{
		$query="select * from ncrm_actionmapping where actionname=?";
...
function getActionname($actionid)
{
	...
	else
	{
		...
		$query="select * from ncrm_actionmapping where actionid=? and securitycheck=0";
...
function getRecordOwnerId($record)
{
	...
	if($ownerId === false) {
		$query="select smownerid from ncrm_crmentity where crmid = ?";
	...
	if($ownerId)
	{
		...
		if($count === false) {
			$sql_result = $adb->pquery("select count(*) as count from ncrm_users where id = ?",array($ownerId));
...
function insertProfile2field($profileid)
{
	...
	$fld_result = $adb->pquery("select * from ncrm_field where generatedtype=1 and displaytype in (1,2,3) and ncrm_field.presence in (0,2) and tabid != 29", array());
    ...
    for($i=0; $i<$num_rows; $i++) {
         ...
         $adb->pquery("insert into ncrm_profile2field values (?,?,?,?,?)", $params);
	}
...
function insert_def_org_field()
{
	...
	$fld_result = $adb->pquery("select * from ncrm_field where generatedtype=1 and displaytype in (1,2,3) and ncrm_field.presence in (0,2) and tabid != 29", array());
        ...
        for($i=0; $i<$num_rows; $i++)
        {
                 ...
                 $adb->pquery("insert into ncrm_def_org_field values (?,?,?,?)", $params);
	}
...
function updateProductQty($product_id, $upd_qty)
{
	...
	global $adb;
	$query= "update ncrm_products set qtyinstock=? where productid=?";
...
function addToProductStock($productId,$qty)
{
	...
	$sql = "UPDATE ncrm_products set qtyinstock=? where productid=?";
...
function addToProductDemand($productId,$qty)
{
	...
	$updQty=$qtyInStck + $qty;
	$sql = "UPDATE ncrm_products set qtyindemand=? where productid=?";
	$adb->pquery($sql, array($updQty, $productId));
...
function getProductQtyInStock($product_id)
{
	...
	global $adb;
        $query1 = "select qtyinstock from ncrm_products where productid=?";
...
function getProductQtyInDemand($product_id)
{
	...
	global $adb;
        $query1 = "select qtyindemand from ncrm_products where productid=?";
        $result = $adb->pquery($query1, array($product_id));
...
/**     Function to get the ncrm_table name from 'field' ncrm_table for the input ncrm_field based on the module
 *      @param  : string $module - current module value
 *      @param  : string $fieldname - ncrm_fieldname to which we want the ncrm_tablename
 *      @return : string $tablename - ncrm_tablename in which $fieldname is a column, which is retrieved from 'field' ncrm_table per $module basis
  */
function getTableNameForField($module,$fieldname)
{
	...
	$sql = "select tablename from ncrm_field where tabid in (". generateQuestionMarks($tabid) .") and ncrm_field.presence in (0,2) and columnname like ?";
...
function getPotentialsRelatedAccounts($record_id)
{
	...
	global $adb;
	$query="select related_to from ncrm_potential where potentialid=?";
...
function getEmailsRelatedAccounts($record_id)
{
	...
	global $adb;
	$query = "select ncrm_seactivityrel.crmid from ncrm_seactivityrel inner join ncrm_crmentity on ncrm_crmentity.crmid=ncrm_seactivityrel.crmid where ncrm_crmentity.setype='Accounts' and activityid=?";
	$result = $adb->pquery($query, array($record_id));
...
function getEmailsRelatedLeads($record_id)
{
	...
	global $adb;
	$query = "select ncrm_seactivityrel.crmid from ncrm_seactivityrel inner join ncrm_crmentity on ncrm_crmentity.crmid=ncrm_seactivityrel.crmid where ncrm_crmentity.setype='Leads' and activityid=?";
	$result = $adb->pquery($query, array($record_id));
...
function getHelpDeskRelatedAccounts($record_id)
{
	...
	global $adb;
        $query="select parent_id from ncrm_troubletickets inner join ncrm_crmentity on ncrm_crmentity.crmid=ncrm_troubletickets.parent_id where ticketid=? and ncrm_crmentity.setype='Accounts'";
        $result=$adb->pquery($query, array($record_id));
...
function getQuotesRelatedAccounts($record_id)
{
	...
	global $adb;
        $query="select accountid from ncrm_quotes where quoteid=?";
        $result=$adb->pquery($query, array($record_id));
...
function getQuotesRelatedPotentials($record_id)
{
	...
	global $adb;
        $query="select potentialid from ncrm_quotes where quoteid=?";
        $result=$adb->pquery($query, array($record_id));
...
function getSalesOrderRelatedAccounts($record_id)
{
	...
	global $adb;
        $query="select accountid from ncrm_salesorder where salesorderid=?";
        $result=$adb->pquery($query, array($record_id));
        $accountid=$adb->query_result($result,0,'accountid');
...
function getSalesOrderRelatedPotentials($record_id)
{
	...
	global $adb;
        $query="select potentialid from ncrm_salesorder where salesorderid=?";
        $result=$adb->pquery($query, array($record_id));
        $potid=$adb->query_result($result,0,'potentialid');
...
function getSalesOrderRelatedQuotes($record_id)
{
	...
	global $adb;
        $query="select quoteid from ncrm_salesorder where salesorderid=?";
        $result=$adb->pquery($query, array($record_id));
...
function getInvoiceRelatedAccounts($record_id)
{
	...
	global $adb;
        $query="select accountid from ncrm_invoice where invoiceid=?";
        $result=$adb->pquery($query, array($record_id));
        $accountid=$adb->query_result($result,0,'accountid');
...
function getInvoiceRelatedSalesOrder($record_id)
{
	...
	global $adb;
        $query="select salesorderid from ncrm_invoice where invoiceid=?";
        $result=$adb->pquery($query, array($record_id));
...
function getAccessPickListValues($module)
{
	...
	$id = getTabid($module);
	$query = "select fieldname,columnname,fieldid,fieldlabel,tabid,uitype from ncrm_field where tabid = ? and uitype in ('15','33','55') and ncrm_field.presence in (0,2)";
	...
	for($i=0;$i < $adb->num_rows($result);$i++)
{
		...
		if (count($roleids) > 1)
	{
			$mulsel="select distinct $fieldname from ncrm_$fieldname inner join ncrm_role2picklist on ncrm_role2picklist.picklistvalueid = ncrm_$fieldname.picklist_valueid where roleid in (\"". implode($roleids,"\",\"") ."\") and picklistid in (select picklistid from ncrm_$fieldname) order by sortid asc";
	}
	else
	{
			$mulsel="select distinct $fieldname from ncrm_$fieldname inner join ncrm_role2picklist on ncrm_role2picklist.picklistvalueid = ncrm_$fieldname.picklist_valueid where roleid ='".$roleid."' and picklistid in (select picklistid from ncrm_$fieldname) order by sortid asc";
	}
...
function transferUserCurrency($old_cur, $new_cur) {
	...
	$sql = "update ncrm_users set currency_id=? where currency_id=?";
...
function transferProductCurrency($old_cur, $new_cur) {
	...
	$prod_res = $adb->pquery("select productid from ncrm_products where currency_id = ?", array($old_cur));
	...
	if(count($prod_ids) > 0) {
		...
		for($i=0;$i<count($prod_ids);$i++) {
			...
			$query = "update ncrm_products set currency_id=?, unit_price=? where productid=?";
...
function transferPriceBookCurrency($old_cur, $new_cur) {
	...
	$pb_res = $adb->pquery("select pricebookid from ncrm_pricebook where currency_id = ?", array($old_cur));
...
/**
 * this function searches for a given number in ncrm and returns the callerInfo in an array format
...
function get_use_asterisk($id){
	...
	$sql = "select * from ncrm_asteriskextensions where userid = ?";
...
function addToCallHistory($userExtension, $callfrom, $callto, $status, $adb, $useCallerInfo){
	$sql = "select * from ncrm_asteriskextensions where asterisk_extension=?";
	$result = $adb->pquery($sql,array($userExtension));
	$userID = $adb->query_result($result, 0, "userid");
	if(empty($userID)) {
		// we have observed call to extension not configured in Ncrm will returns NULL
		return;
	}
	if(empty($callfrom)){
		$callfrom = "Unknown";
	...
	$crmID = $adb->getUniqueID('ncrm_crmentity');
	$timeOfCall = date('Y-m-d H:i:s');

	$query = "INSERT INTO ncrm_crmentity (crmid,smcreatorid,smownerid,modifiedby,setype,description,createdtime,
	...
	$sql = "insert into ncrm_pbxmanager (pbxmanagerid,callfrom,callto,timeofcall,status)values (?,?,?,?,?)";
...
function getRelationTables($module,$secmodule){
	...
	$ui10_query = $adb->pquery("SELECT ncrm_field.tabid AS tabid,ncrm_field.tablename AS tablename, ncrm_field.columnname AS columnname FROM ncrm_field INNER JOIN ncrm_fieldmodulerel ON ncrm_fieldmodulerel.fieldid = ncrm_field.fieldid WHERE (ncrm_fieldmodulerel.module=? AND ncrm_fieldmodulerel.relmodule=?) OR (ncrm_fieldmodulerel.module=? AND ncrm_fieldmodulerel.relmodule=?)",array($module,$secmodule,$secmodule,$module));
	...
	if(is_array($reltables) && !empty($reltables)){
		...
		$rel_array = array("ncrm_crmentityrel"=>array("crmid","relcrmid"),"".$primary_obj->table_name."" => "".$primary_obj->table_index."");
	}
	return $rel_array;
}
...
function installVtlibModule($packagename, $packagepath, $customized=false) {
	global $log, $Ncrm_Utils_Log, $_installOrUpdateVtlibModule;
	...
	require_once('vtlib/Ncrm/Package.php');
	require_once('vtlib/Ncrm/Module.php');
	$Ncrm_Utils_Log = defined('INSTALLATION_MODE_DEBUG')? INSTALLATION_MODE_DEBUG : true;
	...
	if($package->isLanguageType()) {
		require_once('vtlib/Ncrm/Language.php');
		$languagePack = new Ncrm_Language();
	...
	} else if(Ncrm_Module::getInstance($module)) {
...
function com_vtGetModules($adb) {
	$sql="select distinct ncrm_field.tabid, name
		from ncrm_field
		inner join ncrm_tab
			on ncrm_field.tabid=ncrm_tab.tabid
		where ncrm_field.tabid not in(9,10,16,15,29) and ncrm_tab.presence = 0 and ncrm_tab.isentitytype=1";
...
function isRecordExists($recordId) {
	global $adb;
	$query = "SELECT crmid FROM ncrm_crmentity where crmid=? AND deleted=0";
...
function getTabInfo($tabId) {
	...
	$tabInfoResult = $adb->pquery('SELECT prefname, prefvalue FROM ncrm_tab_info WHERE tabid=?', array($tabId));
	$tabInfo = array();
...
function getBlockName($blockid) {
	...
	if(!empty($blockid) && $blockname === false){
		$block_res = $adb->pquery('SELECT blocklabel FROM ncrm_blocks WHERE blockid = ?',array($blockid));
...
function getActivityRelatedContacts($activityId) {
	...
	$query = 'SELECT * FROM ncrm_cntactivityrel WHERE activityid=?';
...
function isLeadConverted($leadId) {
	...
	$query = 'SELECT converted FROM ncrm_leaddetails WHERE converted = 1 AND leadid=?';
	$params = array($leadId);
...
function getSelectAllQuery($input,$module) {
	...
	if($module == "Calendar") {
		...
		if($module == 'Documents') {
			...
			$query .= " AND ncrm_notes.folderid in (".$folderid.")";
		}
	}
...
function getCampaignAccountIds($id) {
	global $adb;
	$sql="SELECT ncrm_account.accountid as id FROM ncrm_account
		INNER JOIN ncrm_campaignaccountrel ON ncrm_campaignaccountrel.accountid = ncrm_account.accountid
		LEFT JOIN ncrm_crmentity ON ncrm_crmentity.crmid = ncrm_account.accountid
		WHERE ncrm_campaignaccountrel.campaignid = ? AND ncrm_crmentity.deleted=0";
	$result = $adb->pquery($sql, array($id));
	return $result;
}
...
function getCampaignContactIds($id) {
	global $adb;
	$sql="SELECT ncrm_contactdetails.contactid as id FROM ncrm_contactdetails
		INNER JOIN ncrm_campaigncontrel ON ncrm_campaigncontrel.contactid = ncrm_contactdetails.contactid
		LEFT JOIN ncrm_crmentity ON ncrm_crmentity.crmid = ncrm_contactdetails.contactid
		WHERE ncrm_campaigncontrel.campaignid = ? AND ncrm_crmentity.deleted=0";
	$result = $adb->pquery($sql, array($id));
	return $result;
}
...
function getCampaignLeadIds($id) {
	global $adb;
	$sql="SELECT ncrm_leaddetails.leadid as id FROM ncrm_leaddetails
		INNER JOIN ncrm_campaignleadrel ON ncrm_campaignleadrel.leadid = ncrm_leaddetails.leadid
		LEFT JOIN ncrm_crmentity ON ncrm_crmentity.crmid = ncrm_leaddetails.leadid
		WHERE ncrm_campaignleadrel.campaignid = ? AND ncrm_crmentity.deleted=0";
	$result = $adb->pquery($sql, array($id));
	return $result;
}
