 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /advent/projects/wesat/ncrm_crm/sugarcrm/include/utils/EditViewUtils.php,v 1.188 2005/04/29 05:5 * 4:39 rank Exp
...
/** This function returns the ncrm_invoice object populated with the details from sales order object.
...
/** This function returns the detailed list of ncrm_products associated to a given entity or a record.
...
function getAssociatedProducts($module,$focus,$seid='')
{
	...
	{
		$query="SELECT
					case when ncrm_products.productid != '' then ncrm_products.productname else ncrm_service.servicename end as productname,
 		            case when ncrm_products.productid != '' then ncrm_products.product_no else ncrm_service.service_no end as productcode,
					case when ncrm_products.productid != '' then ncrm_products.unit_price else ncrm_service.unit_price end as unit_price,
 		            case when ncrm_products.productid != '' then ncrm_products.qtyinstock else 'NA' end as qtyinstock,
 		            case when ncrm_products.productid != '' then 'Products' else 'Services' end as entitytype,
 		                        ncrm_inventoryproductrel.listprice,
 		                        ncrm_inventoryproductrel.description AS product_description,
 		                        ncrm_inventoryproductrel.*,ncrm_crmentity.deleted
 	                            FROM ncrm_inventoryproductrel
								LEFT JOIN ncrm_crmentity ON ncrm_crmentity.crmid=ncrm_inventoryproductrel.productid
 		                        LEFT JOIN ncrm_products
 		                                ON ncrm_products.productid=ncrm_inventoryproductrel.productid
 		                        LEFT JOIN ncrm_service
	...
	elseif($module == 'Potentials')
	{
		$query="SELECT
 		                        ...
 		                        ncrm_products.productcode,
 		                        ncrm_products.unit_price,
 		                        ncrm_products.qtyinstock,
 		                        ncrm_seproductsrel.*,ncrm_crmentity.deleted,
 		                        ncrm_crmentity.description AS product_description
 		                        FROM ncrm_products
 		                        INNER JOIN ncrm_crmentity
 		                                ON ncrm_crmentity.crmid=ncrm_products.productid
 		                        INNER JOIN ncrm_seproductsrel
 		                                ON ncrm_seproductsrel.productid=ncrm_products.productid
 		                        WHERE ncrm_seproductsrel.crmid=?";
			$params = array($seid);
	}
	elseif($module == 'Products')
	{
		$query="SELECT
 		                        ncrm_products.productid,
 		                        ncrm_products.productcode,
 		                        ncrm_products.productname,
 		                        ncrm_products.unit_price,
 		                        ncrm_products.qtyinstock,ncrm_crmentity.deleted,
 		                        ncrm_crmentity.description AS product_description,
	...
	{
		$query="SELECT
 		                        ncrm_service.serviceid AS productid,
 		                        'NA' AS productcode,
 		                        ncrm_service.servicename AS productname,
 		                        ncrm_service.unit_price AS unit_price,
 		                        'NA' AS qtyinstock,ncrm_crmentity.deleted,
 		                        ncrm_crmentity.description AS product_description,
 		                       	'Services' AS entitytype
 								FROM ncrm_service
 		                        INNER JOIN ncrm_crmentity
 		                                ON ncrm_crmentity.crmid=ncrm_service.serviceid
 		                        WHERE ncrm_crmentity.deleted=0
 		                                AND serviceid=?";
			$params = array($seid);
	}
	...
	for($i=1;$i<=$num_rows;$i++)
	{
		...
		if(empty($focus->mode) && $seid!=''){
			$sub_prod_query = $adb->pquery("SELECT crmid as prod_id from ncrm_seproductsrel WHERE productid=? AND setype='Products'",array($seid));
		} else {
			$sub_prod_query = $adb->pquery("SELECT productid as prod_id from ncrm_inventorysubproductrel WHERE id=? AND sequence_no=?",array($focus->id,$i));
		}
		$subprodid_str='';
		$subprodname_str='';
	...
	//if taxtype is group then the tax should be same for all products in ncrm_inventoryproductrel table
...
/** This function returns the data type of the ncrm_fields, with ncrm_field label, which is used for javascript validation.
* Param $validationData - array of ncrm_fieldnames with datatype
* Return type array
*/
