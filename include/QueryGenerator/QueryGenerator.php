...
class QueryGenerator {
	...
	public function initForCustomViewById($viewId) {
		...
		foreach ($this->customViewColumnList as $customViewColumnInfo) {
			...
			if(empty($details[2]) && $details[1] == 'crmid' && $details[0] == 'ncrm_crmentity') {
				$name = 'id';
			...
			if(count($filtercolumns) > 0) {
				...
				foreach ($filtercolumns as $index=>$filter) {
                    ...
                    if($nameComponents[2] == 'due_date' && $nameComponents[3] == 'Events_End_Date_&_Time')
                        $nameComponents[4] = 'DT';
					if(empty($nameComponents[2]) && $nameComponents[1] == 'crmid' && $nameComponents[0] == 'ncrm_crmentity') {
		...
		foreach ($this->fields as $field) {
			...
			if($this->meta->getEntityName() == 'Calendar' && ($field == 'date_start' || $field == 'due_date' || $field == 'taskstatus' || $field == 'eventstatus')) {
				if($field=='date_start') {
					...
					$sql = "CASE WHEN (ncrm_activity.status not like '') THEN ncrm_activity.status ELSE ncrm_activity.eventstatus END AS ";
					if ( $field == 'taskstatus') {
						$sql .= "status";
					} else {
						$sql .= $field;
	...
	public function getFromClause() {
			...
			if($field->getFieldDataType() == 'reference') {
				...
				foreach($moduleList as $module) {
					if($module == 'Users' && $baseModule != 'Users') {
						$tableJoinCondition[$fieldName]['ncrm_users'.$fieldName] = $field->getTableName().
								".".$field->getColumnName()." = ncrm_users".$fieldName.".id";
						$tableJoinCondition[$fieldName]['ncrm_groups'.$fieldName] = $field->getTableName().
								".".$field->getColumnName()." = ncrm_groups".$fieldName.".groupid";
						$tableJoinMapping['ncrm_users'.$fieldName] = 'LEFT JOIN ncrm_users AS';
						$tableJoinMapping['ncrm_groups'.$fieldName] = 'LEFT JOIN ncrm_groups AS';
						$i++;
					}
				}
			} elseif($field->getFieldDataType() == 'owner') {
				$tableList['ncrm_users'] = 'ncrm_users';
				$tableList['ncrm_groups'] = 'ncrm_groups';
				$tableJoinMapping['ncrm_users'] = 'LEFT JOIN';
				$tableJoinMapping['ncrm_groups'] = 'LEFT JOIN';
		...
		foreach ($this->whereFields as $fieldName) {
			...
			if($field->getFieldDataType() == 'reference') {
				...
				// This is special condition as the data is not stored in the base table, 
                                ...
                                if ($fieldName == 'parent_id' && $field->getTableName() == 'ncrm_seactivityrel') {
                                    $tableJoinMapping[$field->getTableName()] = 'LEFT JOIN';
                                } else if ($fieldName == 'contact_id' && $field->getTableName() == 'ncrm_cntactivityrel') {
                                    $tableJoinMapping[$field->getTableName()] = "LEFT JOIN";
                                } else {
                                    $tableJoinMapping[$field->getTableName()] = 'INNER JOIN';
                                }
		...
		if( $this->meta->getTabName() == 'Documents') {
			$tableJoinCondition['folderid'] = array(
				'ncrm_attachmentsfolderfolderid'=>"$baseTable.folderid = ncrm_attachmentsfolderfolderid.folderid"
			);
			$tableJoinMapping['ncrm_attachmentsfolderfolderid'] = 'INNER JOIN ncrm_attachmentsfolder';
		}
			...
			foreach ($this->referenceModuleField as $index=>$conditionInfo) {
				...
				if(!in_array($tableName, $referenceFieldTableList)) {
					if($referenceFieldObject->getFieldName() == 'parent_id' && ($this->getModule() == 'Calendar' || $this->getModule() == 'Events')) {
						$sql .= ' LEFT JOIN ncrm_seactivityrel ON ncrm_seactivityrel.activityid = ncrm_activity.activityid ';
					}
					...
					if($referenceFieldObject->getFieldName() == 'contact_id' && ($this->getModule() == 'Calendar' || $this->getModule() == 'Events')) {
						$sql .= ' LEFT JOIN ncrm_cntactivityrel ON ncrm_cntactivityrel.activityid = ncrm_activity.activityid ';
					}
        ...
        foreach ($this->conditionals as $index=>$conditionInfo) {
			...
			foreach ($valueSqlList as $valueSql) {
				if (in_array($fieldName, $this->referenceFieldList)) {
                    ...
                    if($fieldName == 'created_user_id'){
                        $concatSql = getSqlForNameInDisplayFormat(array('first_name'=>"ncrm_users$fieldName.first_name",'last_name'=>"ncrm_users$fieldName.last_name"), 'Users');
                        $fieldSql .= "$fieldGlue (trim($concatSql) $valueSql)";
                    }else{
						$concatSql = getSqlForNameInDisplayFormat(array('first_name'=>"ncrm_users.first_name",'last_name'=>"ncrm_users.last_name"), 'Users');
						$fieldSql .= "$fieldGlue (trim($concatSql) $valueSql or "."ncrm_groups.groupname $valueSql)";
                    }
					...
					if($fieldName == 'date_start') {
						$dateFieldColumnName = 'ncrm_activity.date_start';
						$timeFieldColumnName = 'ncrm_activity.time_start';
					} else {
						$dateFieldColumnName = 'ncrm_activity.due_date';
						$timeFieldColumnName = 'ncrm_activity.time_end';
					}
					if($operator == 'bw') {
			...
			foreach ($advfilterlist as $groupindex=>$groupcolumns) {
				...
				if(count($filtercolumns) > 0) {
					...
					foreach ($filtercolumns as $index=>$filter) {
						...
						if(empty($name[2]) && $name[1] == 'crmid' && $name[0] == 'ncrm_crmentity') {
							$name = $this->getSQLColumn('id');
						} else {
							$name = $name[2];
						}
