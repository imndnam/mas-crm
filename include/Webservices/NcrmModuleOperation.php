...
class NcrmModuleOperation extends WebserviceEntityOperation {
	protected $tabId;
	protected $isEntity = true;
	protected $partialDescribeFields = null;
	
	public function NcrmModuleOperation($webserviceObject,$user,$adb,$log){
	...
	protected function getMetaInstance(){
		if(empty(WebserviceEntityOperation::$metaCache[$this->webserviceObject->getEntityName()][$this->user->id])){
			WebserviceEntityOperation::$metaCache[$this->webserviceObject->getEntityName()][$this->user->id]  = new NcrmCRMObjectMeta($this->webserviceObject,$this->user);
		}
		return WebserviceEntityOperation::$metaCache[$this->webserviceObject->getEntityName()][$this->user->id];
	}
	
	public function create($elementType,$element){
		$crmObject = new NcrmCRMObject($elementType, false);
	...
	public function retrieve($id){
		...
		$crmObject = new NcrmCRMObject($this->tabId, true);
    ...
    public function relatedIds($id, $relatedModule, $relatedLabel, $relatedHandler=null) {
		...
		$sourceRecordModel = Ncrm_Record_Model::getInstanceById($ids[1], $sourceModule);
		$targetModel       = Ncrm_RelationListView_Model::getInstance($sourceRecordModel, $relatedModule, $relatedLabel);
        $sql = $targetModel->getRelationQuery();

        $relatedWebserviceObject = NcrmWebserviceObject::fromName($adb,$relatedModule);
        ...
        $sql = sprintf("SELECT DISTINCT concat('%sx',ncrm_crmentity.crmid) as wsid FROM %s", $relatedModuleWSId, $sqlFromPart);
