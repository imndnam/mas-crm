...
require_once "include/Webservices/LineItem/NcrmInventoryOperation.php";
...
/**
 * Description of NcrmLineItemOperation
 */
class NcrmLineItemOperation  extends NcrmActorOperation {
	...
	public function __construct($webserviceObject,$user,$adb,$log) {
		...
		$this->meta = new NcrmLineItemMeta($this->entityTableName,$webserviceObject,$adb,$user);
	...
	private function updateTaxes($createdElement){
		if(count($this->taxList) > 0 ) {
			...
			$id = $id[1];
			$sql = 'UPDATE ncrm_inventoryproductrel set ';
	...
	function getProductPrice($productId){
		...
		$sql = "select unit_price from ncrm_products where productid=?";
	...
	public function getCollectiveTaxList(){
		$db = PearDatabase::getInstance();
		$sql = 'select * from ncrm_inventorytaxinfo where deleted=0';
		$params = array();
		$result = null;
		$transactionSuccessful = vtws_runQueryAsTransaction($sql,$params,$result);
		if(!$transactionSuccessful){
	...
	private function getProductTaxList($productId){
		$db = PearDatabase::getInstance();
		$sql = 'select * from ncrm_producttaxrel inner join ncrm_inventorytaxinfo on
			ncrm_producttaxrel.taxid=ncrm_inventorytaxinfo.taxid and deleted=0
			where productid=?';
		$params = array($productId);
		$result = null;
		$transactionSuccessful = vtws_runQueryAsTransaction($sql,$params,$result);
