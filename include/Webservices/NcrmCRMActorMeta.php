...
class NcrmCRMActorMeta extends EntityMeta {
	protected $pearDB;
	protected static $fieldTypeMapping = array();
	protected static $referenceTypeMapping = array();
	
	function NcrmCRMActorMeta($tableName,$webserviceObject,$adb,$user){
	...
	protected function getReferenceList($dbField, $tableName){
		...
		if(!isset(NcrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name])){
			$this->getFieldType($dbField, $tableName);
		}
		$fieldTypeData = NcrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name];
		
		if (empty(NcrmCRMActorMeta::$referenceTypeMapping)) {
			$sql = "SELECT * FROM ncrm_ws_entity_referencetype";
			...
			for ($index =0, $count = $this->pearDB->num_rows($result); $index < $count; ++$index) {
				...
				NcrmCRMActorMeta::$referenceTypeMapping[$row['fieldtypeid']][] = $row['type'];
			}
		}
		
		$referenceTypes = array();
		if (isset(NcrmCRMActorMeta::$referenceTypeMapping[$fieldTypeData['fieldtypeid']])) {
			$referenceTypes = NcrmCRMActorMeta::$referenceTypeMapping[$fieldTypeData['fieldtypeid']];
		}
	...
	protected function getFieldType($dbField,$tableName){
		...
		if(isset(NcrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name])){
			if(NcrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name] === 'null'){
				return null;
			}
			$row = NcrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name];
			return $row['fieldtype'];
		}
		
		if (empty(NcrmCRMActorMeta::$fieldTypeMapping)) {
			// Optimization to avoid repeated initialization
			$sql = "select * from ncrm_ws_entity_fieldtype";
			$result = $this->pearDB->pquery($sql,array());
			$rowCount = $this->pearDB->num_rows($result);
			while ($rowCount) {
				$row = $this->pearDB->query_result_rowdata($result,$rowCount-1);
				NcrmCRMActorMeta::$fieldTypeMapping[$row['table_name']][$row['field_name']] = $row;
				--$rowCount;
			}
		}
		
		if (!isset(NcrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name])) {
			NcrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name] = 'null';
			return null;
		}
		return NcrmCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name]['fieldtype'];
	}
	...
	public function getNameFields(){
		$query = "select name_fields from ncrm_ws_entity_name where entity_id = ?";
	...
	public function getEntityDeletedQuery() {
		if($this->getEntityName() == 'Currency'){
			return 'ncrm_currency_info.deleted=0';
		}
