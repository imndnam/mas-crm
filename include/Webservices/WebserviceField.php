...
class WebserviceField{
	...
	public function getReferenceList(){
		...
		if($this->referenceList === null){
			...
			if($this->getUIType() != $this->genericUIType){
				$sql = "select * from ncrm_ws_referencetype where fieldtypeid=?";
				$params = array($fieldTypeData['fieldtypeid']);
			}else{
				$sql = 'select relmodule as type from ncrm_fieldmodulerel where fieldid=?';
	...
	private function getFieldTypeFromUIType(){
		...
		if(empty(self::$fieldTypeMapping)) {
			$result = $this->pearDB->pquery("select * from ncrm_ws_fieldtype", array());
	...
	function getPickListOptions(){
		...
		$options = array();
		$sql = "select * from ncrm_picklist where name=?";
		$result = $this->pearDB->pquery($sql,array($fieldName));
		$numRows = $this->pearDB->num_rows($result);
		if($numRows == 0){
			$sql = "select * from ncrm_$fieldName";
