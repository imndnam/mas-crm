...
function vtws_convertlead($entityvalues, $user) {
	...
	$leadObject = NcrmWebserviceObject::fromName($adb, 'Leads');
	...
	foreach ($availableModules as $entityName) {
		if ($entityvalues['entities'][$entityName]['create']) {
			...
			$entityObject = NcrmWebserviceObject::fromName($adb, $entityvalue['name']);
			...
			try {
				...
				if ($entityvalue['name'] == 'Accounts') {
					$sql = "SELECT ncrm_account.accountid FROM ncrm_account,ncrm_crmentity WHERE ncrm_crmentity.crmid=ncrm_account.accountid AND ncrm_account.accountname=? AND ncrm_crmentity.deleted=0";
...
function vtws_updateConvertLeadStatus($entityIds, $leadId, $user) {
	...
	if ($entityIds['Accounts'] != '' || $entityIds['Contacts'] != '') {
		$sql = "UPDATE ncrm_leaddetails SET converted = 1 where leadid=?";
		$result = $adb->pquery($sql, array($leadIdComponents[1]));
		if ($result === false) {
		...
		$sql = "DELETE FROM ncrm_campaignleadrel WHERE leadid=?";
		$adb->pquery($sql, array($leadIdComponents[1]));

		$sql = "DELETE FROM ncrm_tracker WHERE item_id=?";
		$adb->pquery($sql, array($leadIdComponents[1]));

		//update the modifiedtime and modified by information for the record
		$leadModifiedTime = $adb->formatDate(date('Y-m-d H:i:s'), true);
		$crmentityUpdateSql = "UPDATE ncrm_crmentity SET modifiedtime=?, modifiedby=? WHERE crmid=?";
		$adb->pquery($crmentityUpdateSql, array($leadModifiedTime, $user->id, $leadIdComponents[1]));
                ...
                $em->triggerEvent("ncrm.entity.leadconverted", $entityData);
	}
