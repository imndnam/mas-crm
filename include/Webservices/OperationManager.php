 ...
 *************************************************************************************/
	...
	class OperationManager{
		...
		private function fillOperationDetails($operationName){
			$sql = "select * from ncrm_ws_operation where name=?";
		...
		private function fillOperationParameters(){
			$sql = "select * from ncrm_ws_operation_parameters where operationid=? order by sequence";
			...
			try{
				...
				if(!$this->preLogin){
					...
					if(is_array($userDetails)){
						...
						global $adb;
						$webserviceObject = NcrmWebserviceObject::fromName($adb,"Users");
						$userId = vtws_getId($webserviceObject->getEntityId(),$userDetails->id);
						$ncrmVersion = vtws_getNcrmVersion();
						$resp = array("sessionName"=>$this->sessionManager->getSessionId(),"userId"=>$userId,"version"=>$API_VERSION,"ncrmVersion"=>$ncrmVersion);
						return $resp;
					}
				}
