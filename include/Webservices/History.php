...
function vtws_history($element, $user) {
	...
	$sql = 'SELECT ncrm_modtracker_basic.* FROM ncrm_modtracker_basic
		INNER JOIN ncrm_crmentity ON ncrm_modtracker_basic.crmid = ncrm_crmentity.crmid
		AND ncrm_crmentity.deleted = 0';

	if ($mode == 'Private') {
		$sql .= ' WHERE ncrm_modtracker_basic.whodid = ?';
		$params[] = $user->getId();
	} else if ($mode == 'All') {
		if ($acrossAllModule) {
			// TODO collate only active (or enabled) modules for tracking.
		} else if($moduleName) {
            $sql .= ' WHERE ncrm_modtracker_basic.module = ?';
            $params[] = $moduleName;
        } else {
            $sql .= ' WHERE ncrm_modtracker_basic.crmid = ?';
	...
	$sql .= sprintf(' ORDER BY ncrm_modtracker_basic.id DESC LIMIT %s,%s', $start, $MAXLIMIT);
	...
	if (!empty($orderedIds)) {
		$sql = 'SELECT ncrm_modtracker_detail.* FROM ncrm_modtracker_detail';
		$sql .= ' WHERE ncrm_modtracker_detail.id IN (' . generateQuestionMarks($orderedIds) . ')';
