...
class Ncrm_Tools_Console_Controller {
	const PROMPT_ANY      = 1;
	...
	protected function handleCreateModule() {
		$controller = new Ncrm_Tools_Console_ModuleController();
		$controller->setArguments($this->arguments, $this->interactive)->handle();
	}

	protected function handleCreateLanguage() {
		$controller = new Ncrm_Tools_Console_LanguageController();
		$controller->setArguments($this->arguments, $this->interactive)->handle();
	}

	protected function handleCreateTestLanguage() {
		$controller = new Ncrm_Tools_Console_TestLanguageController();
		$controller->setArguments($this->arguments, $this->interactive)->handle();
	}

	protected function handleCreateLayout() {
		$controller = new Ncrm_Tools_Console_LayoutController();
		$controller->setArguments($this->arguments, $this->interactive)->handle();
	}
	
	protected function handleImportModule() {
		$controller = new Ncrm_Tools_Console_ImportController();
		$controller->setArguments($this->arguments, $this->interactive)->handle();
	}
	
	protected function handleUpdateModule() {
		$controller = new Ncrm_Tools_Console_UpdateController();
		$controller->setArguments($this->arguments, $this->interactive)->handle();
	}
	
	protected function handleRemoveModule() {
		$controller = new Ncrm_Tools_Console_RemoveController();
		$controller->setArguments($this->arguments, $this->interactive)->handle();
	}
...
class Ncrm_Tools_Console_ModuleController extends Ncrm_Tools_Console_Controller {
	...
	public function find($name) {
		return Ncrm_Module::getInstance($name);
	}
	...
	protected function create($moduleInformation) {
		...
		/** Common fields that should be in every module, linked to NCRM core table */
		$field2 = new Ncrm_Field();
		$field2->name = 'assigned_user_id';
		$field2->label = 'Assigned To';
		$field2->table = 'ncrm_crmentity';
	...
	protected function createFiles(Ncrm_Module $module, Ncrm_Field $entityField) {
...
class Ncrm_Tools_Console_LayoutController extends Ncrm_Tools_Console_Controller {
	...
	protected function create($layoutInformation) {
		...
		foreach ($files as $file) {
			$tplfolder = $layoutdir . "modules/Ncrm";
...
class Ncrm_Tools_Console_LanguageController extends Ncrm_Tools_Console_Controller {
	...
	protected function deploy($languageInformation) {
		...
		$check = $db->pquery('SELECT 1 FROM ncrm_language WHERE prefix=?', $languageInformation['prefix']);
		if ($check && $db->num_rows($check)) {
			;
		} else {
			$db->pquery('INSERT INTO ncrm_language (id,name,prefix,label,lastupdated,isdefault,active) VALUES(?,?,?,?,?,?,?)',
					array($db->getUniqueId('ncrm_language'), $languageInformation['label'], $languageInformation['prefix'],
							$languageInformation['label'], date('Y-m-d H:i:s'), 0, 1));
		}
	}
}

class Ncrm_Tools_Console_TestLanguageController extends Ncrm_Tools_Console_LanguageController {
...
class Ncrm_Tools_Console_ImportController extends Ncrm_Tools_Console_Controller {
...
class Ncrm_Tools_Console_UpdateController extends Ncrm_Tools_Console_Controller {
...
class Ncrm_Tools_Console_RemoveController extends Ncrm_Tools_Console_Controller {
...
if (php_sapi_name() == 'cli') {
	Ncrm_Tools_Console_Controller::run();
} else {
