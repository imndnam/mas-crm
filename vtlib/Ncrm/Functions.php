...
class Ncrm_Functions {
	...
	static function getTranslatedString($str, $module = '') {
		return Ncrm_Language_Handler::getTranslatedString($str, $module);
	}
	...
	static function userCurrencyId($userid) {
		...
		if (!isset(self::$userIdCurrencyIdCache[$userid])) {
			$result = $adb->pquery('SELECT id,currency_id FROM ncrm_users', array());
	...
	protected static function getCurrencyInfo($currencyid) {
		...
		if (!isset(self::$currencyInfoCache[$currencyid])) {
			$result = $adb->pquery('SELECT * FROM ncrm_currency_info', array());
			while ($row = $adb->fetch_array($result)) {
	...
	static function getCurrencyName($currencyid, $show_symbol = true) {
		...
		if ($show_symbol) {
			return sprintf("%s : %s", Ncrm_Deprecated::getTranslatedCurrencyString($currencyInfo['currency_name']), $currencyInfo['currency_symbol']);
		}
	...
	protected static function getBasicModuleInfo($mixed) {
		...
		if ($reload) {
			global $adb;
			$result = $adb->pquery('SELECT tabid, name, ownedby FROM ncrm_tab', array());
			while ($row = $adb->fetch_array($result)) {
	...
	static function getModuleData($mixed) {
		...
		if ($reload) {
			global $adb;
			$result = $adb->pquery('SELECT * FROM ncrm_tab', array());
			while ($row = $adb->fetch_array($result)) {
				self::$moduleIdNameCache[$row['tabid']] = $row;
				self::$moduleNameIdCache[$row['name']]  = $row;
	...
	static function getEntityModuleInfo($mixed) {
		...
		if ($name && !isset(self::$moduleEntityCache[$name])) {
			global $adb;
			$result = $adb->pquery('SELECT fieldname,modulename,tablename,entityidfield,entityidcolumn from ncrm_entityname', array());
			while ($row = $adb->fetch_array($result)) {
	...
	protected static function getCRMRecordMetadata($mixedid) {
		...
		if ($missing) {
			$sql = sprintf("SELECT crmid, setype, label FROM ncrm_crmentity WHERE %s", implode(' OR ', array_fill(0, count($missing), 'crmid=?')));
	...
	static function updateCRMRecordLabel($module, $id, $label) {
		...
		if ($labelInfo) {
			...
			$adb->pquery('UPDATE ncrm_crmentity SET label=? WHERE crmid=?', array($label, $id));
	...
	static function computeCRMRecordLabels($module, $ids) {
		...
		if ($module) {
			...
			if ($ids) {
				...
				if ($module == 'Groups') {
					$metainfo = array('tablename' => 'ncrm_groups','entityidfield' => 'groupid','fieldname' => 'groupname');
				} else if ($module == 'DocumentFolders') { 
                                        $metainfo = array('tablename' => 'ncrm_attachmentsfolder','entityidfield' => 'folderid','fieldname' => 'foldername'); 
                                } else {
	...
	static function getGroupName($id) {
		...
		if (!self::$groupIdNameCache[$id]) {
			$result = $adb->pquery('SELECT groupid, groupname FROM ncrm_groups');
			while ($row = $adb->fetch_array($result)) {
	...
	static function getUserName($id) {
		...
		if (!self::$userIdNameCache[$id]) {
			$result = $adb->pquery('SELECT id, user_name FROM ncrm_users');
			while ($row = $adb->fetch_array($result)) {
	...
	static function getModuleFieldInfos($mixed) {
		...
		$module = $moduleInfo['name'];
                ...
                $no_of_fields = $adb->pquery('SELECT COUNT(fieldname) FROM ncrm_field WHERE tabid=?',array(self::getModuleId($module)));
		...
		if ($module && (!isset(self::$moduleFieldInfoByNameCache[$module]) || ((int)$fields_count != (int)$cached_fields_count))) {
			$result =
				($module == 'Calendar')?
				$adb->pquery('SELECT * FROM ncrm_field WHERE tabid=? OR tabid=?', array(9, 16)) :
				$adb->pquery('SELECT * FROM ncrm_field WHERE tabid=?', array(self::getModuleId($module)));
	...
	static function getModuleFieldInfoWithId($fieldid) {
		global $adb;
		$result = $adb->pquery('SELECT * FROM ncrm_field WHERE fieldid=?', array($fieldid));
	...
	static function getInventoryTermsAndCondition() {
		global $adb;
		$sql = "select tandc from ncrm_inventory_tandc";
...
//		}
	...
	static function getTicketComments($ticketid) {
		...
		$commentlist = '';
		$sql = "SELECT commentcontent FROM ncrm_modcomments WHERE related_to = ?";
	...
	static function getTagCloudView($id = "") {
		...
		if ($id == '') {
			...
			$query = "select visible from ncrm_homestuff where userid=? and stufftype='Tag Cloud'";
	...
	static function transformFieldTypeOfData($table_name, $column_name, $type_of_data) {
		...
		static $new_field_details = Array(
			...
			"ncrm_contactdetails:accountid" => "V",
			"ncrm_contactsubdetails:birthday" => "D",
			"ncrm_contactdetails:email" => "V",
			"ncrm_contactdetails:secondaryemail" => "V",
			//Potential Related Fields
			"ncrm_potential:campaignid" => "V",
			//Account Related Fields
			"ncrm_account:parentid" => "V",
			"ncrm_account:email1" => "V",
			"ncrm_account:email2" => "V",
			//Lead Related Fields
			"ncrm_leaddetails:email" => "V",
			"ncrm_leaddetails:secondaryemail" => "V",
			//Documents Related Fields
			"ncrm_senotesrel:crmid" => "V",
			//Calendar Related Fields
			"ncrm_seactivityrel:crmid" => "V",
			"ncrm_seactivityrel:contactid" => "V",
			"ncrm_recurringevents:recurringtype" => "V",
			//HelpDesk Related Fields
			"ncrm_troubletickets:parent_id" => "V",
			"ncrm_troubletickets:product_id" => "V",
			//Product Related Fields
			"ncrm_products:discontinued" => "C",
			"ncrm_products:vendor_id" => "V",
			"ncrm_products:parentid" => "V",
			//Faq Related Fields
			"ncrm_faq:product_id" => "V",
			//Vendor Related Fields
			"ncrm_vendor:email" => "V",
			//Quotes Related Fields
			"ncrm_quotes:potentialid" => "V",
			"ncrm_quotes:inventorymanager" => "V",
			"ncrm_quotes:accountid" => "V",
			//Purchase Order Related Fields
			"ncrm_purchaseorder:vendorid" => "V",
			"ncrm_purchaseorder:contactid" => "V",
			//SalesOrder Related Fields
			"ncrm_salesorder:potentialid" => "V",
			"ncrm_salesorder:quoteid" => "V",
			"ncrm_salesorder:contactid" => "V",
			"ncrm_salesorder:accountid" => "V",
			//Invoice Related Fields
			"ncrm_invoice:salesorderid" => "V",
			"ncrm_invoice:contactid" => "V",
			"ncrm_invoice:accountid" => "V",
			//Campaign Related Fields
			"ncrm_campaign:product_id" => "V",
			//Related List Entries(For Report Module)
			"ncrm_activityproductrel:activityid" => "V",
			"ncrm_activityproductrel:productid" => "V",
			"ncrm_campaigncontrel:campaignid" => "V",
			"ncrm_campaigncontrel:contactid" => "V",
			"ncrm_campaignleadrel:campaignid" => "V",
			"ncrm_campaignleadrel:leadid" => "V",
			"ncrm_cntactivityrel:contactid" => "V",
			"ncrm_cntactivityrel:activityid" => "V",
			"ncrm_contpotentialrel:contactid" => "V",
			"ncrm_contpotentialrel:potentialid" => "V",
			"ncrm_pricebookproductrel:pricebookid" => "V",
			"ncrm_pricebookproductrel:productid" => "V",
			"ncrm_seactivityrel:crmid" => "V",
			"ncrm_seactivityrel:activityid" => "V",
			"ncrm_senotesrel:crmid" => "V",
			"ncrm_senotesrel:notesid" => "V",
			"ncrm_seproductsrel:crmid" => "V",
			"ncrm_seproductsrel:productid" => "V",
			"ncrm_seticketsrel:crmid" => "V",
			"ncrm_seticketsrel:ticketid" => "V",
			"ncrm_vendorcontactrel:vendorid" => "V",
			"ncrm_vendorcontactrel:contactid" => "V",
			"ncrm_pricebook:currency_id" => "V",
		);
	...
	static function getPickListValuesFromTableForRole($tablename, $roleid) {
		global $adb;
		$query = "select $tablename from ncrm_$tablename inner join ncrm_role2picklist on ncrm_role2picklist.picklistvalueid = ncrm_$tablename.picklist_valueid where roleid=? and picklistid in (select picklistid from ncrm_picklist) order by sortid";
	...
	static function getActivityType($id) {
		global $adb;
		$query = "select activitytype from ncrm_activity where activityid=?";
		$res = $adb->pquery($query, array($id));
	...
	static function getInvoiceStatus($id) {
		global $adb;
		$result = $adb->pquery("SELECT invoicestatus FROM ncrm_invoice where invoiceid=?", array($id));
    ...
    */
    static function getUnitPrice($productid, $module='Products') {
        ...
        if($module == 'Services') {
            $query = "select unit_price from ncrm_service where serviceid=?";
        } else {
            $query = "select unit_price from ncrm_products where productid=?";
        }
