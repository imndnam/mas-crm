...
include_once('vtlib/Ncrm/ModuleBasic.php');
/**
 * Provides API to work with NCRM Modules
 * @package vtlib
 */
class Ncrm_Module extends Ncrm_ModuleBasic {
	...
	function __getRelatedListUniqueId() {
		global $adb;
		return $adb->getUniqueID('ncrm_relatedlists');
	}
	...
	function __getNextRelatedListSequence() {
		global $adb;
		$max_sequence = 0;
		$result = $adb->pquery("SELECT max(sequence) as maxsequence FROM ncrm_relatedlists WHERE tabid=?", Array($this->id));
	 ...
	 * @param Ncrm_Module Instance of target module with which relation should be setup
	 ...
	 *
	 * @internal Creates table ncrm_crmentityrel if it does not exists
	 */
	function setRelatedList($moduleInstance, $label='', $actions=false, $function_name='get_related_list') {
		...
		if(!Ncrm_Utils::CheckTable('ncrm_crmentityrel')) {
		...
		// Add column to ncrm_relatedlists to save extended actions
		Ncrm_Utils::AddColumn('ncrm_relatedlists', 'actions', 'VARCHAR(50)');

		$adb->pquery("INSERT INTO ncrm_relatedlists(relation_id,tabid,related_tabid,name,sequence,label,presence,actions) VALUES(?,?,?,?,?,?,?,?)",
	...
	function unsetRelatedList($moduleInstance, $label='', $function_name='get_related_list') {
		...
		$adb->pquery("DELETE FROM ncrm_relatedlists WHERE tabid=? AND related_tabid=? AND name=? AND label=?",
	...
	function addLink($type, $label, $url, $iconpath='', $sequence=0, $handlerInfo=null) {
		Ncrm_Link::addLink($this->id, $type, $label, $url, $iconpath, $sequence, $handlerInfo);
	}
	...
	function deleteLink($type, $label, $url=false) {
		Ncrm_Link::deleteLink($this->id, $type, $label, $url);
	}
	 ...
	 */
	function getLinks() {
		return Ncrm_Link::getAll($this->id);
	}
	...
	function getLinksForExport() {
		return Ncrm_Link::getAllForExport($this->id);
	}
	...
	function initWebservice() {
		Ncrm_Webservice::initialize($this);
	}
	...
	function deinitWebservice() {
		Ncrm_Webservice::uninitialize($this);
	}
	...
	static function getInstance($value) {
		$instance = false;
		$data = Ncrm_Functions::getModuleData($value);
		if ($data) {
			$instance = new self();
	...
	static function getClassInstance($modulename) {
		...
		if(Ncrm_Utils::checkFileAccessForInclusion($filepath, false)) {
