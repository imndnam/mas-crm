...
/**
 * Provides API to work with NCRM Module Blocks
 * @package vtlib
 */
class Ncrm_Block {
	...
	function __getUniqueId() {
		...
		$maxblockid = $adb->getUniqueID('ncrm_blocks');
		return $maxblockid;
	}
	...
	function __getNextSequence() {
		global $adb;
		$result = $adb->pquery("SELECT MAX(sequence) as max_sequence from ncrm_blocks where tabid = ?", Array($this->module->id));
		$maxseq = 0;
	 ...
	 * @param Ncrm_Module Instance of module to which this block is associated
	 * @access private
	 */
	function initialize($valuemap, $moduleInstance=false) {
		...
		$this->module= $moduleInstance ? $moduleInstance : Ncrm_Module::getInstance($tabid);
	}
	...
	function __create($moduleInstance) {
		...
		$adb->pquery("INSERT INTO ncrm_blocks(blockid,tabid,blocklabel,sequence,show_title,visible,create_view,edit_view,detail_view,iscustom)
	...
	function __delete() {
		...
		$adb->pquery("DELETE FROM ncrm_blocks WHERE blockid=?", Array($this->id));
		self::log("DONE");
	}

	/**
	 * Save this block instance
	 * @param Ncrm_Module Instance of the module to which this block is associated
	 */
	...
	function delete($recursive=true) {
		if($recursive) {
			$fields = Ncrm_Field::getAllForBlock($this);
	 ...
	 * @param Ncrm_Field Instance of field to add to this block.
	 ...
	 * @param Ncrm_Module Instance of the module if block label is passed
	 */
	static function getInstance($value, $moduleInstance=false) {
		...
		if($moduleInstance && $cache->getBlockInstance($value, $moduleInstance->id)){
			...
			if(Ncrm_Utils::isNumber($value)) {
				$query = "SELECT * FROM ncrm_blocks WHERE blockid=?";
				$queryParams = Array($value);
			} else {
				$query = "SELECT * FROM ncrm_blocks WHERE blocklabel=? AND tabid=?";
	...
	static function getAllForModule($moduleInstance) {
		...
		$query = "SELECT * FROM ncrm_blocks WHERE tabid=? ORDER BY sequence";
	 ...
	 * @param Ncrm_Module Instnace of module to use
	...
	static function deleteForModule($moduleInstance, $recursive=true) {
		global $adb;
		if($recursive) Ncrm_Field::deleteForModule($moduleInstance);
		$adb->pquery("DELETE FROM ncrm_blocks WHERE tabid=?", Array($moduleInstance->id));
