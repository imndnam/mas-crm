 ...
 * All Rights Reserved.
 ************************************************************************************/
include_once('vtlib/Ncrm/PackageImport.php');

/**
 * Provides API to update module into NCRM
 * @package vtlib
 */
class Ncrm_PackageUpdate extends Ncrm_PackageImport {
     ...
     * @param Ncrm_Module Instance of the module to update
                ...
                foreach ($installSequenceArray as $sequence) {
                    foreach ($buildModuleArray as $moduleInfo) {
                        if($moduleInfo['install_sequence'] == $sequence) {
                            $moduleInstance = Ncrm_Module::getInstance($moduleInfo['name']);
    ...
    function update_Module($moduleInstance) {
        ...
        Ncrm_Module::fireEvent($moduleInstance->name,
                Ncrm_Module::EVENT_MODULE_PREUPDATE);
        ...
        Ncrm_Module::fireEvent($moduleInstance->name,
                Ncrm_Module::EVENT_MODULE_POSTUPDATE);
    }
                ...
                if(!empty($migrationnode->tables) && !empty($migrationnode->tables->table)) {
                    foreach($migrationnode->tables->table as $tablenode) {
                        ...
                        if(Ncrm_Utils::IsDestructiveSql($tablesql)) {
                            ...
                            Ncrm_Utils::ExecuteQuery($tablesql, true);
                            self::log("DONE");
                        }
                    }
                }
    ...
    function update_Blocks($modulenode, $moduleInstance) {
        ...
        foreach($modulenode->blocks->block as $blocknode) {
            $blockInstance = Ncrm_Block::getInstance((string)$blocknode->label, $moduleInstance);
            if(!$blockInstance) {
    ...
    function update_Fields($blocknode, $blockInstance, $moduleInstance) {
        ...
        foreach($blocknode->fields->field as $fieldnode) {
            $fieldInstance = Ncrm_Field::getInstance((string)$fieldnode->fieldname, $moduleInstance);
            if(!$fieldInstance) {
    ...
    function update_CustomViews($modulenode, $moduleInstance) {
        ...
        foreach($modulenode->customviews->customview as $customviewnode) {
            $filterInstance = Ncrm_Filter::getInstance($customviewnode->viewname, $moduleInstance);
            if(!$filterInstance) {
    ...
    function update_Event($modulenode, $moduleInstance, $eventnode) {
        //Ncrm_Event::register($moduleInstance, $eventnode->eventname, $eventnode->classname, $eventnode->filename);
    ...
    function update_Relatedlist($modulenode, $moduleInstance, $relatedlistnode) {
        $relModuleInstance = Ncrm_Module::getInstance((string)$relatedlistnode->relatedmodule);
	...
	function update_CronTasks($modulenode) {
		...
		$cronTasks = Ncrm_Cron::listAllInstancesByModule($modulenode->name);
		foreach ($modulenode->crons->cron as $importCronTask) {
			foreach($cronTasks as $cronTask) {
				if($cronTask->getName() == $importCronTask->name && $importCronTask->handler == $cronTask->getHandlerFile()) {
					Ncrm_Cron::deregister($importCronTask->name);
				}
			}
			...
			if((empty($importCronTask->sequence))){
				$importCronTask->sequence=Ncrm_Cron::nextSequence();
			}
			Ncrm_Cron::register("$importCronTask->name","$importCronTask->handler", "$importCronTask->frequency", "$modulenode->name","$importCronTask->status","$importCronTask->sequence","$importCronTask->description");
		}
	}
}
?>
